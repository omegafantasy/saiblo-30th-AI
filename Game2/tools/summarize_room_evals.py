...
def summarize() -> dict[str, Any]:
    ...
    for room in sorted(ROOM_DIR.glob('*_room')):
        ...
        if not rows:
            item['attempts'] += len(list((room / 'matches').glob('*')))
...
def main() -> int:
    ...
    print(json.dumps({'labels': len(data.get('labels', [])), 'out_json': str(OUT_JSON), 'out_md': str(OUT_MD)}, indent=2))
    return 0
