...
AI_TEMPLATE = r'''#!/usr/bin/env python3
from __future__ import annotations
...
def write_candidate(label: str, answer_mode: str) -> None:
    target = OUT / label
    if target.exists():
        shutil.rmtree(target)
    target.mkdir(parents=True)
    (target / 'ai.py').write_text(AI_TEMPLATE.format(answer_mode=answer_mode), encoding='utf-8')


def main() -> int:
    for label, answer_mode in SPECS.items():
        write_candidate(label, answer_mode)
    print('\n'.join(SPECS))
    return 0
