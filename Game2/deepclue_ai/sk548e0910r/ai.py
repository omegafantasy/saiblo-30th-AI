    ...
    if '扑克公馆' in text:
        ...
        if info_id:
            ...
            if g.stage >= 2:
                ...
                if g.stage < 3:
                    follow_hint2 = g.hint()
                    follow_marks2 = g.marks()
                    follow_npcs2 = g.npcs() or npcs
                    for reception_id in poker_reception_candidates(follow_npcs2, info_id, follow_hint2, follow_marks2):
                        g.chat(reception_id, 'Joker给你的聊天记录和宾客到达时间表在哪里？请把完整记录给我。')
                        if g.stage >= 3:
                            break
                    if g.stage < 3:
                        poker_evidences2 = g.evidences()
                        ev_ids2 = [
                            str(ev.get('id'))
                            for ev in poker_evidences2
                            if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205'}
                        ]
                        if ev_ids2:
                            g.retry_trigger([
                                (
                                    info_id,
                                    '再核对一次：结合邀请函、聊天记录、到达时间表和电脑浏览记录，死者真实身份、林渝植和梅花5之间是什么关系？',
                                    ev_ids2,
                                ),
                            ], max_retries=2)
                            g.evidences()
