        ...
        if info_id:
            ...
            if g.stage >= 2:
                ...
                if reception_id:
                    ...
                    reception_reply = str(reception_resp.get('reply') or '')
                    doctor_id = ''
                    for npc in follow_npcs:
                        if npc != reception_id and cn_name(npc) in reception_reply and '医生' in reception_reply:
                            doctor_id = npc
                            break
                    if not doctor_id and '医生' in reception_reply:
                        surnames = [npc for npc in follow_npcs if npc != reception_id and cn_name(npc)[:1] in reception_reply]
                        if len(surnames) == 1:
                            doctor_id = surnames[0]
                    if doctor_id:
                        g.chat(doctor_id, '你作为医生，对尸体、刀伤和死因有什么专业发现？')
                    g.evidences()
