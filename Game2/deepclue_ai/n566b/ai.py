...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        ask_all('先只按人物和时间线说明：袁樱瞳周四聚会、周五展示、凌晨1点照片、周六世纪林尸块、李海天旧案、生物馆、1919黑车、保安网站各自是谁亲眼确认的？')
        ask_all('请每个人只说本人亲眼确认的事实：谁从生物馆慌张跑出来，谁在看奇怪网站，谁坐1919黑车，谁捡手机，谁推黄色行李箱，谁知道投票多出一票。')
        yuan_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}]
        ask_all('重点追问李海天旧案：谁掌握尸检报告，蓝色背包海豚挂件是谁的，背刺失血和四肢被砍与袁樱瞳碎尸案有什么相同和不同？', yuan_ids)
        ask_all('请给我能证明生物馆跑出者身份的物证或证词，并说明这个人和张壹传闻、李海天尸检、袁樱瞳手机照片之间的关系。', yuan_ids)
        ask_all('结合袁樱瞳手机、投票纸、李海天尸检报告和目击证词，直接指出真正隐瞒者或凶手是谁，为什么不是只赢了一票的竞争者。', yuan_ids)
        runner = ''
