                                ...
                                if not (post606_set & {'605', '607', '608'}):
                                    for npc_id in post606_targets[:8]:
                                        if npc_id in asked_post606:
                                            continue
                                        asked_post606.add(npc_id)
                                        chat_visible_or_probe(g, npc_id, follow_npcs, '606已出现，现在只推进下一门槛：请用可核验证据证明谁杀了Joker。目标材料是605现场未交物证、许清和接待记录、暗格账本夹页、地下室档案室、607/608最终卷宗、POKER组织成员名册、Joker人口贩卖名单、林渝植亲子鉴定、刀具血迹或手机云端记录；你持有哪一项就直接交出编号和内容。', post606_ids)
                                        post606_ids, post606_set = refresh_post601()
                                        if post606_set & {'605', '607', '608'}:
                                            break
                        late_ids = [
