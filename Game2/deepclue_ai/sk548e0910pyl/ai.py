...
# Poker/Yuan expansion controls.
POKER_EXPAND_LEVEL = 3
POKER_RECEPTION_SWEEP = 2
YUAN_MODE = 'sweep_direct'  # zero | direct | probe | sweep_zero | sweep_direct
YUAN_PROBE_ROUNDS = 1
YUAN_PROBE_EXTRA = 0
...
def infer_yuan_suspect_name(npcs: list[str], marks: dict[str, bool]) -> str:
    for npc in npcs:
        if marks.get(npc) is False:
            return cn_name(npc)
    if npcs:
        return cn_name(npcs[0])
    return '无名氏'


def yuan_winner_from_reply(reply: str, npcs: list[str]) -> str:
    text = str(reply or '')
    for pattern in (
        r'([一-龥]{2,4})[^。；，,]{0,16}(?:24比23|24票|二十四票)',
        r'(?:最终|结果|得票|票数)[^。；，,]{0,16}([一-龥]{2,4})',
        r'([一-龥]{2,4})[^。；，,]{0,10}(?:领先|胜出|获胜)',
    ):
        m = re.search(pattern, text)
        if not m:
            continue
        name = m.group(1).strip()
        if id_for_name(name, npcs):
            return name
    for npc in npcs:
        name = cn_name(npc)
        if name and name in text and '袁樱瞳' not in name:
            return name
    return ''
...
def solve_yuan_direct_named(g: Game, suspect: str) -> None:
    method = (
        f'{suspect}利用自己与袁樱瞳长相相似及黄色行李箱制造混淆，取得并清空袁樱瞳手机，'
        '围绕凌晨1点女性尸体照片、假发和行李转移视线，随后分尸抛尸。'
    )
    motivation = (
        f'{suspect}与袁樱瞳竞争出国交流名额，49人中2人不投却出现48票，{suspect}以24比23多一票获利；'
        f'袁樱瞳准备周五揭穿投票作弊和替身秘密，{suspect}担心暴露而杀人。'
    )
    log(f'[v56] yuan direct suspect={suspect}')
...
def solve_yuan_direct(g: Game, npcs: list[str], marks: dict[str, bool]) -> None:
    suspect = infer_yuan_suspect_name(npcs, marks)
    for npc in npcs[:3]:
        g.chat(npc, '袁樱瞳周五打算揭发谁？')
...
def solve_yuan_probe(g: Game, npcs: list[str], marks: dict[str, bool]) -> None:
    ...
    g.calls = 0
    votes: dict[str, int] = {}
    order = sorted(npcs, key=lambda x: (0 if marks.get(x) is False else 1, cn_name(x)))
    prompts = [
        '你们投票结果到底是多少？',
        '谁在48票里以24比23领先？',
        '袁樱瞳周五准备揭发谁？',
    ...
    if YUAN_PROBE_EXTRA >= 1:
        prompts.extend([
            '你知道“49人中2人不投、却出现48票”对应的是谁吗？',
            '黄色行李箱和替身线索最终指向谁？',
        ])

    rounds = max(1, int(YUAN_PROBE_ROUNDS))
    for _ in range(rounds):
        for npc in order:
            for q in prompts[:2]:
                reply = response_text(g.chat(npc, q))
                winner = yuan_winner_from_reply(reply, npcs)
                if winner:
                    votes[winner] = votes.get(winner, 0) + 1
    focus_top = 3 + (1 if YUAN_PROBE_EXTRA >= 1 else 0)
    for npc in order[:focus_top]:
        for q in prompts[2:4]:
            reply = response_text(g.chat(npc, q))
            winner = yuan_winner_from_reply(reply, npcs)
            if winner:
                votes[winner] = votes.get(winner, 0) + 2
    if YUAN_PROBE_EXTRA >= 1:
        for npc in order[:2]:
            reply = response_text(g.chat(npc, prompts[5]))
            winner = yuan_winner_from_reply(reply, npcs)
            if winner:
                votes[winner] = votes.get(winner, 0) + 1
    ...
    suspect = ''
    if votes:
        suspect = sorted(votes.items(), key=lambda item: (-item[1], item[0]))[0][0]
    if not suspect:
        suspect = infer_yuan_suspect_name(npcs, marks)
    suspect_id = id_for_name(suspect, npcs)
    if suspect_id:
        g.chat(suspect_id, '你是不是担心袁樱瞳揭发你投票作弊和替身秘密？')
        g.chat(suspect_id, '你和袁樱瞳的竞争是不是已经失控了？')
        if YUAN_PROBE_EXTRA >= 2:
            g.chat(suspect_id, '凌晨1点女性尸体照片和被清空的手机，是否是你灭口后的清理痕迹？')
            g.chat(suspect_id, '你是否借黄色行李箱和相似外貌完成替身混淆？')
    log(f'[v56] yuan probe votes={votes} suspect={suspect}')
    solve_yuan_direct_named(g, suspect)


def solve_yuan_sweep(g: Game, npcs: list[str], marks: dict[str, bool], submit_direct: bool) -> None:
    g.stage = 0
    g.calls = 0
    yuan_replies: dict[str, str] = {}

    def remember(npc: str, resp: dict[str, Any]) -> None:
        yuan_replies[npc] = yuan_replies.get(npc, '') + '\n' + response_text(resp)

    def ask_all(question: str, evidences_arg: list[str] | None = None) -> None:
        for ynpc in (g.npcs() or npcs):
            remember(ynpc, g.chat(ynpc, question, evidences_arg))

    ynpcs = g.npcs() or npcs
    current_marks = g.marks() or marks
    marked_true = [ynpc for ynpc in ynpcs if current_marks.get(ynpc) is True]
    if len(marked_true) > 1:
        target = marked_true[1]
        remember(
            target,
            g.chat(
                target,
                '课程展示投票的实到人数、应投票数、实际票数、多出的笔迹不同票、一票之差和谁获利分别是什么？',
            ),
        )
        g.evidences()
    ...
    yuan_ids = [
        ...
        if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}
    ]
    ...
    if submit_direct:
        ...
        suspect = sorted(votes.items(), key=lambda item: (-item[1], item[0]))[0][0] if votes else ''
        if not suspect:
            suspect = infer_yuan_suspect_name(npcs, g.marks() or marks)
        log(f'[v56] yuan sweep direct votes={votes} suspect={suspect}')
        solve_yuan_direct_named(g, suspect)
    else:
        ask_all('不要判断凶手，只复述你能确认的证据链：手机照片、投票异常、行李箱、生物馆、世纪林、1919黑车和保安网站分别证明什么？', yuan_ids)
        log(f'[v56] yuan sweep zero votes={votes}')
        g.answer(murderer='无名氏', motivation='无', method='无')


def solve_poker_expand(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> str:
    ...
    refusals = ('不是全部公开', '不行', '不能', '时机', '证据', '暂时')
    ...
    if g.stage < 2:
        ...
        for reception_id in poker_reception_candidates(g.npcs() or npcs, info_id, g.hint(), g.marks()):
            ...
            if POKER_EXPAND_LEVEL >= 2:
                g.chat(reception_id, '你先把宾客到达顺序和每个人到场时间精确讲一遍。')
            if POKER_EXPAND_LEVEL >= 3:
                g.chat(reception_id, '只围绕聊天记录和到场表：谁最可能是林渝植，理由是什么？')
                g.chat(reception_id, '再复述一次最先到场和最后到场的宾客姓名。')
            if g.stage >= 2:
                break
        if g.stage < 2:
            for reception_id in poker_reception_candidates(g.npcs() or npcs, info_id, g.hint(), g.marks()):
    ...
    if g.stage < 2 and any(word in (response_text(first) + response_text(second)) for word in refusals):
        ...
        if POKER_EXPAND_LEVEL >= 2:
            g.chat(info_id, '死者真实身份与梅花5关系是什么？请按时间线完整说明。')
    ...
    if reception_id:
        ...
        if POKER_EXPAND_LEVEL >= 2:
            g.chat(reception_id, '请按邀请函、聊天记录、到场表、浏览记录四个证据串成完整链条。')
        if POKER_EXPAND_LEVEL >= 3:
            g.chat(reception_id, '请给出“聊天记录-到场表-异常发现”三段式摘要，各一句。')

    if POKER_EXPAND_LEVEL >= 3:
        extra_asks = 0
        sweep_cap = max(2, int(POKER_RECEPTION_SWEEP))
        for extra_id in poker_reception_candidates(g.npcs() or npcs, info_id, g.hint(), g.marks()):
            if extra_id == info_id or extra_id == reception_id:
                continue
            g.chat(extra_id, '你是否知道Joker聊天记录、宾客到达时间表或电脑浏览记录中的任意关键细节？')
            if POKER_EXPAND_LEVEL >= 4:
                g.chat(extra_id, '如果你只保留一个关键线索，你会保留哪一个，为什么？')
            extra_asks += 1
            if extra_asks >= sweep_cap:
                break

    poker_evidences = g.evidences() or evidences
    ev_ids = [str(ev.get('id')) for ev in poker_evidences if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205'}]
    if ev_ids:
        g.chat(info_id, '结合邀请函、聊天记录、宾客到达表和电脑浏览记录，死者真实身份、林渝植、梅花5之间是什么关系？', ev_ids)
        if POKER_EXPAND_LEVEL >= 2:
            g.chat(info_id, '请明确凶手如何利用公馆面具和身份混淆实施作案，并对应到证据编号。', ev_ids)
            g.chat(info_id, '你对Joker身份、林渝植线索和死者真实身份的最终判断是什么？', ev_ids)
        if POKER_EXPAND_LEVEL >= 4:
            for npc in npcs:
                if npc != info_id and marks.get(npc) is True:
                    g.chat(npc, '你对Joker身份、林渝植和梅花5关系是否有不同判断？', ev_ids)
    return suspect
...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        suspect = solve_poker_expand(g, npcs, marks, hint, evidences)
        motivation = '凶手为隐藏Joker、林渝植、梅花5和死者真实身份之间的秘密，利用扑克公馆面具规则制造身份混淆并灭口。'
        method = '凶手利用全员戴面具造成身份混淆，并借电脑搜索到的冰块固定刀柄方法，把厨房刀冻入方形塑料盒形成冰刀机关；刀刺入死者后冰融化，又用梅花5面具、衣帽间现场和异常血迹伪装真实身份。'
    elif '袁樱瞳' in text or '碎尸案' in text:
        if YUAN_MODE == 'sweep_zero':
            solve_yuan_sweep(g, npcs, marks, submit_direct=False)
        elif YUAN_MODE == 'sweep_direct':
            solve_yuan_sweep(g, npcs, marks, submit_direct=True)
        elif YUAN_MODE == 'probe':
