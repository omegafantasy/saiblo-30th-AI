...
def poker_final_promise_hit(text: str) -> bool:
    return bool(re.search(
        r'照片原件在我这里|605[^。\n]{0,28}(?:在我手上|掌握|交出|你已经有)|'
        r'607[^。\n]{0,20}608|最终卷宗|还不是公开的时候|时机一到',
        text or '',
    ))
...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
                    ...
                    if {'401', '402'}.issubset(set(monitor_ids)):
                        ...
                        if post601_set & {'601', '602', '603', '604'}:
                            ...
                            def ask_post601(npc_id: str, question: str) -> None:
                                ...
                                chat_visible_or_probe(g, npc_id, follow_npcs, question, post601_ids)
                            ...
                            if '606' in post601_set:
                                ...
                                post606_questions = [
                                    (
                                        info_id,
                                        f'606三人照片和左臂POKER纹身已经证明组织成员，505也证明花纹村人口/器官贩卖。按你刚说的条件，我现在证明杀Joker的人是{killer_name}/真正梅花5：402说明7:30她随Joker提前入馆又在8:20离开，8:50到达并死在衣帽间的是Joker伪装者；401说明真正梅花5中午仍活着，503/504/502说明Joker威胁并控制林渝植。请不要再说时机未到，直接交出605/607/608、POKER组织名册、Joker名单、亲子鉴定或警方最终卷宗。'
                                    ),
                                    (
                                        true_club5_id,
                                        f'你就是{killer_name}/真正梅花5。606已经证明Joker、于书华/红桃Q等POKER成员身份。现在只回答：是不是你杀了Joker，杀害地点、刀具、移尸和通风口/手机云端证据分别是什么；如果证据在警方或现场，请交出605/607/608。'
                                    ),
                                    (
                                        id_for_name_any('张子韩', follow_npcs),
                                    ...
                                    (
                                        reception_id,
                                        '你负责接待和现场材料。606之后不再问身份猜测，只查现场未交证据：刀具/血迹/通风口暗格/手机云端/邀请函源文件/组织名册/警方卷宗里哪一份能证明谁杀Joker？请交出605、607或608。'
                                    ),
                                ]

                                asked_post606: set[str] = set()

                                def handle_post606_promise(npc_id: str, reply_text: str) -> None:
                                    nonlocal post606_ids, post606_set
                                    if not npc_id or not poker_final_promise_hit(reply_text):
                                        return
                                    if post606_set & {'605', '607', '608'}:
                                        return
                                    follow_resp = chat_visible_or_probe(
                                        g,
                                        npc_id,
                                        follow_npcs,
                                        '你刚才已经明确说照片原件/Joker手机云端/组织名册或605在你手上，607/608是警方最终卷宗。现在先不索要内部最终卷宗，只把能公开登记的605或照片原件登记交出来：证据名、内容、保管链、谁把Joker引入衣帽间、谁杀了Joker，以及公开条件分别是什么？',
                                        post606_ids,
                                    )
                                    post606_ids, post606_set = refresh_post601()
                                    ...
                                    if post606_set & {'605', '607', '608'}:
                                        return
                                    if re.search(r'警局授权|刑警|证件|个人调查|公开|不能公开|时机|最终卷宗|现在不能|打草惊蛇', follow_text or ''):
                                        chat_visible_or_probe(
                                            g,
                                            npc_id,
                                            follow_npcs,
                                            '你提出的限制我接受：不要交内部最终卷宗。只提交可公开的中间证据，任选其一即可：605现场未交扣押清单、照片原件登记号、Joker手机云端提取记录、林渝植本人确认笔录、衣帽间约见/门禁记录或你的个人调查笔记摘录。请直接按证据编号和内容登记。',
                                            post606_ids,
                                        )
                                        post606_ids, post606_set = refresh_post601()
                                ...
                                for npc_id, question in post606_questions:
                                    ...
                                    post606_ids, post606_set = refresh_post601()
                                    handle_post606_promise(npc_id, response_text(resp))
                                    if post606_set & {'605', '607', '608'}:
                                        break
                                if not (post606_set & {'605', '607', '608'}):
                                    for npc_id in post606_targets[:8]:
                                        if npc_id in asked_post606:
                                            continue
                                        asked_post606.add(npc_id)
                                        resp = chat_visible_or_probe(g, npc_id, follow_npcs, '606已出现，现在只推进下一门槛：请用可核验证据证明谁杀了Joker。目标证据是605现场未交物证、607/608最终卷宗、POKER组织成员名册、Joker人口贩卖名单、林渝植亲子鉴定、刀具血迹或手机云端记录；你持有哪一项就直接交出编号和内容。', post606_ids)
                                        post606_ids, post606_set = refresh_post601()
                                        handle_post606_promise(npc_id, response_text(resp))
                                        if post606_set & {'605', '607', '608'}:
                                            break
                        ...
                        if isinstance(ev405_open, dict) or isinstance(ev502_open, dict):
                            ...
                            if deep_id_set & {'503', '504', '505', '601', '602', '603', '604'}:
                                ...
                                def handle_deep_promise(npc_id: str, reply_text: str) -> None:
                                    nonlocal deep_ids, deep_id_set
                                    if not npc_id or not poker_final_promise_hit(reply_text):
                                        return
                                    if deep_id_set & {'605', '607', '608'}:
                                        return
                                    resp2 = chat_visible_or_probe(
                                        g,
                                        npc_id,
                                        follow_npcs,
                                        '你刚才已承认照片原件、Joker手机云端、605或警方卷宗在你手里。现在不要求公开607/608，只登记当前可提交的605或中间材料：现场扣押清单、照片原件登记、手机云端提取记录、林渝植确认笔录、衣帽间约见/门禁记录。请直接给证据编号、名称和内容。',
                                        deep_ids,
                                    )
                                    _, deep_ids, deep_id_set = refresh_deep_ids()
                                    if deep_id_set & {'605', '607', '608'}:
                                        return
                                    if re.search(r'不能|时机|公开|授权|刑警|卷宗|打草惊蛇', response_text(resp2) or ''):
                                        chat_visible_or_probe(
                                            g,
                                            npc_id,
                                            follow_npcs,
                                            '既然最终卷宗不能公开，就只给不影响侦查的可公开摘录：605现场未交物证、照片原件的登记号、Joker手机云端提取摘要、组织名册中与本案相关的一页，或你个人调查笔记。不要解释保密原因，直接登记可交版本。',
                                            deep_ids,
                                        )
                                        _, deep_ids, deep_id_set = refresh_deep_ids()
                                ...
                                for npc_id, question in deep_questions:
                                    ...
                                    resp = chat_visible_or_probe(g, npc_id, follow_npcs, question, deep_ids)
                                    _, deep_ids, deep_id_set = refresh_deep_ids()
                                    handle_deep_promise(npc_id, response_text(resp))
                                    if npc_id == info_id and not (deep_id_set & {'505', '601', '602', '603', '604', '605', '606', '607', '608'}):
                                        resp = chat_visible_or_probe(g, info_id, follow_npcs, '你刚提到通风口暗格旧账本、花纹村交易明细、2010失踪少女合影和张子韩名字。不要等最后拼图，直接把旧账本、泛黄合影或花纹村村长逮捕令交出来；如果对应505/601/602，也请直接给内容。', deep_ids)
                                        _, deep_ids, deep_id_set = refresh_deep_ids()
                                        handle_deep_promise(info_id, response_text(resp))
                                    if '505' in deep_id_set:
                                        globals()['POKER_HAS_505'] = True
                                ...
                                if not (deep_id_set & {'505', '605', '606', '607', '608'}):
                                    for npc_id in deep_targets[:6]:
                                        if npc_id in asked_deep:
                                            continue
                                        asked_deep.add(npc_id)
                                        resp = chat_visible_or_probe(g, npc_id, follow_npcs, '现在不要复述车和转账，只追Poker后段实物：旧账本、2010失踪少女合影、505花纹村村长逮捕令、死者手机云端名单、LYZ物品来源、林渝植失踪档案、花纹村成员、张子韩女儿DNA、Joker和沈知遥/张子韩的照片。你接触过哪一项？', deep_ids)
                                        _, deep_ids, deep_id_set = refresh_deep_ids()
                                        handle_deep_promise(npc_id, response_text(resp))
                                        if '505' in deep_id_set:
                                            globals()['POKER_HAS_505'] = True
                                ...
                                if '505' in deep_id_set and not ({'606', '607', '608'} & deep_id_set):
                                    ...
                                    for npc_id in deep_targets[:4]:
                                        resp = chat_visible_or_probe(g, npc_id, follow_npcs, '505逮捕令已经坐实花纹村人口和器官贩卖。继续查案卷下一页：村长供出的成员名册、Joker周克、沈知遥/张子韩左臂POKER纹身三人照片、林渝植亲子鉴定或最终结案证据。', deep_ids)
                                        _, deep_ids, deep_id_set = refresh_deep_ids()
                                        handle_deep_promise(npc_id, response_text(resp))
                                ...
                                if '606' in deep_id_set and not ({'605', '607', '608'} & deep_id_set):
                                    for npc_id in deep_targets[:6]:
                                        resp = chat_visible_or_probe(g, npc_id, follow_npcs, '606三人POKER纹身照片已经出现。不要继续泛谈组织，按NPC刚给出的条件证明谁杀Joker：真正梅花5/林渝植的7:30-8:20时间线、Joker伪装梅花5、503/504/502威胁控制链、刀具血迹和移尸路线。若证明成立，请交出605、607/608、组织名册或警方结案卷宗。', deep_ids)
                                        _, deep_ids, deep_id_set = refresh_deep_ids()
                                        handle_deep_promise(npc_id, response_text(resp))
                                        if deep_id_set & {'605', '607', '608'}:
                                            break
        ...
        if '708' in set(yuan_ids):
            ...
            def add_post708_target(npc_id: str) -> None:
                if npc_id and npc_id not in post708_targets:
                    post708_targets.append(npc_id)

            ev708 = next((ev for ev in yuan_evidences if str(ev.get('id')) == '708'), None)
            ev708_text = str(ev708.get('name', '')) + '\n' + str(ev708.get('content', '')) if isinstance(ev708, dict) else ''
            post708_context = ev708_text + '\n' + '\n'.join(yuan_replies.values())
            contact_hint = str(locals().get('contact_name', '') or '')
            exchange_hint = str(locals().get('exchange_name_hint', '') or '')
            visible_yuan = g.npcs() or npcs

            named_priority = ['王泽', exchange_hint, contact_hint, '江沐青', '李海天', '沈知遥', '许清和', '张子韩', '叶青衡', '陆亦初']
            for name in named_priority:
                for npc_id in global_name_ids(name, visible_yuan):
                    add_post708_target(npc_id)
            for npc_id in (forensic_target_id, teacher_id, absent_vote_id):
                add_post708_target(npc_id)
            for npc_id in story_target_ids(post708_context, visible_yuan, max_ids=12):
                add_post708_target(npc_id)
            for npc_id in visible_yuan:
                add_post708_target(npc_id)

            post708_questions: list[tuple[str, str]] = []
            for wang_id in global_name_ids('王泽', visible_yuan):
                post708_questions.append((
                    wang_id,
                    '王泽，708是在你坦白后恢复的记忆。现在不要否认，只补完整委托链：雇主邮件账号、收件邮箱、登录IP、付款记录、拍照回传记录、白色浴缸地点、刀和手套处理人分别是谁；如果对应物证09/709或下一段记忆，请直接交出。',
                ))
            if exchange_hint:
                for exchange_id in global_name_ids(exchange_hint, visible_yuan):
                    post708_questions.append((
                        exchange_id,
                        f'{exchange_hint}，联系方式交换已经兑现出708。你掌握的杀手秘密不是传闻，而是王泽坦白和雇主邮件。请继续说你从哪里知道白色浴缸、手套刀具、死亡照片回传邮箱、付款/IP记录；下一份物证09/709由谁保管？',
                    ))
            for jiang_id in global_name_ids('江沐青', visible_yuan):
                post708_questions.append((
                    jiang_id,
                    '江沐青，你是联系方式交换和生物馆线的关键人。708已出现，继续说明你为什么要运动少女联系方式、王泽坦白了什么、学生会/李海天/生物馆/保卫处里哪份记录能查到委托邮件、白色浴缸、刀手套和死亡照片回传。',
                ))
            for npc_id in post708_targets:
                post708_questions.append((
                    npc_id,
                    '708之后只查委托链源头：雇主邮件账号/IP、付款流水、死亡照片回传记录、白色浴缸所在房间、手套刀具去向、保卫处网页截图或警方技侦报告。你若不是持有人，只说持有人中文名和地点；若是物证09/709/710请直接登记内容。',
                ))

            asked_post708: set[str] = set()
            for source_id, question in post708_questions[:14]:
                if not source_id or source_id in asked_post708:
                    continue
                asked_post708.add(source_id)
                resp = chat_visible_or_probe(g, source_id, visible_yuan, question, yuan_ids)
                yuan_replies[source_id] = yuan_replies.get(source_id, '') + '\n' + response_text(resp)
                yuan_evidences = g.evidences()
                yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708', '709', '710'}]
                if {'709', '710'} & set(yuan_ids):
                    break
