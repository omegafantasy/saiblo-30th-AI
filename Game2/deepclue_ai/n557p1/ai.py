        ...
        if info_id:
            ...
            if g.stage >= 2:
                ...
                if 'reception_id' in locals() and reception_id:
                    priority.append(reception_id)
                if info_id:
                    priority.append(info_id)
                priority.extend(npc for npc in deep_npcs if npc not in set(priority))
                for npc in priority:
                    g.chat(npc, '围绕Joker聊天、到达时间表、电脑浏览记录、方形塑料盒、厨房缺刀、衣帽间和医生判断，完整复盘梅花5面具下的人是谁以及死亡方式。', deep_ids)
                    g.evidences()
                for npc in priority:
                    g.chat(npc, '如果表面他杀结论不对，请直接说明林渝植是否自杀或被替身，谁伪造现场，哪些物证能推翻普通证词？', deep_ids)
                    if g.stage >= 4:
                        break
