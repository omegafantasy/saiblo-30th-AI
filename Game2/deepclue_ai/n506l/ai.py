...
def solve_unknown(g: Game, npcs: list[str], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        motivation = (
            f'{suspect}发现死者身份与林渝植、梅花5面具和邀请函秘密有关，'
            f'担心自己在扑克公馆中的真实身份、证词破绽或相关旧事暴露而杀人灭口。'
        )
        method = (
            f'{suspect}利用扑克公馆全员戴面具造成身份混淆，在衣帽间用厨房丢失的三把刀刺杀戴梅花5面具的死者，'
            '粘住面具、灼烧双臂、稀释血迹，并利用门窗和房间未上锁状态伪造现场。'
        )
