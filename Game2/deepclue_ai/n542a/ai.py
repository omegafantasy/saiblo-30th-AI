    ...
    if '扑克公馆' in text:
        ...
        if info_id:
            ...
            if g.stage < 2 and any(word in (response_text(first) + response_text(second)) for word in refusals):
                for question in (
                    '你刚才说手机有聊天记录、随身物品可疑，请把手机聊天记录、随身物品、邀请函和案发现场证据都交给我。',
                    '请把死者手机、聊天记录、随身物品、邀请函和案发现场照片作为证据给我。',
                ):
                    g.chat(info_id, question, ['001', '002'])
                    if g.stage >= 2:
                        break
                for reception_id in poker_reception_candidates(g.npcs() or npcs, info_id, g.hint(), g.marks()):
                    g.chat(reception_id, '案发现场、聊天记录、宾客到达时间表和公馆异常发现分别是什么？')
                    if g.stage >= 2:
                        break
                if g.stage < 2:
