...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
                ...
                if 'poker_after_ids' in locals():
                    ...
                    if {'601', '602', '603', '604', '606'} & set(n603_ids):
                        for npc_id in n603_targets[:18]:
                            n603_ask(npc_id, '现在只查红桃Q/刘瑄/张朔这条线：她为什么负责接待和联络，密码从哪里来，她怎样把Joker引入衣帽间，她与周克、于书华、POKER纹身组织和林渝植失踪案是什么关系。')
                        n603_ids = [
                        ...
                        for npc_id in n603_targets[:18]:
                            n603_ask(npc_id, '如果张朔真名刘瑄并负责联络，请直接交出她的组织档案、密码记录、聊天原件、衣帽间约见记录、Joker周克入场记录和能够证明谁杀Joker的那份现场证据。')
        ...
        for ynpc in ordered:
            ask_visible(ynpc, '跨案官方链排查：Joker人口贩卖、林渝植失踪、匿名转账、1919黑车、李海天、袁樱瞳、保安网页、生物馆、蓝色背包和手机照片是否同案；下一证据在哪。')
        ...
        for ynpc in ordered:
            ask_visible(ynpc, '按系统而不是口供追：警方旧案卷宗、人口贩卖名单、银行流水、车辆登记、保卫处监控、网站后台、手机云端、DNA/尸检原件分别谁能调取。', yuan_ids)
    ...
    final_mode = str(globals().get('N603_ANSWER_MODE', ''))
