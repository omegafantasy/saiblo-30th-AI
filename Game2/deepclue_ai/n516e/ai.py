    ...
    if '扑克公馆' in text:
        ...
        for npc in npcs:
            if current_marks.get(npc) is False:
                suspect = cn_name(npc)
                break
        marked_true = [npc for npc in npcs if current_marks.get(npc) is True]
        probe_order = marked_true[1:2] or marked_true or npcs
        for npc in probe_order:
            resp = g.chat(npc, '你在课程展示和投票中负责什么？投票结果是否有异常？')
