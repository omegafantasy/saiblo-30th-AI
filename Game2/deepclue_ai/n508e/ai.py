...
def solve_unknown(g: Game, npcs: list[str], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        g.calls = 0
        for npc in npcs:
            resp = g.chat(npc, '你对死者袁樱瞳和这起碎尸案了解多少？请说最关键的线索。')
            replies[npc] = str(resp.get('reply') or resp.get('content') or resp.get('npc_reply') or '')
        peer = role_from_replies(replies, ['长得', '黄色行李箱', '等到周五', '捡到'])
        teacher = role_from_replies(replies, ['学生', '出国交流', '项目名额'])
        witness = role_from_replies(replies, ['1919', '黑色'])
        if peer:
            g.chat(peer, '你捡到的手机里有什么？袁樱瞳为什么说等到周五？')
        if teacher:
            g.chat(teacher, '课程展示投票结果为什么多出一票？出国交流名额是谁获利？')
        if witness:
            g.chat(witness, '你看到的1919黑色豪车、保安奇怪网站和生物馆线索分别说明什么？')
        if peer:
            g.chat(peer, '袁樱瞳手机里的凌晨1点女性尸体照片、栗色假发和黄色行李箱是怎么回事？')
