...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        admin_first_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
        ask_all('先不要走联系方式交换线。只沿704投票异常查教务/系办/失物招领：原始票、签到表、课堂监控、教师办公室监控、推荐名额后台、失物招领处李海天随身U盘、电子系保研名单和不雅视频照片是谁拿到？若能给物证06/706请直接交出。', admin_first_ids)
        ask_all('继续只查706前置：今天谁去过失物招领处，谁见过李海天随身U盘，U盘中的电子系保研名单为何显示袁樱瞳和某人成功、王科瑾未保研，谁因此清空袁樱瞳手机？请给证据编号、证据名和持有人。', admin_first_ids)
