...
def solve_rose(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    g.answer(
        murderer=cui_name,
        motivation=f'{cui_name}误认为{deng_name}对Rose有意，为扫清情敌、独占{deng_name}而谋划杀害Rose。',
...
def solve_unknown(g: Game, npcs: list[str], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        g.calls = 0
        info = ''
        m = re.search(r'，([^，。]+)是个好的信息来源', hint)
        if m:
            info = id_for_name(m.group(1).strip(), npcs)
        if not info:
            info = next((npc for npc in npcs if g.marks().get(npc) is True), npcs[0] if npcs else '')
        if info:
            for question in [
                ...
                g.chat(info, question)

        next_hint = g.hint()
        next_npcs = g.npcs()
        reception = ''
        m2 = re.search(r'接待者([^，,。 ]+)', next_hint)
        if m2:
            reception = id_for_name(m2.group(1).strip(), next_npcs)
        if not reception:
            next_marks = g.marks()
            reception = next((npc for npc in next_npcs if next_marks.get(npc) is False), info)
        if reception:
            suspect = cn_name(reception)
            for question in [
                '聊天记录里死者和林渝植、梅花5有什么关系？',
                '宾客到达时间表是什么？每个人分别几点到达？',
                '公馆内有什么异常发现？厨房刀具、门窗、血迹和面具哪里不对？',
                '你的证词有什么破绽？案发时你到底在哪里？',
                '你为什么说自己在餐厅，监控和时间表有什么矛盾？',
            ]:
                g.chat(reception, question)
        motivation = f'{suspect}为掩盖自己掌握的聊天记录、到达时间表和林渝植身份秘密，担心梅花5真实身份及自己证词破绽暴露而杀人灭口。'
        method = f'{suspect}利用接待者身份熟悉扑克公馆结构和宾客到达时间，提前盗走厨房三把刀，在衣帽间刺杀戴梅花5面具的死者，并灼烧双臂、稀释血迹、处理面具与门窗来制造身份混淆。'
