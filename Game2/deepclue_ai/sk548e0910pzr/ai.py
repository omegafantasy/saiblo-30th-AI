...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
                ...
                if reception_id and g.stage >= 3:
                    ...
                    if {'401', '402'}.issubset(set(monitor_ids)):
                        ...
                        if not isinstance(ev404, dict) and not isinstance(ev501, dict):
                            g.chat(info_id, '如果我的时间线和密码0512已经满足，请不要泛泛说“下一阶段”：直接给404车牌、501匿名转账，或隐藏物证405/502的证据编号、证据名和保管人；如果还不能给，请说明缺哪个持有人、地点或官方卷宗。', branch_ids)
                            poker_evidences = g.evidences()
                            ev404 = next((ev for ev in poker_evidences if str(ev.get('id')) == '404'), None)
                            ev501 = next((ev for ev in poker_evidences if str(ev.get('id')) == '501'), None)
                            branch_ids = [
        ...
        if globals().get('POKER_HAS_501'):
            ...
            ask_all('上一案501显示Joker人口贩卖集团、于书华看诊和匿名50万元转账。袁樱瞳、李海天、1919黑车、生物馆、保安奇怪网站是否与同一集团或同一条尸体/人口转移链有关？只说官方证据编号、证据名和持有人。', cross_ids)
        ...
        for target_id in screenshot_targets[:2]:
            resp = g.chat(target_id, '开场回忆说我失忆但被保安认出，口袋里还有一张模糊网页截图；你又反复被目击在看奇怪网站并周日离岗。这个网站是不是尸块/人口/车辆交易入口？请直接交出物证06/706，或网页截图、登录账号、访问记录、1919车辆、世纪林报案记录的证据编号和持有人。', yuan_ids)
        ...
        if not isinstance(ev705, dict):
            ...
            for target_id in early_targets[:3]:
                resp = g.chat(target_id, '不要猜凶手，先把能打开下一阶段的官方物证交出来：物证06/706、李海天尸检报告、蓝色背包海豚挂件、世纪林尸块DNA、1919黑车记录或生物馆监控。你本人接触或知道哪一项？如果是707/708也直接给编号。', yuan_ids)
                yuan_replies[target_id] = yuan_replies.get(target_id, '') + '\n' + response_text(resp)
                yuan_evidences = g.evidences()
                yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
                ev705 = next((ev for ev in yuan_evidences if str(ev.get('id')) == '705'), None)
        ...
        if isinstance(ev705, dict):
            ...
            if target_id:
                resp = g.chat(target_id, '705李海天尸检报告显示背刺失血、四肢被砍断、蓝色背包海豚挂件，并且和袁樱瞳案相似又有差异。请直接说明蓝色背包和海豚挂件是谁的、李海天案与袁樱瞳凌晨照片/世纪林尸块/生物馆跑出者怎么连接，谁持有物证06/706或下一份官方证据。', yuan_ids)
                yuan_replies[target_id] = yuan_replies.get(target_id, '') + '\n' + response_text(resp)
                yuan_evidences = g.evidences()
                yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
            runner_name = ''
            ...
            if runner_id and runner_id != target_id and '706' not in set(yuan_ids):
                resp = g.chat(runner_id, '705尸检报告和目击证言已经对上：你被看见从生物馆慌张跑出，李海天尸体旁有蓝色背包海豚挂件，袁樱瞳案又出现凌晨尸体照片和世纪林尸块。请直接说明你当晚在生物馆做了什么、背包挂件是谁的、物证06/706或下一份官方证据在哪里。', yuan_ids)
            ...
            if guard_id and guard_id not in {target_id, runner_id} and '706' not in set(yuan_ids):
                resp = g.chat(guard_id, '705尸检报告指向李海天旧案，线索又反复指到保安室：你看的奇怪网站、我口袋里的模糊网页截图、周日离岗、世纪林尸块、1919黑车和生物馆监控都需要保卫处记录。请直接交出物证06/706、网站截图/登录记录、巡逻日志、世纪林/生物馆监控或下一份官方证据。', yuan_ids)
                yuan_replies[guard_id] = yuan_replies.get(guard_id, '') + '\n' + response_text(resp)
                yuan_evidences = g.evidences()
                yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
            if '706' not in set(yuan_ids):
                ask_all('只围绕705李海天尸检报告继续。蓝色背包海豚挂件、背部刀伤、失血死亡、四肢分离和袁樱瞳碎尸案之间，哪一项物证或监控能打开物证06/706或下一阶段？直接说证据编号、证据名和持有人。', yuan_ids)
                yuan_evidences = g.evidences()
                yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
