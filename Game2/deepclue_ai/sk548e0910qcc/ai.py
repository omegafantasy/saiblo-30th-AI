...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        corpse_first_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
        ask_all('先不要追联系方式。只查尸源和转运系统：黄色行李箱购买/借用/送修记录、维修店登记、宿舍楼监控、箱内血迹指纹纤维、lo裙栗色假发来源、凌晨照片原图EXIF、1919黑车路线、生物馆门禁和世纪林报警记录，哪一项能打开物证06/706或下一阶段？', corpse_first_ids)
        ask_all('继续查安保系统：开场保安认识失忆侦探、口袋网页截图、保安奇怪网站、周日离岗、1919车辆登记、生物馆/世纪林监控和网页登录账号之间是什么关系？请直接给官方记录编号、证据名和持有人。', corpse_first_ids)
