#!/usr/bin/env python3
"""Game2 DeepClue AI v55.

Isolate Yuan-answer hypothesis:
- old cases identical to v49;
- poker reverts to v49 generic fallback;
- Yuan uses the first visible role with a concrete vote/phone motive.
"""
...
class Game:
    ...
    def req(self, action: str, **kwargs: Any) -> Any:
        try:
            ...
            log(f'[v55] request failed action={action}: {exc}')
            return {}
...
def solve_rose(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    if not banker:
        banker = '银行家'
    log(f'[v55] rose murderer_id={murderer_id} murderer={murderer} banker={banker}')
    g.answer(
        murderer=murderer,
...
def solve_z(g: Game, evidences: list[dict[str, Any]]) -> None:
    ...
    d = roles.get('005', 'D')
    e = roles.get('006', 'E')
    log(f'[v55] z roles A={a} B={b} C={c} D={d} E={e}')
    g.answer(
        murderer=e,
...
def solve_unknown(g: Game, npcs: list[str], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        log(f'[v55] poker generic suspect={suspect}')
        g.answer(
            murderer=suspect,
            motivation='未知',
        ...
        log(f'[v55] yuan exact suspect={suspect}')
        g.answer(
            murderer=suspect,
            motivation=f'{suspect}与袁樱瞳竞争出国交流名额，投票中多出一票使{suspect}险胜；袁樱瞳说“等到周五”，{suspect}担心作弊和相关秘密暴露而杀人。',
            method=f'{suspect}利用自己与袁樱瞳长相相似制造替身和时间线混淆，取得并清空袁樱瞳手机，留下凌晨1点身穿lo裙、戴栗色假发的女性尸体照片误导调查，随后借行李箱、生物馆和抛尸现场转移视线并分尸抛尸。',
        )
    else:
        log(f'[v55] unknown hint={hint[:50]} suspect={suspect}')
...
def solve_case(g: Game, case_idx: int) -> bool:
    ...
    text = all_text(hint, evidences)
    log(f'[v55] case={case_idx} npcs={sorted(npcs)} marks={marks} hint={hint[:60]}')
    if 'Rose' in text:
...
def main() -> int:
    ...
    for case_idx in range(6):
        try:
            ...
            log(f'[v55] fatal case={case_idx}: {exc}')
            try:
                g.answer('', '未知', '未知')
            except Exception:
                pass
