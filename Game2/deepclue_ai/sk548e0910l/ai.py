...
def solve_rose_direct(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    g.answer(
        murderer=murderer,
        motivation=f'{murderer}因情感与占有欲将Rose视为{banker}身边障碍，遂预谋投毒杀害。',
        method=f'{murderer}利用家族药材渠道取得夹竹桃毒素，于18:40左右在准备室向Rose专用蜂蜜水投毒，Rose饮下后中毒死亡。',
    )
...
def solve_rose(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    g.answer(
        murderer=cui_name,
        motivation=f'{cui_name}因情感与占有欲将Rose视为{deng_name}身边障碍，遂预谋投毒杀害。',
        method=f'{cui_name}利用家族药材渠道取得夹竹桃毒素，于18:40左右在准备室向Rose专用蜂蜜水投毒，Rose饮下后中毒死亡。',
    )
