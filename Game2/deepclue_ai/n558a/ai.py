...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        if marked:
            g.chat(marked[0], '请只说你确认的袁樱瞳手机、凌晨1点尸体照片、lo裙栗色假发、黄色行李箱、出国名额竞争和身份混淆线索。')
        if len(marked) > 1:
            g.chat(marked[1], '请只说课程展示投票：实到人数、应投票数、实际票数、多出的笔迹不同票、一票之差和谁直接获利。')
        if unmarked:
            g.chat(unmarked[0], '请只说生物馆、世纪林、1919黑车、保安奇怪网站、张壹传闻和你亲眼看到谁跑出来。')
        yuan_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}]
        if marked:
            g.chat(marked[0], '结合现有证据，手机照片、假发、行李箱和出国名额如何说明真实死者与替身？', yuan_ids)
        if len(marked) > 1:
            g.chat(marked[1], '结合现有证据，投票异常中多出的票是谁写的，谁因此获利，袁樱瞳为什么说等到周五？', yuan_ids)
        if unmarked:
            g.chat(unmarked[0], '结合现有证据，生物馆目击、世纪林尸块、1919黑车和保安网站如何连接到袁樱瞳死亡？', yuan_ids)
        for ynpc in (marked[:2] + unmarked[:1]):
            g.chat(ynpc, '如果你能确认关键隐瞒者，只给名字、动机、过程和证据链；不能确认就说明缺哪条证据。', yuan_ids)
