...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        for ynpc in marked[:2]:
            g.chat(ynpc, '结合703和704，只判断谁因手机照片、黄色行李箱、投票异常或出国名额直接获利；给出证据链。', yuan_ids)
        for ynpc in [ynpc for ynpc in ynpcs if ynpc not in set(marked[:2])][:1]:
            g.chat(ynpc, '结合703和704，只说明生物馆目击、世纪林尸块、1919黑车和保安网站如何验证或推翻张壹传闻。', yuan_ids)
