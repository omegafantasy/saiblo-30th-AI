...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        ask_all('不要先判断凶手。请确认真实死者是谁：袁樱瞳、凌晨1点照片女性、lo裙栗色假发女性、黄色行李箱尸块和世纪林尸块是否是同一人？')
        ask_all('逐一说明手机、行李箱、假发、照片、尸块发现地点、张壹生物馆传闻中哪些是身份置换，哪些是真实死亡线索。')
        yuan_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}]
        ask_all('结合现有证据，死者身份是否被替换或误认？袁樱瞳真正死亡时间、照片女性身份和尸块来源分别是什么？', yuan_ids)
        ask_all('如果有人利用替身或误认掩护凶手，请说出掩护者、被掩护者、证据链和可核验物证。', yuan_ids)
