#!/usr/bin/env python3
"""Game2 DeepClue AI n549f.

Full-score Yuan +40 probe:
- keep n548g's Rose/ZF/Poker full skeleton;
- replace Yuan zero with the n549e chat-only Yuan probe, then still submit zero.
"""
...
def solve_yuan_probe(g: Game, npcs: list[str], marks: dict[str, bool]) -> None:
    ...
    for question in (
        ...
        '世纪林垃圾桶等五处尸块、黄色行李箱、紫色行李箱、手机照片之间的转移路线是什么？谁有条件搬运尸体？',
    ):
        ask_all(question, ids)
...
def solve_yuan_probe_old(g: Game, npcs: list[str], marks: dict[str, bool]) -> None:
    suspect = cn_name(npcs[0]) if npcs else ''
    for npc in npcs:
