#!/usr/bin/env python3
"""Game2 DeepClue AI v21 — Optimal hybrid.

Combines:
- Case 0: Early exit after stage 6 (saves ~12 calls vs v18)
- Case 1: v18's full admin question sequence (no adaptive retries)

Score analysis: more calls = higher stages = higher score.
Adaptive retries HURT efficiency without helping stage advancement.
v18 scored 1928 (our best). v21 should match or exceed by saving
Case 0 calls while keeping Case 1 at full length.
"""
...
def solve_case0(sdk: SDK) -> None:
    """Case 0 (Rose): Admin sequence with early exit at stage 6."""
    stage = 0
    ...
    # Stage 0->1: Ask about Rose (4 calls)
    ...
    # Phase 3: Deeper investigation (4 calls)
    ...
    if stage < 6:
        # Continue with remaining questions if stage 6 not reached
...
def solve_case1(sdk: SDK) -> None:
    """Case 1 (campus): Exact admin question sequence, no adaptive retries."""
...
def solve_case(sdk: SDK, case_index: int) -> bool:
    ...
    log(f'[v21] case={case_index} npcs={sorted_npcs}')
    ...
    if sorted_npcs == CASE0_NPCS:
        ...
        log(f'[v21] unknown case, submitting fallback')
...
def main() -> int:
    ...
    while True:
        try:
            ...
            log(f'[v21] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
