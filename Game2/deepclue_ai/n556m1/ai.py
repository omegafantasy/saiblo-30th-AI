...
class Game:
    ...
    def req(self, action: str, **kwargs: Any) -> Any:
        try:
            ...
            log(f'[n556m1] request failed action={action}: {exc}')
            return {}
    ...
    def chat(self, npc: str, question: str, evidences: list[str] | None = None) -> dict[str, Any]:
        ...
        for attempt in range(3):
            ...
            if not (isinstance(resp, dict) and resp.get('error')):
                break
            log(f'[n556m1] chat error retry={attempt} npc={npc} q={compact(question, 36)} err={compact(resp.get("error"), 80)}')
        ...
        if new_stage > self.stage:
            log(f'[n556m1] stage {self.stage}->{new_stage} npc={npc} q={compact(question, 36)}')
    ...
    def retry_trigger(self, trigger_qs: list[tuple[Any, ...]], max_retries: int = 2) -> bool:
        ...
        for attempt in range(max_retries):
            for item in trigger_qs:
                ...
                log(f'[n556m1] retry a{attempt} npc={npc} q={compact(question, 40)}')
...
def solve_rose_direct(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    if not banker:
        banker = '银行家'
    log(f'[n556m1] rose direct murderer_id={murderer_id} murderer={murderer} banker={banker}')
    g.answer(
        murderer=murderer,
...
def solve_rose(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    if set(roles) != {'xiao', 'bai', 'deng', 'fan', 'ye', 'cui'}:
        log(f'[n556m1] rose role mapping incomplete roles={roles}')
    ...
    log(
        f'[n556m1] rose script xiao={xiao_name}/{xiao} bai={bai_name}/{bai} '
    ...
    log(f'[n556m1] rose final stage={g.stage} calls={g.calls}')
    g.answer(
        murderer=cui_name,
...
def solve_z_script(g: Game, npcs: list[str], evidences: list[dict[str, Any]]) -> None:
    ...
    if set(roles) != {'A', 'B', 'C', 'D', 'E'}:
        log(f'[n556m1] z role mapping incomplete roles={roles}')
    ...
    d_name, d = roles['D']
    e_name, e = roles['E']
    log(f'[n556m1] z script A={a_name}/{a} B={b_name}/{b} C={c_name}/{c} D={d_name}/{d} E={e_name}/{e}')
    g.stage = 0
    g.calls = 0
    ...
    log(f'[n556m1] z final stage={g.stage} calls={g.calls}')
    g.answer(
        murderer=e_name,
...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        if info_id:
            ...
            if g.stage >= 2:
                ...
                if g.stage < 3 and ev_ids:
                    ...
                    g.evidences()
                deep_npcs = g.npcs() or npcs
                deep_evidences = g.evidences()
                deep_ids = [str(ev.get('id')) for ev in deep_evidences if str(ev.get('id')) in {'001', '002', '101', '102', '103', '104', '201', '202', '203', '204', '205'}]
                for npc in deep_npcs:
                    if npc == info_id:
                        continue
                    g.chat(npc, '请直接说明扑克公馆案所有未公开事实：Joker、林渝植、梅花5、医生、接待者、死者真实身份、自杀或他杀、冰柜塑料盒、厨房三把刀、衣帽间和电脑浏览记录分别怎样串起来？', deep_ids[:6])
                    if g.stage >= 4:
                        break
                if g.stage < 4:
                    for npc in deep_npcs:
                        g.chat(npc, '如果常规说法不完整，请判断谁才是真正凶手；是否存在林渝植自杀伪装他杀、面具替身、接待者作伪证或医生隐瞒身份？给出完整推理。', deep_ids[-6:])
                        if g.stage >= 4:
                            break
        ...
        for ynpc, reply in yuan_replies.items():
            score = 0
            for key in ('黄色行李箱', '长相', '凌晨1点', '尸体照片', 'lo裙', '栗色假发', '清空手机', '投票', '出国', '名额', '获利', '生物馆', '世纪林', '尸块'):
                if key in reply:
                    score += 1
            scores[ynpc] = score
        false_marked = [ynpc for ynpc in (g.npcs() or npcs) if g.marks().get(ynpc) is False]
        suspect_id = max(scores, key=scores.get) if scores else (false_marked[0] if false_marked else (npcs[0] if npcs else ''))
        if false_marked and scores.get(suspect_id, 0) < 3:
            suspect_id = false_marked[0]
        suspect = cn_name(suspect_id)
        g.answer(
            murderer=suspect,
            motivation=f'{suspect}与袁樱瞳竞争出国名额或隐藏投票作弊、手机照片、替身和生物馆尸块线索，担心袁樱瞳揭露真相而杀人灭口。',
            method=f'{suspect}利用与袁樱瞳相关的手机、凌晨1点女性尸体照片、lo裙栗色假发和黄色行李箱制造死亡时间与身份混淆，将袁樱瞳杀害后分尸，并通过张壹生物馆传闻、世纪林尸块、1919黑车和保安网站线索转移视线。',
        )
        return
    else:
        method = '未知'
    log(f'[n556m1] unknown hint={compact(hint, 50)} suspect={suspect}')
...
def solve_case(g: Game, case_idx: int) -> bool:
    ...
    text = all_text(hint, evidences)
    log(f'[n556m1] case={case_idx} npcs={sorted(npcs)} marks={marks} hint={compact(hint, 60)}')
    if 'Rose' in text:
...
def main() -> int:
    ...
    for case_idx in range(6):
        try:
            ...
            log(f'[n556m1] fatal case={case_idx}: {exc}')
            try:
                g.answer('', '未知', '未知')
            except Exception:
                pass
