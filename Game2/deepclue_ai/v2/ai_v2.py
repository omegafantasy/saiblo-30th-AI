...
def solve_case(sdk: SDK, case_index: int) -> bool:
    ...
    log(f'[game2-v2] answer case={case_index} murderer={answer["murderer"]!r}')
    resp = sdk.request('answer', **answer)
    log(f'[game2-v2] answer_result case={case_index} resp={resp}')
    return True
...
def main() -> int:
    ...
    log(f'[game2-v2] welcome={welcome}')
    case_index = 0
    while True:
        try:
            ...
            log(f'[game2-v2] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
