...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        ask_all('袁樱瞳碎尸案只查证据保管链：手机、投票、李海天尸检、蓝色背包海豚挂件、1919黑车、生物馆监控、保安网站后台分别在哪个完整姓名或系统。')
        ask_all('不要猜凶手。请说你本人能调取或知道来源的官方物证：705尸检报告、706/707/708、保卫处、法医、警方旧案、车辆登记、网站后台、手机云端。')
        ...
        if '705' in yuan_ids:
            for npc_id in target_ids[:12]:
                ask(npc_id, '705李海天尸检已出现。你作为报告来源或被报告指向的人，只回答下一份官方材料：报告原件编号、DNA比对、蓝色背包主人、海豚挂件来源、1919车、保安网页日志、生物馆监控、警方旧案卷宗。', yuan_ids)
        else:
            for npc_id in target_ids[:10]:
                ask(npc_id, '当前还缺705/706。请只给官方来源完整名称：哪位老师、哪名保安、哪名学生、法医/警方/保卫处/车辆/网站/手机云端系统能调取。', yuan_ids)
