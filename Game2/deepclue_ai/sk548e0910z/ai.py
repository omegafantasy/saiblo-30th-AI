...
def rose_motivation_text(murderer: str, banker: str) -> str:
    return (
        f'{murderer}觊觎并想独占银行家{banker}，认定Rose对{banker}的纠缠会阻断自己的目标，'
        f'因此预谋对Rose下毒。'
    )


def rose_method_text(murderer: str) -> str:
    return (
        f'{murderer}利用家族药材渠道取得夹竹桃毒素，在18:40前后进入准备室，'
        f'将毒素投入Rose专用蜂蜜水并使其饮下，最终导致中毒死亡。'
    )
...
def solve_rose_direct(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    g.answer(
        ...
        method=rose_method_text(murderer),
    )
...
def solve_rose(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    g.answer(
        ...
        method=rose_method_text(cui_name),
    )
