...
class KnowledgeBase:
    ...
    def recent_evidence_items(self, budget: int = 4) -> list[dict[str, str]]:
        items: list[dict[str, str]] = []
        for source in (self.others, self.testimony):
            for item in source:
                if isinstance(item, dict) and item.get('id'):
                    items.append({
                        'id': str(item.get('id', '')),
...
def evidence_probe(sdk: SDK, kb: KnowledgeBase) -> None:
    current_marks = marks(sdk)
    targets = [npc for npc in kb.npcs if current_marks.get(npc)] or list(kb.npcs)
    for npc, item in zip(targets, reversed(kb.recent_evidence_items(3))):
        name = item.get('name', '').strip()
        eid = item.get('id', '').strip()
        if not name or not eid:
            continue
        question = f'关于证据“{name[:36]}”，它与你、死者或当前最可疑的人之间有什么关系？'
        if not ask_once(sdk, kb, npc, question, [eid]):
            break
    refresh(sdk, kb)
...
def final_answer(sdk: SDK, kb: KnowledgeBase) -> dict[str, str]:
    prompt = {
        ...
        'stage': kb.stage,
        'npcs': kb.npcs,
        'recent_evidence': kb.recent_evidence_items(6),
        'evidence': kb.evidence_text(limit=7200),
        'requirements': [
            '输出严格 JSON 对象，格式: {"murderer":...,"motivation":...,"method":...}',
            '优先保证 murderer 正确',
            '不要只根据表面冲突选择凶手，要综合时间线、关系和关键证据',
            'motivation 和 method 要具体且简洁',
            '若 method 不确定，优先给出最符合证据的保守描述，不要编造长故事',
            '不要输出额外说明',
        ],
    }
    resp = sdk.call_llm(
...
def solve_case(sdk: SDK, case_index: int) -> bool:
    ...
    if not kb.background and not kb.npcs:
        return False
    log(f'[game2-v8] case={case_index} stage={kb.stage} npcs={len(kb.npcs)}')
    ...
    for idx in range(2):
        ...
        if idx == 0 and kb.steps < kb.step_budget - 2:
            evidence_probe(sdk, kb)

    answer = final_answer(sdk, kb)
    log(f"[game2-v8] answer case={case_index} steps={kb.steps} murderer={answer['murderer']!r}")
    resp = sdk.request('answer', **answer)
    log(f'[game2-v8] answer_result case={case_index} resp={resp}')
    return True
...
def main() -> int:
    ...
    log(f'[game2-v8] welcome={welcome}')
    case_index = 0
    while True:
        try:
            ...
            log(f'[game2-v8] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
