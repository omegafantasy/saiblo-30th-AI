...
def yuan_candidate_from_replies(yuan_replies: dict[str, str], npcs: list[str], marks: dict[str, bool]) -> str:
    ...
    def add(name: str, weight: int) -> None:
        clean = name.strip()
        if clean and id_for_name(clean, npcs):
            votes[clean] = votes.get(clean, 0) + weight

    for npc, reply in yuan_replies.items():
        text = str(reply or '')
        npc_name = cn_name(npc)
        if re.search(r'(我[^。！？]{0,24}(?:险胜|赢了|多一票|拿到).{0,20}袁樱瞳|我们长得.{0,8}像)', text):
            if not re.search(r'(老师|教师|副教授|授课|主持|清点票)', text):
                add(npc_name, 3)
        for pattern in (
            r'获利者显然是([一-龥]{2,4})',
            r'最终([一-龥]{2,4})以\s*24\s*票?对\s*23\s*票?险胜',
            r'([一-龥]{2,4})以\s*24\s*票?对\s*23\s*票?险胜',
            r'([一-龥]{2,4})以一票之差险胜袁樱瞳',
            r'导致([一-龥]{2,4})以一票之差',
        ):
        ...
        if info_id:
            ...
            if g.stage >= 2:
                ...
                if deep_ids:
                    ...
                    g.chat(method_target, '只谈死亡方式：背部刀伤、冰块融化、方形塑料盒、厨房缺刀、无指纹刀具和稀释血水分别说明什么？', deep_ids)
