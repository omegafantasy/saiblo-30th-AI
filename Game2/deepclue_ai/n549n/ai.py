...
def solve_yuan_probe_corrected(g: Game, npcs: list[str], marks: dict[str, bool]) -> None:
    ...
    for npc in npcs:
        ...
        is_teacher = any(key in text for key in ('老师', '教授', '课程展示', '全班', '投票'))
        is_winner_like = any(key in text for key in ('捡到', '手机', '黄色行李箱', '长得', '出国名额'))
        is_route_like = any(key in text for key in ('黑色豪车', '1919', '世纪林', '学生会', '普通同学'))
        if is_route_like and not is_teacher and not is_winner_like:
            witness_targets.append(npc)
    if not witness_targets:
        witness_targets = list(npcs)
    targeted = [
