...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        if '705' in yuan_ids:
            focus_ids = [eid for eid in yuan_ids if eid in {'001', '703', '704', '705'}]
            if witness:
                remember(witness, g.chat(witness, '依据705李海天尸检报告，只比较可核验事实：背刺失血、四肢被砍、蓝色背包海豚挂件、生物馆跑出者和世纪林尸块，哪一项能推进到下一份物证？', focus_ids))
                yuan_ids = yuan_ids_now()
                focus_ids = [eid for eid in yuan_ids if eid in {'001', '703', '704', '705', '706', '707', '708'}]
            if teacher:
                remember(teacher, g.chat(teacher, '把703手机、704投票纸、705李海天尸检报告放在一起看：袁樱瞳说等到周五究竟要揭发谁，下一份原始证据在谁手里？', focus_ids))
                yuan_ids = yuan_ids_now()
                focus_ids = [eid for eid in yuan_ids if eid in {'001', '703', '704', '705', '706', '707', '708'}]
            if competitor:
                remember(competitor, g.chat(competitor, '结合703、704、705，只回答你是否利用相似外貌、手机照片、lo裙栗色假发和黄色行李箱伪造袁樱瞳死亡时间；如果不是，哪件物证能证明？', focus_ids))
                yuan_ids = yuan_ids_now()
            yuan_ids = [eid for eid in yuan_ids if eid in {'001', '703', '704', '705', '706', '707', '708'}]
            ask_all('只根据已经公开的原始证据，说明还缺哪一件具体物证才能确认袁樱瞳真实死亡时间、尸块转移路线和李海天案关联。', yuan_ids)
        else:
