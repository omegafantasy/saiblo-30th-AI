...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        ask_all('请只索取物证：袁樱瞳手机相册和删除记录、黄色行李箱来源、lo裙和栗色假发来源、血迹指纹DNA、监控和1919黑车记录分别在哪里？')
        ask_all('谁接触过手机、行李箱、假发、尸块、保安网站和黑车？请不要讲传闻，只说可验证证据。')
        yuan_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}]
        ask_all('结合现有证据，哪些物证能排除张壹传闻并确认真实处理尸体的人？', yuan_ids)
        ask_all('还有哪些未公开证据或证词能补完整案情？请按证据编号或物证名称列出。', yuan_ids)
