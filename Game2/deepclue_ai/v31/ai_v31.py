#!/usr/bin/env python3
"""Game2 DeepClue AI v31 — Probe evidence 113 (尸体勘察) in Case 0.

v26-v30 all score 2220. This version probes evidence 113:
- 尸体勘察: 死者嘴唇青紫，口内有异味，颈部有掐痕
- Available from Case 0 stage 2, but admin never presents it
- Target: CuiAnYan (murderer) and YeWenXiao with body examination findings
- Hypothetical missing testimonies in Case 0: 301 (3xx range), 502 (5xx range)
- If no new testimonies, confirms 2220 as absolute ceiling
"""
        ...
        if isinstance(resp, dict):
            ...
            if s is not None:
                try:
                    ...
                    pass
            # Log testimony unlocks for analysis
            ut = resp.get('unlock_testimony')
            if ut:
                log(f'  TESTIMONY_UNLOCK [{npc}] q={question[:30]} unlock={ut}')
        return False
...
def solve_case0(g: Game) -> None:
    """Case 0 (Rose): Mirror admin's evidence-presentation sequence + evidence 113 probe."""
    # Stage 1
    ...
    # Stage 2 — evidence 113 becomes available here
    g.chat('XiaoDingAng', '你今晚在做什么？')
    g.chat('BaiJingTing', '你今晚在做什么？')
    g.chat('CuiAnYan', '你今晚在干什么？')
    g.chat('DengDaLing', '你今晚在干什么？')

    # Probe evidence 113 (尸体勘察: 死者嘴唇青紫，口内有异味，颈部有掐痕)
    # Present to CuiAnYan (murderer) and YeWenXiao — looking for missing testimonies 301/502
    g.chat('CuiAnYan', '这份尸检报告显示死者口内有异味，你知道Rose是怎么死的吗？', evidences=['113'])
    g.chat('YeWenXiao', '尸检发现死者颈部有掐痕，你知道Rose遇害的经过吗？', evidences=['113'])
    g.chat('FanMinMin', '尸检说Rose嘴唇青紫，你觉得Rose是怎么死的？', evidences=['113'])
    ...
    # XiaoDingGang appears at stage 6 — ask directly (no get_npcs() call needed)
    ...
    log(f'[v31] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1(g: Game) -> None:
    ...
    log(f'[v31] case1 stage={g.stage} calls={g.calls}')
    result = g.submit(
        murderer='E',
    ...
    log(f'[v31] case1 result={result}')


def solve_case(g: Game, case_index: int) -> bool:
    log(f'[v31] case={case_index}')
    g.stage = 0
    g.calls = 0
    ...
    if case_index == 0:
        ...
        log(f'[v31] unexpected case_index={case_index}')
        return False
    return True
...
def main() -> int:
    ...
    while True:
        try:
            ...
            log(f'[v31] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
