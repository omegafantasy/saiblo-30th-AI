...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
                ...
                if reception_id and g.stage >= 3:
                    ...
                    if {'401', '402'}.issubset(set(monitor_ids)):
                        ...
                        if ({'404', '501'} & set(late_ids)) and not isinstance(ev405_late, dict) and not isinstance(ev502_late, dict):
                            g.chat(info_id, '现在后续不是再猜动机，而是补身份鉴定闭环：死者到底是Joker还是林渝植、真正梅花5为何活着、面具内侧皮屑/指纹/牙科记录/失踪人口档案/DNA比对哪一项能证明？请直接交出身份鉴定或尸源报告。', late_ids)
                            poker_evidences = g.evidences()
                            late_ids = [
                                str(ev.get('id'))
                                for ev in poker_evidences
                        ...
                        if police_revealed and ({'404', '501'} & set(late_ids)):
                            ...
                            if not isinstance(ev405_late, dict) and not isinstance(ev502_late, dict):
                                g.chat(info_id, '你既然是刑警队长“景观”，请不要再用嫌疑人口吻回答，直接调警局卷宗：林渝植失踪案、Joker人口贩卖案、车辆高清监控、DNA/指纹鉴定、银行流水和于书华病历里，哪一份是下一阶段证据？请交出证据名和内容。', late_ids)
        ...
        if globals().get('POKER_HAS_501'):
            cross_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}]
        ...
        for target_id in screenshot_targets[:2]:
            resp = g.chat(target_id, '开场回忆说我失忆但被保安认出，口袋里还有一张模糊网页截图；你又反复被目击在看奇怪网站并周日离岗。这个网站是不是尸块/人口/车辆交易入口？请直接交出网页截图、登录账号、访问记录、1919车辆或世纪林报案记录。', yuan_ids)
        ...
        if isinstance(ev705, dict):
            ...
            if guard_id and guard_id not in {target_id, runner_id} and '706' not in set(yuan_ids):
                resp = g.chat(guard_id, '705尸检报告指向李海天旧案，线索又反复指到保安室：你看的奇怪网站、我口袋里的模糊网页截图、周日离岗、世纪林尸块、1919黑车和生物馆监控都需要保卫处记录。请直接交出网站截图/登录记录、巡逻日志、世纪林/生物馆监控或下一份官方证据。', yuan_ids)
        ...
        if '706' not in set(yuan_ids) and ('705' in set(yuan_ids) or globals().get('POKER_HAS_501')):
            ask_all('不要再讨论投票细节，只查身份和尸源鉴定：袁樱瞳、凌晨照片里的女性、世纪林尸块和李海天是否存在替身或尸块二次利用？哪份DNA报告、尸源鉴定、手机原图元数据或生物馆监控能证明？直接说证据名和持有人。', yuan_ids)
            yuan_evidences = g.evidences()
            yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}]
