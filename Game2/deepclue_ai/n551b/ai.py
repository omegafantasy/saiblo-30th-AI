...
def solve_yuan(g: Game, npcs: list[str]) -> None:
    ...
    def ask_all(question: str, ids: list[str] | None = None) -> None:
        for npc in npcs:
            ...
            if any(key in reply for key in ('手机', '凌晨1点', '尸体照片', 'lo裙', '假发', '投票', '24', '23', '47', '48', '笔迹', '行李箱', '生物馆', '世纪林', '李海天', '1919', '张壹')):
                g.evidences()
    ...
    ask_all('李海天会长上周六是否在生物馆附近惨死？张壹是否在世纪林看奇怪网站或上周日失踪？袁樱瞳的尸块、1919黑车、生物馆和世纪林之间有什么联系？', ids)
    ask_all('请只说你亲眼看到或确切知道的：周六22:30谁从生物馆出来或在生物馆附近？李海天死亡和袁樱瞳分尸是否同一条线？', ids)

    if ANSWER_MODE == 'zhangyi':
        g.answer(
            '张壹',
            '张壹作为世纪林保安掌握李海天生物馆惨死、袁樱瞳尸块、1919黑车和校园异常网站线索，担心袁樱瞳揭开两起案件关联而灭口。',
            '张壹利用保安身份和校园路线把袁樱瞳引到生物馆、世纪林相关地点杀害并分尸，借凌晨1点手机照片、假发、行李箱和相似者制造身份与时间混淆。',
        )
    else:
        zero_answer(g)
