        ...
        if info_id:
            ...
            if g.stage >= 2:
                ...
                if reception_id:
                    ...
                    g.evidences()
                    if g.stage >= 3:
                        reception_name = cn_name(reception_id)
        if reception_name:
            suspect = reception_name
            motivation = f'{suspect}受Joker委托负责寄送邀请函、接待宾客和整理到达时间表，掌握梅花5未在名单中及死者身份秘密，为掩盖自己证词破绽而杀人灭口。'
            method = f'{suspect}利用接待者身份熟悉扑克公馆结构，提前处理厨房刀具、死者房间和面具线索，在衣帽间杀害戴梅花5面具的死者，并用全员戴面具和现场异常伪造身份混淆。'
        else:
