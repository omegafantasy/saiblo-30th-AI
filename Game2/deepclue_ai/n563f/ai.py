...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        ask_all('请只按时间线说：袁樱瞳最后出现、手机被捡到或清空、凌晨1点照片、课程投票、尸块发现、生物馆和1919黑车先后顺序。')
        ask_all('谁能在这些时间点接触手机、行李箱、尸块、假发或照片？谁的不在场证明最矛盾？')
        yuan_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}]
        ask_all('结合现有证据，死亡时间被伪造在哪里？请指出手机、照片、假发和尸块的先后矛盾。', yuan_ids)
        ask_all('我还缺哪项关键调查才能锁定死亡时间？请说未公开证词或物证。', yuan_ids)
