    ...
    if '扑克公馆' in text:
        ...
        if info_id:
            ...
            if g.stage >= 2:
                ...
                if reception_id:
                    g.chat(reception_id, '请调取扑克公馆餐厅11:00到13:00监控和大门口0:00到13:00监控；重点说明7:30不明身份人、8:20离开、8:50梅花5到达、12:00梅花5进餐厅和12:05离开。')
                    poker_evidences = g.evidences()
                    monitor_ids = [
                        str(ev.get('id'))
                        for ev in poker_evidences
                        if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '301', '302', '303', '304', '305', '401', '402'}
                    ]
                    if monitor_ids:
