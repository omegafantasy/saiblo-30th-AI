...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        g.req('background')
        ask_all('先不要猜凶手。请按完整剧情说明袁樱瞳碎尸案：周四聚会、周五课程展示、凌晨1点照片、周六世纪林尸块、黄色行李箱、手机清空、lo裙栗色假发、1919黑车、保安网站、生物馆、李海天旧案分别怎么连接？')
        ask_all('逐人说明你本人确认的事实：谁和袁樱瞳长得像并竞争出国名额，谁捡到或清空手机，谁推黄色行李箱，谁知道投票异常，谁看到生物馆和1919黑车，谁掌握李海天尸检或蓝色背包海豚挂件？')
        ask_all('把袁樱瞳案和李海天案并排比较：死亡时间、背刺、大量失血、四肢被砍、尸块位置、海豚挂件、蓝色背包、生物馆、世纪林、张壹传闻中哪些相同哪些不同？')
        yuan_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}]
        ask_all('请优先给我未公开物证：袁樱瞳手机、课程展示投票纸、李海天尸检报告、监控、黑车记录、保安网站日志、行李箱来源、假发lo裙来源和尸块DNA。缺哪一个就说明谁持有。', yuan_ids)
        ask_all('结合现有证据重建真实时间线：袁樱瞳何时死亡，凌晨1点照片是谁发的，照片女性是谁，谁伪装她活着，谁处理尸块，谁制造张壹从生物馆跑出的传闻？', yuan_ids)
        ask_all('最后只回答证据链，不讲传闻：哪件物证能把手机、投票异常、李海天尸检、生物馆、1919黑车和黄色行李箱连到同一个隐瞒者或凶手？', yuan_ids)
