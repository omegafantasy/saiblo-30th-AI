...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        if g.stage >= 3:
            suspect = '林渝植'
            motivation = '林渝植为了隐藏自己三年前失踪后的真实身份和梅花5身份秘密，利用扑克公馆全员戴面具的规则把自己的死亡伪装成他杀。'
            method = '林渝植提前搜索自杀伪装成谋杀的方法，用方形塑料盒把三把厨房刀的刀柄冻入冰块，借冰块固定刀具刺入背部；冰融化后留下无指纹刀具、稀释血水、塑料盒和厨房少刀，形成他杀假象。'
