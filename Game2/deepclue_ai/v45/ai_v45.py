#!/usr/bin/env python3
"""Game2 DeepClue AI v45 — single pre-answer probe for NPC_D.

Purpose:
- Preserve the proven v43 path everywhere else
- Keep only one hidden pre-answer question in case1:
  `NPC_D / F的QQ里面还有什么信息？`
- Verify whether the D-side hidden question alone is sufficient to reproduce
...
def solve_case0(g: Game) -> None:
    ...
    log(f'[v45] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1(g: Game) -> None:
    ...
    resp = g.probe_chat('NPC_D', 'F的QQ里面还有什么信息？')
    if isinstance(resp, dict):
        log(
            f"[v45] pre-answer probe [NPC_D] stage={resp.get('stage')} "
            f"keys={sorted(resp.keys())} error={resp.get('error')!r}"
        )
    else:
        log(f'[v45] pre-answer probe [NPC_D] non-dict={type(resp).__name__}')

    log(f'[v45] case1 stage={g.stage} calls={g.calls}')
    result = g.submit(
        murderer='E',
    ...
    log(f'[v45] case1 submit_keys={sorted(result.keys()) if isinstance(result, dict) else []}')
...
def solve_case(g: Game, case_index: int) -> bool:
    ...
    log(f'[v45] case={case_index} npcs={sorted_npcs}')
    g.stage = 0
    g.calls = 0
...
def main() -> int:
    ...
    while True:
        try:
            ...
            log(f'[v45] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
