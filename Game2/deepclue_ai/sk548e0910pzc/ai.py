                ...
                if reception_id and g.stage >= 3:
                    ...
                    if {'401', '402'}.issubset(set(monitor_ids)):
                        ...
                        if isinstance(ev404, dict):
                            car_name = str(ev404.get('name', '')).replace('车牌号', '').strip()
                            car_id = id_for_name(car_name, follow_npcs) if car_name else ''
                            ids404 = [
                                str(ev.get('id'))
                                for ev in poker_evidences
                                if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404'}
                            ]
                            target_id = car_id or info_id
                            g.chat(target_id, '结合404车牌号京F·A7590、7:20驶过大门的车辆、401/402监控和衣帽间移尸破绽，直接说明车是谁开的、死者真正在哪里遇害、尸体如何被移到衣帽间，并给出下一份证据。', ids404)
                            g.evidences()
