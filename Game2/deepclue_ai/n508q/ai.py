    ...
    if '扑克公馆' in text:
        ...
        for npc in npcs:
            if current_marks.get(npc) is False:
                suspect = cn_name(npc)
                break
        method = f'{suspect}利用自己与袁樱瞳长相相似及黄色行李箱制造混淆，清空袁樱瞳手机并借凌晨1点尸体照片、假发、行李箱和生物馆线索转移视线，随后分尸抛尸。'
        motivation = f'{suspect}因与袁樱瞳竞争出国交流名额并在异常投票中获利，担心袁樱瞳周五揭穿投票作弊、1919黑车和生物馆相关秘密而杀人灭口。'
        log(f'[v56] unknown hint={compact(hint, 50)} suspect={suspect}')
        g.answer(murderer=suspect, motivation=motivation, method=method)
        return
    else:
