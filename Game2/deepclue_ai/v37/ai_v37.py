#!/usr/bin/env python3
"""Game2 DeepClue AI v37 — exact-admin retest on isolated entity.

Purpose:
- Re-test the exact admin-style scripted path on 2026-04-18
- Keep Case 1 ordering identical to the v27/admin sequence
- Keep final murderer answer as 'E'
- Run on a separate entity so the current g2auto mainline is untouched

If this still stabilizes at 2220 across multiple self-runs, the remaining gap
is very unlikely to come from question ordering or the NPC_E/E alias alone.
"""
...
def solve_case0(g: Game) -> None:
    ...
    log(f'[v37] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1(g: Game) -> None:
    ...
    log(f'[v37] case1 stage={g.stage} calls={g.calls}')
    result = g.submit(
        murderer='E',
...
def solve_case(g: Game, case_index: int) -> bool:
    ...
    log(f'[v37] case={case_index} npcs={sorted_npcs}')
    g.stage = 0
    g.calls = 0
...
def main() -> int:
    ...
    while True:
        try:
            ...
            log(f'[v37] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
