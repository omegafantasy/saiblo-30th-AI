...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
                ...
                if 'poker_after_ids' in locals():
                    ...
                    if {'601', '602', '603', '604'} & set(n601_ids) or '606' in n601_ids:
                        for npc_id in n601_targets[:18]:
                            n601_ask(npc_id, '601-604或606若已出现，直接闭环最终层：谁把Joker引入衣帽间，谁实际杀死Joker，谁给密码，林渝植/真正梅花5、刘丽雯/张壹、于书华、张子韩、景观警方各自分工和可提交答案。')
                        n601_ids = [
                        ...
                        for npc_id in n601_targets[:18]:
                            n601_ask(npc_id, '不要再列证据编号。请按作案链回答：8:50聊天、特殊邀请函、LYZ项链、衣帽间密码、刀具/移尸、POKER组织、最终凶手和动机。')
    ...
    final_mode = str(globals().get('N601_ANSWER_MODE', ''))
