...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
            ...
            if g.stage >= 2:
                ...
                if reception_id and g.stage >= 3:
                    ...
                    if {'401', '402'}.issubset(set(monitor_ids)):
                        ...
                        deep_after_set = set(deep_after_ids)
                        if '501' in deep_after_set or deep_after_set & {'601', '602', '603', '604'}:
                            def refresh_deep_after() -> tuple[list[dict[str, Any]], list[str], set[str]]:
                            ...
                            deep_after_targets: list[str] = []

                            def add_deep_after_target(npc_id: str) -> None:
                                if npc_id and npc_id not in deep_after_targets:
                                    deep_after_targets.append(npc_id)
                            ...
                            for npc_id in (
                                ...
                                id_for_name_any('王泽', follow_npcs),
                                id_for_name_any('于书华', follow_npcs),
                                id_for_name_any('张子韩', follow_npcs),
                                id_for_name_any('沈知遥', follow_npcs),
                                id_for_name_any('顾云舒', follow_npcs),
                            ):
                                add_deep_after_target(npc_id)

                            deep_after_context = '\n'.join(
                                str(ev.get('name', '')) + str(ev.get('content', '')) for ev in poker_evidences
                            )
                            for name in extract_story_names(deep_after_context):
                                for npc_id in global_name_ids(name, follow_npcs):
                                    add_deep_after_target(npc_id)
                            if deep_after_set & {'601', '602', '603', '604'}:
                                for npc_id in follow_npcs:
                                    add_deep_after_target(npc_id)

                            asked_after: set[str] = set()

                            def ask_after(npc_id: str, question: str) -> None:
                                nonlocal deep_after_ids, deep_after_set
                                if not npc_id or npc_id in asked_after:
                                    return
                                asked_after.add(npc_id)
                                chat_visible_or_probe(g, npc_id, follow_npcs, question, deep_after_ids)
                                _, deep_after_ids, deep_after_set = refresh_deep_after()

                            if '501' in deep_after_set and not (deep_after_set & {'503', '504', '601', '602', '603', '604'}):
                                ask_after(info_id, '501匿名50万元转账已经出现。不要回到车辆和动机，直接查后段剧情：死者手机云端、梅花5与Joker聊天、特殊邀请函、LYZ随身物、林渝植失踪卷宗、张子韩女儿亲子鉴定、花纹村人口贩卖案和POKER纹身照片，下一份证据编号和持有人是谁？')
                                ask_after(reception_id, '你掌握Joker账号、地址表和邀请函。现在只查501之后的源文件：Joker设备/IP、梅花5特殊邀请函、死者手机聊天、寄送记录、云端名单、POKER纹身照片或警方最终卷宗，下一证据在哪里？')

                            if deep_after_set & {'601', '602', '603', '604'}:
                                globals()['POKER_HAS_601'] = True
                                post601_questions: list[tuple[str, str]] = [
                                    (info_id, '601-604已经出现：2010失踪少女、2015花纹村人口贩卖、刘丽雯手术事故和张子韩复学证明已经成链。下一步只查605/606/607/608：Joker、沈知遥、张子韩左臂POKER纹身三人照片，死者手机云端名单，花纹村成员名册，警方结案证据。证据编号、名称、持有人分别是什么？'),
                                    (true_club5_id, '你就是林渝植/真正梅花5。601的右眼角心形胎记和504的LYZ随身物已经对上。请直接说明谁把Joker引进衣帽间、谁杀了Joker、你和张子韩的亲子关系、左臂POKER纹身三人照片或最终物证在哪里。'),
                                    ...
                                    (id_for_name_any('沈知遥', follow_npcs), '601-604后线索指向沈知遥。请不要否认编号，直接说明你与Joker、张子韩、花纹村人口贩卖集团、左臂POKER纹身三人照片和最终凶手证据的关系。'),
                                    (id_for_name_any('王科瑾', follow_npcs), '501和601-604已经串起来。你电脑/账户可能连接转账源和Joker名单：请直接交出银行流水、人口贩卖名单、花纹村成员、POKER纹身三人照片或最终卷宗编号。'),
                                ]
                                for npc_id, question in post601_questions:
                                    ask_after(npc_id, question)
                                    if deep_after_set & {'605', '606', '607', '608'}:
                                        break

                                if not (deep_after_set & {'605', '606', '607', '608'}):
                                    for npc_id in deep_after_targets[:8]:
                                        ask_after(npc_id, 'Poker后段不要再复述601-604。现在只查下一层官方证据：605/606/607/608、左臂POKER纹身三人照片、死者手机云端名单、花纹村成员名册、张子韩女儿DNA、Joker真实身份和警方最终卷宗。若你持有，请直接给编号、证据名、内容。')
                                        if deep_after_set & {'605', '606', '607', '608'}:
                                            break

                                if '606' in deep_after_set:
                                    for npc_id in deep_after_targets[:4]:
                                        ask_after(npc_id, '606三人照片已经出现。继续追605/607/608：POKER纹身组织名册、Joker人口贩卖名单、沈知遥和张子韩关系、林渝植亲子鉴定、谁杀Joker以及最终可提交证据。')
                        late_ids = [
                            str(ev.get('id'))
                            for ev in poker_evidences
                            if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404', '405', '501', '502'}
        ...
        if forensic_target_id and '704' in set(yuan_ids) and '706' not in set(yuan_ids):
            resp = g.chat(forensic_target_id, '不要只解释投票纸。出国名额的行政链在哪里：推荐表、学院系统日志、名单变更、导师签字、办公室邮件/监控、谁能改结果、袁樱瞳周五要揭发哪份记录？如果是物证06/706请直接给编号和持有人。', yuan_ids)
            yuan_replies[forensic_target_id] = yuan_replies.get(forensic_target_id, '') + '\n' + response_text(resp)
            yuan_evidences = g.evidences()
            yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
