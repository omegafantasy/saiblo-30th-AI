        ...
        if info_id:
            ...
            if g.stage >= 2:
                ...
                if 'poker_after_ids' in locals():
                    ...
                    if {'601', '602', '603', '604', '606'} & set(n604_ids):
                        for npc_id in n604_targets[:14]:
                            n604_ask(npc_id, '后续材料已经指向Joker周克、于书华、林渝植、刘丽雯女儿和花纹村组织。现在不要复述时间线，只查那张三人合照、左臂POKER纹身、照片原件来源、组织成员名单和现场尚未交出的原始物证；请说清证据名、保管人、内容和它如何证明谁把Joker引入衣帽间。')
                        n604_ids = [
                            str(ev.get('id')) for ev in g.evidences()
                            if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404', '405', '501', '502', '503', '504', '601', '602', '603', '604', '605', '606', '607', '608'}
                        ]
                        for npc_id in n604_targets[:14]:
                            n604_ask(npc_id, '如果三人照片或POKER纹身已经出现，请继续查照片中每个人的真实姓名、红桃Q或接待者身份、Joker周克与于书华的关系、花纹村人口贩卖名单、林渝植身份原件和最终结案卷宗。')
                    elif {'501', '502', '503', '504'} & set(n604_ids):
                        for npc_id in n604_targets[:10]:
                            n604_ask(npc_id, '不要停在车牌、转账或邀请函。沿着照片纹身链继续查：Joker真实姓名、死者手机、LYZ随身物、刘丽雯女儿、花纹村组织、POKER纹身照片和现场未交原件分别由谁保管。')
                ...
                if 'poker_after_ids' in locals():
                    ...
                    if {'601', '602', '603', '604', '606'} & set(n604_ids):
                        for npc_id in n604_targets[:14]:
                            n604_ask(npc_id, '现在只查接待和联络链：红桃Q、刘瑄、张朔、密码、特殊邀请函、8:50衣帽间约见、面具编号和Joker周克入场记录。请直接说明谁掌握密码，谁安排约见，谁把Joker引入衣帽间，现场未交原件是什么。')
                        n604_ids = [
                            str(ev.get('id')) for ev in g.evidences()
                            if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404', '405', '501', '502', '503', '504', '601', '602', '603', '604', '605', '606', '607', '608'}
                        ]
                        for npc_id in n604_targets[:14]:
                            n604_ask(npc_id, '如果张朔只是表面名字，请说出刘瑄/红桃Q的完整身份、她与Joker周克、于书华、花纹村组织、林渝植失踪案的关系，以及能证明最终凶手的原始联络记录或结案证据。')
                    elif {'501', '502', '503', '504'} & set(n604_ids):
                        for npc_id in n604_targets[:10]:
                            n604_ask(npc_id, '不要停在车牌、转账或邀请函。沿着联络密码链继续查：Joker真实姓名、死者手机、LYZ随身物、刘丽雯女儿、花纹村组织、POKER纹身照片和现场未交原件分别由谁保管。')
                    globals()['N604_POKER_IDS'] = ','.join(str(ev.get('id')) for ev in g.evidences())
                    poker_after_ids = [str(ev.get('id')) for ev in g.evidences()]
