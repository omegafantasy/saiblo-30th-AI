...
DEBUG = False
ISO_MODE = 'rpfull_zdirect_yzero'
ROSE_GUARD_MIN_HITS = 2
CASE_BOOTSTRAP_RETRIES = 5
CASE_BOOTSTRAP_SLEEP_SEC = 0.16
ROSE_ROLE_REMAP_RETRIES = 6
ROSE_ROLE_REMAP_SLEEP_SEC = 0.16
ROSE_GUARD_FALSE_IDS = {'ZhouLinJun'}
ROSE_GUARD_HOST_NAMES = {'楚戎臻'}
ROSE_GUARD_PARTNER_NAMES = {'赵一橙'}
ROSE_GUARD_BANKER_NAMES = {'张壹'}
ROSE_SUFFIX_KILLERS = {'陆亦初', '楚戎臻', '王泽', '周林君'}
ROSE_REQUIRED_ROLES = {'xiao', 'bai', 'deng', 'fan', 'ye', 'cui'}
...
def wrong_name(kind: str, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> str:
    if kind in {'rose', 'yuan'}:
        for npc in npcs:
            if marks.get(npc) is True:
                return cn_name(npc)
    if kind == 'zf':
        for ev in evidences:
            if str(ev.get('id')) == '002':
...
def zero_answer(g: Game, kind: str, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]], style: str) -> None:
    if style == 'soft':
        g.answer(wrong_name(kind, npcs, marks, evidences), '无', '无')
    else:
        g.answer('无名氏', '无', '无')
...
def solve_poker_direct(g: Game, npcs: list[str], marks: dict[str, bool], hint: str) -> None:
    ...
    if hint_id:
        ...
        suspect = cn_name(marked_true[0]) if marked_true else (cn_name(npcs[0]) if npcs else '')
    g.answer(
        murderer=suspect,
        motivation='未知',
...
def solve_yuan_direct(g: Game, npcs: list[str], marks: dict[str, bool]) -> None:
    ...
    g.answer(
        murderer=suspect,
        motivation=f'{suspect}与袁樱瞳竞争出国交流名额，49人中2人不投却出现48票，{suspect}以24比23多一票获利；袁樱瞳准备周五揭穿投票作弊和替身秘密，{suspect}担心暴露而杀人。',
...
def yuan_winner_from_reply(reply: str, npc: str) -> str:
    ...
    if '袁樱瞳' not in text:
        return ''
    if re.search(r'(我只比袁樱瞳多|我.*险胜袁樱瞳|我们长得挺像|竞争那个出国名额)', text) and not re.search(r'授课教师|课程.*教师|主持', text):
        return cn_name(npc)
    for pattern in (
        r'最终([一-龥]{2,4})以\s*24\s*票?对\s*23\s*票?险胜',
        r'([一-龥]{2,4})以\s*24\s*票?对\s*23\s*票?险胜',
        r'([一-龥]{2,4})和袁樱瞳[^。]*表现[^。]*(?:平分|难分伯仲)',
    ):
        m = re.search(pattern, text)
        if m:
            return m.group(1)
...
def solve_yuan_probe(g: Game, npcs: list[str], marks: dict[str, bool]) -> None:
    suspect = cn_name(npcs[0]) if npcs else ''
    for npc in npcs:
        if marks.get(npc) is False:
            suspect = cn_name(npc)
    ...
    winner = ''
    for npc in npcs:
        if marks.get(npc) is not True:
            continue
        resp = g.chat(npc, '你和袁樱瞳是否长得相似并竞争出国名额？你有没有捡到她的手机，手机里留下了什么？投票结果为什么只多一票？')
        reply = response_text(resp)
        parsed = yuan_winner_from_reply(reply, npc)
        if parsed:
            winner = parsed
        if any(key in reply for key in ('手机', '凌晨1点', '尸体照片', 'lo裙', '假发', '多出', '48', '49', '24', '23', '投票箱', '笔迹')):
            g.evidences()
        if any(key in reply for key in ('手机', '凌晨1点', '尸体照片')) and winner:
            break
    for npc in npcs:
        if marks.get(npc) is not True:
            continue
        resp = g.chat(npc, '课程展示投票的总人数、得票数和笔迹有没有异常？谁以24比23险胜袁樱瞳？')
        reply = response_text(resp)
        parsed = yuan_winner_from_reply(reply, npc)
        if parsed:
            winner = parsed
        if any(key in reply for key in ('多出', '48', '49', '24', '23', '投票箱', '笔迹')):
            g.evidences()
            break
    if winner:
        suspect = winner
    solve_yuan_direct_named(g, suspect)


def solve_yuan_direct_named(g: Game, suspect: str) -> None:
    g.answer(
        murderer=suspect,
        motivation=f'{suspect}与袁樱瞳竞争出国交流名额，49人中2人不投却出现48票，{suspect}以24比23多一票获利；袁樱瞳准备周五揭穿投票作弊和替身秘密，{suspect}担心暴露而杀人。',
        method=f'{suspect}利用自己与袁樱瞳长相相似及黄色行李箱制造混淆，取得并清空袁樱瞳手机，围绕凌晨1点女性尸体照片、假发和行李转移视线，随后分尸抛尸。',
...
def solve_direct_kind(g: Game, kind: str, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    if kind == 'rose':
        ...
        solve_yuan_direct(g, npcs, marks)
    else:
...
def rose_guardrail_hits(roles: dict[str, tuple[str, str]]) -> list[str]:
    hits: list[str] = []
    cui_id = roles.get('cui', ('', ''))[1]
    xiao_name = roles.get('xiao', ('', ''))[0]
    bai_name = roles.get('bai', ('', ''))[0]
    deng_name = roles.get('deng', ('', ''))[0]
    if cui_id in ROSE_GUARD_FALSE_IDS:
        hits.append(f'false={cui_id}')
    if xiao_name in ROSE_GUARD_HOST_NAMES:
        hits.append(f'host={xiao_name}')
    if bai_name in ROSE_GUARD_PARTNER_NAMES:
        hits.append(f'partner={bai_name}')
    if deng_name in ROSE_GUARD_BANKER_NAMES:
        hits.append(f'banker={deng_name}')
    return hits


def rose_motivation_text(murderer: str, banker: str) -> str:
    text = (
        f'{murderer}爱慕并想独占{banker}，认为Rose纠缠{banker}、会阻碍自己与{banker}在一起，'
        f'因此为扫清情敌而杀害Rose。'
    )
    if murderer in ROSE_SUFFIX_KILLERS:
        text += f'此外，Rose已围绕{banker}关系持续施压，{murderer}担心关系败露后失去{banker}，遂下决心行凶。'
    return text


def refresh_rose_roles(
    g: Game,
    npcs: list[str],
    marks: dict[str, bool],
    evidences: list[dict[str, Any]],
) -> tuple[dict[str, tuple[str, str]], list[str], dict[str, bool], list[dict[str, Any]]]:
    cur_npcs = list(npcs)
    cur_marks = dict(marks)
    cur_evidences = list(evidences)
    roles = rose_roles(cur_npcs, cur_marks, cur_evidences)
    if set(roles) == ROSE_REQUIRED_ROLES:
        return roles, cur_npcs, cur_marks, cur_evidences

    for attempt in range(ROSE_ROLE_REMAP_RETRIES):
        time.sleep(ROSE_ROLE_REMAP_SLEEP_SEC)
        got_npcs = g.npcs()
        if got_npcs:
            cur_npcs = got_npcs
        got_marks = g.marks()
        if got_marks:
            cur_marks = got_marks
        got_evidences = g.evidences()
        if got_evidences:
            cur_evidences = got_evidences
        roles = rose_roles(cur_npcs, cur_marks, cur_evidences)
        if set(roles) == ROSE_REQUIRED_ROLES:
            log(f'[v56] rose remap recovered on attempt={attempt + 1}')
            return roles, cur_npcs, cur_marks, cur_evidences
    return roles, cur_npcs, cur_marks, cur_evidences
...
def solve_rose_direct(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    g.answer(
        murderer=murderer,
        motivation=rose_motivation_text(murderer, banker),
...
def solve_rose(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    roles, npcs, marks, evidences = refresh_rose_roles(g, npcs, marks, evidences)
    if set(roles) != ROSE_REQUIRED_ROLES:
        log(f'[v56] rose role mapping incomplete roles={roles}')
        solve_rose_direct(g, npcs, marks, evidences)
        return

    hits = rose_guardrail_hits(roles)
    if len(hits) >= ROSE_GUARD_MIN_HITS:
        log(f'[v56] rose guardrail hits={hits} -> direct')
        solve_rose_direct(g, npcs, marks, evidences)
        return
    ...
    g.answer(
        murderer=cui_name,
        motivation=rose_motivation_text(cui_name, deng_name),
        ...
        if info_id:
            ...
            if g.stage < 2:
                for reception_id in poker_reception_candidates(g.npcs() or npcs, info_id, g.hint(), g.marks()):
                    resp = g.chat(reception_id, '请说说聊天记录、宾客到达时间表，以及公馆内的异常发现。')
                    if any(word in response_text(resp) for word in ('Joker', '聊天记录', '到达时间', '电脑', '方形塑料盒', '厨房')):
                        g.evidences()
                    if g.stage >= 2:
                        break
...
COMBO_MODES: dict[str, tuple[set[str], set[str], str]] = {
    'rfull_zp_direct_yzero': ({'rose'}, {'zf', 'poker'}, 'zero'),
    'zfull_rp_direct_yzero': ({'zf'}, {'rose', 'poker'}, 'zero'),
    'pfull_rz_direct_yzero': ({'poker'}, {'rose', 'zf'}, 'zero'),
    'rzfull_pdirect_yzero': ({'rose', 'zf'}, {'poker'}, 'zero'),
    'rpfull_zdirect_yzero': ({'rose', 'poker'}, {'zf'}, 'zero'),
    'zpfull_rdirect_yzero': ({'zf', 'poker'}, {'rose'}, 'zero'),
    'rzpfull_yzero': ({'rose', 'zf', 'poker'}, set(), 'zero'),
    'rzpfull_ydirect': ({'rose', 'zf', 'poker'}, set(), 'direct'),
    'rzpfull_yprobe': ({'rose', 'zf', 'poker'}, set(), 'probe'),
    'rzp_direct_yprobe': (set(), {'rose', 'zf', 'poker'}, 'probe'),
}


def solve_full_kind(g: Game, kind: str, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    if kind == 'rose':
        ...
        solve_yuan_probe(g, npcs, marks)
    else:
        zero_answer(g, kind, npcs, marks, evidences, 'hard')


def solve_combo_mode(g: Game, mode: str, kind: str, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    full_kinds, direct_kinds, yuan_mode = COMBO_MODES[mode]
    if kind == 'yuan':
        if yuan_mode == 'probe':
            solve_yuan_probe(g, npcs, marks)
        elif yuan_mode == 'direct':
            solve_yuan_direct(g, npcs, marks)
        else:
            zero_answer(g, kind, npcs, marks, evidences, 'hard')
    elif kind in full_kinds:
        solve_full_kind(g, kind, npcs, marks, hint, evidences)
...
def bootstrap_case_state(g: Game) -> tuple[list[str], dict[str, bool], str, list[dict[str, Any]]]:
    npcs: list[str] = []
    marks: dict[str, bool] = {}
    hint = ''
    evidences: list[dict[str, Any]] = []

    for attempt in range(CASE_BOOTSTRAP_RETRIES):
        npcs = g.npcs()
        if npcs:
            break
        marks = g.marks()
        if marks:
            npcs = sorted(str(key) for key in marks.keys())
            if npcs:
                break
        hint = g.hint() or hint
        got_evidences = g.evidences()
        if got_evidences:
            evidences = got_evidences
        if npcs:
            break
        log(f'[v56] bootstrap retry={attempt} npcs-empty')
        time.sleep(CASE_BOOTSTRAP_SLEEP_SEC)

    if not marks:
        marks = g.marks()
    if not npcs and marks:
        npcs = sorted(str(key) for key in marks.keys())
    hint = g.hint() or hint
    got_evidences = g.evidences()
    if got_evidences:
...
def solve_case(g: Game, case_idx: int) -> bool:
    npcs, marks, hint, evidences = bootstrap_case_state(g)
    if not npcs:
        log(f'[v56] case={case_idx} bootstrap failed, fallback answer')
        g.answer('无名氏', '未知', '未知')
        return True
    text = all_text(hint, evidences)
    kind = case_kind(text)
    log(f'[iso] mode={ISO_MODE} case={case_idx} kind={kind} npcs={sorted(npcs)} marks={marks} hint={compact(hint, 60)}')
    if ISO_MODE == 'zero_hard':
        zero_answer(g, kind, npcs, marks, evidences, 'hard')
    elif ISO_MODE == 'zero_soft':
        zero_answer(g, kind, npcs, marks, evidences, 'soft')
    elif ISO_MODE == 'direct_min':
        solve_direct_kind(g, kind, npcs, marks, hint, evidences)
    elif ISO_MODE == 'rose_only':
        solve_rose(g, npcs, marks, evidences) if kind == 'rose' else zero_answer(g, kind, npcs, marks, evidences, 'hard')
    elif ISO_MODE == 'zf_only':
        solve_z_script(g, npcs, evidences) if kind == 'zf' else zero_answer(g, kind, npcs, marks, evidences, 'hard')
    elif ISO_MODE == 'poker_only':
        solve_unknown(g, npcs, marks, hint, evidences) if kind == 'poker' else zero_answer(g, kind, npcs, marks, evidences, 'hard')
    elif ISO_MODE == 'yuan_direct':
        solve_yuan_direct(g, npcs, marks) if kind == 'yuan' else zero_answer(g, kind, npcs, marks, evidences, 'hard')
    elif ISO_MODE == 'yuan_probe':
        solve_yuan_probe(g, npcs, marks) if kind == 'yuan' else zero_answer(g, kind, npcs, marks, evidences, 'hard')
    elif ISO_MODE == 'rose_direct_only':
        solve_rose_direct(g, npcs, marks, evidences) if kind == 'rose' else zero_answer(g, kind, npcs, marks, evidences, 'hard')
    elif ISO_MODE == 'zf_direct_only':
        solve_z_direct(g, evidences) if kind == 'zf' else zero_answer(g, kind, npcs, marks, evidences, 'hard')
    elif ISO_MODE == 'poker_direct_only':
        solve_poker_direct(g, npcs, marks, hint) if kind == 'poker' else zero_answer(g, kind, npcs, marks, evidences, 'hard')
    elif ISO_MODE == 'rose_zf_direct':
        solve_direct_kind(g, kind, npcs, marks, hint, evidences) if kind in {'rose', 'zf'} else zero_answer(g, kind, npcs, marks, evidences, 'hard')
    elif ISO_MODE == 'rose_poker_direct':
        solve_direct_kind(g, kind, npcs, marks, hint, evidences) if kind in {'rose', 'poker'} else zero_answer(g, kind, npcs, marks, evidences, 'hard')
    elif ISO_MODE == 'zf_poker_direct':
        solve_direct_kind(g, kind, npcs, marks, hint, evidences) if kind in {'zf', 'poker'} else zero_answer(g, kind, npcs, marks, evidences, 'hard')
    elif ISO_MODE == 'rose_zf_poker_direct':
    ...
    elif ISO_MODE in COMBO_MODES:
        solve_combo_mode(g, ISO_MODE, kind, npcs, marks, hint, evidences)
    else:
        solve_direct_kind(g, kind, npcs, marks, hint, evidences)
    return True
