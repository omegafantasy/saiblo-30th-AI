                    ...
                    if {'401', '402'}.issubset(set(monitor_ids)):
                        ...
                        if post601_set & {'601', '602', '603', '604'}:
                            ...
                            post601_questions: list[tuple[str, str]] = [
                                (info_id, '601-604已经出现：2010失踪少女、2015花纹村人口贩卖、刘丽雯手术事故和张子韩复学证明已经成链。不要回头问车和转账，直接查605/606/607/608：Joker、沈知遥、张子韩左臂POKER纹身三人照片，死者手机云端名单，花纹村成员名册，警方结案证据。证据编号、名称、持有人分别是什么？'),
                            ...
                            for npc_id, question in post601_questions:
                                ...
                                if post601_set & {'605', '606', '607', '608'}:
                                    break
                            if not (post601_set & {'605', '606', '607', '608'}):
                                for npc_id in post601_targets[:6]:
                                    ask_post601(npc_id, 'Poker后段不要再复述601-604。现在只查下一层官方证据：605/606/607/608、左臂POKER纹身三人照片、死者手机云端名单、花纹村成员名册、张子韩女儿DNA、Joker真实身份和警方最终卷宗。若你持有，请直接给编号、证据名、内容。')
                                    if post601_set & {'605', '606', '607', '608'}:
                                        break
                                ...
                                if not (deep_id_set & {'605', '606', '607', '608'}):
                                    for npc_id in deep_targets[:6]:
                                        if npc_id in asked_deep:
                                            continue
                                        asked_deep.add(npc_id)
                                        chat_visible_or_probe(g, npc_id, follow_npcs, '现在不要复述车和转账，只追Poker后段实物：死者手机剩余聊天或云端名单、LYZ物品来源、林渝植失踪档案、2010失踪少女报道、花纹村成员、张子韩女儿DNA、王科瑾电脑银行流水、Joker和沈知遥/张子韩的照片。你接触过哪一项？', deep_ids)
                                        _, deep_ids, deep_id_set = refresh_deep_ids()
                                        if deep_id_set & {'605', '606', '607', '608'}:
                                            break
