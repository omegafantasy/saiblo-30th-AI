            ...
            if g.stage >= 2:
                ...
                if reception_id and g.stage >= 3:
                    ...
                    if {'401', '402'}.issubset(set(monitor_ids)):
                        ...
                        if {'501', '601', '602', '603', '604'} & set(deep_after_ids):
                            deep_targets: list[str] = []

                            def add_deep_after(npc_id: str) -> None:
                            ...
                            for npc_id in (
                                ...
                                id_for_name_any('刘丽雯', follow_npcs),
                            ):
                                add_deep_after(npc_id)
                            deep_context = '\n'.join(
                                str(ev.get('name', '')) + str(ev.get('content', '')) for ev in poker_evidences
                            )
                            for name in extract_story_names(deep_context):
                                for npc_id in global_name_ids(name, follow_npcs):
                                    add_deep_after(npc_id)
                            for npc_id in follow_npcs:
                                add_deep_after(npc_id)

                            deep_resp = g.chat(info_id, '你是景观/警方来源。不要总结时间线，直接交出501-504及后续：车主、匿名转账、死者手机里梅花5与Joker聊天、梅花5特殊邀请函、刻着LYZ的随身物、DNA/指纹、人口失踪案、POKER纹身照片和下一阶段证据。', deep_after_ids)
                            deep_context += '\n' + response_text(deep_resp)
                            poker_evidences = g.evidences()
                            deep_after_ids = [
                                str(ev.get('id'))
                                for ev in poker_evidences
                            ...
                            for npc_id in deep_targets[:14]:
                                resp = chat_visible_or_probe(g, npc_id, follow_npcs, '501-504已经出现或即将出现。请直接给后续证据601/602/603/604/605/606：人口失踪案、人口贩卖集团、医生刘丽雯/张子韩、失踪女儿、心形胎记、LYZ项链、梅花5邀请函、Joker真实身份、左臂POKER纹身照片和最终凶手。', deep_after_ids)
                                deep_context += '\n' + response_text(resp)
                                poker_evidences = g.evidences()
                                deep_after_ids = [
                                    str(ev.get('id'))
                                ...
                                if '606' in set(deep_after_ids):
                                    break
                            if {'601', '602', '603', '604'} & set(deep_after_ids) and '606' not in set(deep_after_ids):
                                for npc_id in deep_targets[:14]:
                                    resp = chat_visible_or_probe(g, npc_id, follow_npcs, '601-604已经出现。继续追605/606或最终层：2010失踪少女、右眼角心形胎记、刘丽雯手术事故、张子韩复学、林渝植真实身份、谁把Joker引入衣帽间、谁杀死Joker、左臂POKER纹身三人照片和警方结案证据。', deep_after_ids)
                                    deep_context += '\n' + response_text(resp)
                                    poker_evidences = g.evidences()
                                    deep_after_ids = [
                                        str(ev.get('id'))
