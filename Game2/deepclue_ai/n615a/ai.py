            ...
            if g.stage >= 2:
                ...
                if 'poker_after_ids' in locals():
                    ...
                    deep_ids = [
                        ...
                        if eid in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404', '405', '501', '502', '503', '504', '505', '601', '602', '603', '604', '605', '606'}
                    ]
                ...
                if 'poker_after_ids' in locals():
                    ...
                    n601_ids = [
                        eid for eid in list(locals().get('poker_after_ids', []))
                        if eid in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404', '501', '502', '503', '504', '505', '601', '602', '603', '604', '605', '606', '607', '608'}
                    ]
                    ...
                    if {'601', '602', '603', '604'} & set(n601_ids) or '606' in n601_ids:
                        for npc_id in n601_targets[:18]:
                            n601_ask(npc_id, '601-604或606若已出现，停止复述案情，先按POKER组织最终材料追缺口：606三人照片、605现场未交原件、Joker周克/于书华/红桃Q刘瑄组织名册、密码来源、衣帽间约见记录、死者手机云端和警方结案证据。')
                        n601_ids = [
                            str(ev.get('id')) for ev in g.evidences()
                            if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404', '501', '502', '503', '504', '505', '601', '602', '603', '604', '605', '606', '607', '608'}
                        ]
