...
def poker_info_name(hint: str, npcs: list[str] | None = None) -> str:
    text = hint or ''
    valid_names = {cn_name(npc) for npc in (npcs or [])}
    for pattern in (
        r'([一-龥]{2,4})是个好的信息来源',
        r'([一-龥]{2,4})会是[^，。]*好的信息来源',
        r'问问([一-龥]{2,4})关于',
        ...
        if m:
            ...
            if not valid_names or name in valid_names:
                return name
    if npcs is not None:
        hits = [cn_name(npc) for npc in npcs if cn_name(npc) and cn_name(npc) in text]
        if len(hits) == 1:
            return hits[0]
    return ''
    ...
    if '扑克公馆' in text:
        ...
        for npc in npcs:
            if current_marks.get(npc) is False:
                ...
                break
        if suspect_id:
            g.chat(suspect_id, '袁樱瞳手机、凌晨1点女性尸体照片、栗色假发、长相相似和课程展示投票多一票分别说明什么？')
            g.evidences()
