#!/usr/bin/env python3
"""Game2 DeepClue AI v34 — Alternate Case 1 investigation path.

Strategy: Use a COMPLETELY DIFFERENT question ordering for Case 1
to see if different investigation paths can:
1. Unlock missing testimonies (502/601/606)
2. Potentially reach stage 8 through an alternate trigger
3. Discover new stage transitions via different evidence combinations

Case 0 stays the same (proven optimal).
Case 1 reverses the NPC investigation order and tries different
evidence presentation timing.

Key hypothesis: admin reaches stage 8 not because of identity,
but because of some subtle ordering/timing difference we haven't replicated.
"""
...
def solve_case0(g: Game) -> None:
    """Case 0: Same proven strategy as v26."""
    ...
    if g.stage < 2:
        g.retry_trigger([('BaiJingTing', 'Rose给你的印象怎么样？')], max_retries=2)
    ...
    if g.stage < 3:
        g.retry_trigger([('YeWenXiao', '你今晚都去了哪些地方？')], max_retries=2)
    ...
    if g.stage < 4:
        g.retry_trigger([('YeWenXiao', 'Rose今天有什么异常吗？')], max_retries=2)
    ...
    if g.stage < 5:
        g.retry_trigger([('DengDaLing', '崔安彦最近有什么异常行为？')], max_retries=2)
    ...
    log(f'[v34] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1_alternate(g: Game) -> None:
    """Case 1: Alternate investigation path — different NPC order, evidence timing.

    Key differences from v26:
    - Start with NPC_E (the culprit) instead of NPC_A
    - Present initial evidence (007/008) early
    - Present evidence 313 EARLIER (at stage 3 instead of 4)
    - Ask about F's death before establishing relationships
    - Try confrontational questions at different stages
    - Try multi-evidence presentation
    """
    # Stage 1 — Start with E and C (key NPCs), present initial evidence
    g.chat('NPC_E', '你和Z是什么关系？', evidences=['008'])  # Z's situation evidence
    g.chat('NPC_E', '你知道Z失踪了吗？')
    g.chat('NPC_C', '你和F关系怎么样？', evidences=['007'])  # F unreachable evidence
    ...
    # Stage 2 — explore relationships early
    ...
    # Stage 3 — explore the high school incident
    ...
    # Stage 4 — EARLY evidence presentation with DIFFERENT questions
    g.chat('NPC_D', '关于高中那件事，你知道是谁造谣的吗？')

    # Present autopsy evidence but with DIFFERENT questions than v26
    g.chat('NPC_E', 'F被杀了，你怎么看？', evidences=['313'])
    g.chat('NPC_C', '你昨晚回宿舍看到什么了？', evidences=['313'])
    ...
    # Stage 5 — evidence-heavy investigation
    ...
    # Try multi-evidence presentations for missing testimonies
    g.chat('NPC_D', 'F的日记里提到了你，你怎么解释？', evidences=['311', '313'])
    g.chat('NPC_C', '这张地图和F的死有关吗？', evidences=['312', '313'])
    g.chat('NPC_B', '这个可乐瓶子是你的吗？', evidences=['314', '313'])
    g.chat('NPC_E', 'F的日记里写了什么关于你的事？', evidences=['311', '315'])
    ...
    g.chat('NPC_A', '红U盘去哪了？')
    if g.stage < 6:
        g.retry_trigger([('NPC_A', 'F的U盘你知道在哪吗？')])

    # Stage 6
    g.chat('NPC_A', '你是不是拆了D的车？')
    g.chat('NPC_B', '你知道那个F的U盘去哪了吗？')
    g.chat('NPC_D', '你对E还有感情吗？')
    ...
    g.chat('NPC_D', '你为什么装病？')
    if g.stage < 7:
        g.retry_trigger([('NPC_D', '你的脚到底有没有受伤？')])

    # Stage 7 — try extra questions before submitting to see if stage 8 triggers
    g.chat('NPC_C', '你电脑上的杀人计划书是怎么回事？')
    g.chat('NPC_C', '为什么杀人计划书里面都是同学的名字？')
    g.chat('NPC_B', '你是不是准备杀A？')

    # Extra confrontational questions — might trigger stage 8?
    g.chat('NPC_E', '我知道你杀了F，你有什么要说的？', evidences=['313', '315', '311'])
    g.chat('NPC_E', '你用C的水果刀杀的F对吧？', evidences=['315'])

    log(f'[v34] case1 stage={g.stage} calls={g.calls}')
    result = g.submit(
        murderer='NPC_E',
...
def solve_case(g: Game, case_index: int) -> bool:
    ...
    log(f'[v34] case={case_index} npcs={sorted_npcs}')
    g.stage = 0
    g.calls = 0
    if sorted_npcs == CASE0_NPCS:
        solve_case0(g)
    elif sorted_npcs == CASE1_NPCS:
        solve_case1_alternate(g)
    else:
