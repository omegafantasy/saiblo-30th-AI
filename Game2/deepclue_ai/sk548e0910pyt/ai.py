...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        if witness:
            remember(witness, g.chat(witness, '重点追问你亲眼看到的生物馆跑出者、1919黑车、保安奇怪网站和李海天案：谁掌握尸检报告，蓝色背包海豚挂件是谁的？', yuan_ids))
            yuan_ids = yuan_ids_now()
        if teacher:
            remember(teacher, g.chat(teacher, '结合袁樱瞳手机、课程展示投票纸和李海天案，说明投票异常是否只是导火索，谁最怕袁樱瞳周五后揭露真相？', yuan_ids))
            yuan_ids = yuan_ids_now()
        if competitor:
            remember(competitor, g.chat(competitor, '凌晨1点手机照片、lo裙栗色假发、黄色行李箱和你与袁樱瞳长相相似之间，是谁在制造死亡时间或身份混淆？', yuan_ids))
            yuan_ids = yuan_ids_now()
        if '705' in yuan_ids:
            if witness:
                remember(witness, g.chat(witness, '依据李海天尸检报告705：背刺失血、四肢被砍、蓝色背包海豚挂件，与袁樱瞳碎尸案哪些相同哪些不同？这能推出谁在说谎？', yuan_ids))
            if teacher:
                remember(teacher, g.chat(teacher, '如果705李海天尸检报告为真，袁樱瞳手机703和投票704之外还缺哪件物证才能进入下一阶段？请直接说证据来源。', yuan_ids))
        else:
            ask_all('如果李海天尸检报告、蓝色背包海豚挂件或生物馆监控还没有公开，请说明谁持有、如何取得、它能证明谁在说谎。', yuan_ids)
        yuan_suspect = yuan_candidate_from_replies(yuan_replies, npcs, g.marks() or marks)
        g.answer(
            murderer=yuan_suspect,
            motivation=f'{yuan_suspect}因袁樱瞳掌握出国名额投票异常、手机照片、李海天旧案或生物馆线索，担心真相暴露而杀人灭口。',
            method=f'{yuan_suspect}利用袁樱瞳手机、凌晨1点尸体照片、lo裙栗色假发、黄色行李箱、1919黑车、生物馆传闻和保安网站制造死亡时间、身份和尸块来源混淆；李海天尸检报告中的背刺失血、四肢被砍和海豚挂件说明两案可能有共同处理链条。',
        )
        return
    else:
        method = '未知'
