...
def response_text(resp: dict[str, Any]) -> str:
    if not isinstance(resp, dict):
        return ''
    for key in ('reply', 'content', 'message', 'text', 'npc_reply'):
        value = resp.get(key)
    ...
    if '扑克公馆' in text:
        ...
        for npc in npcs:
            if current_marks.get(npc) is False:
                suspect = cn_name(npc)
                break
        probe_npcs = [npc for npc in npcs if current_marks.get(npc) is True] or npcs
        for npc in probe_npcs:
            resp = g.chat(npc, '你觉得谁在说谎？为什么？')
            reply = response_text(resp)
            if any(key in reply for key in ('49人', '48票', '笔迹', '24票', '24:23', '二十四', '多出')):
                g.evidences()
                break
        refreshed_npcs = g.npcs() or npcs
        refreshed_marks = g.marks() or current_marks
        for npc in refreshed_npcs:
            if refreshed_marks.get(npc) is False:
                suspect = cn_name(npc)
                break
