...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
                    ...
                    if {'401', '402'}.issubset(set(monitor_ids)):
                        ...
                        if post601_set & {'601', '602', '603', '604'}:
                            ...
                            if '606' in post601_set:
                                for npc_id in post601_targets[:4]:
                                    ask_post601(npc_id, '606三人照片已经出现。继续追605/607/608：POKER纹身组织名册、Joker人口贩卖名单、沈知遥和张子韩关系、林渝植亲子鉴定、谁杀Joker以及最终可提交证据。')
                        late_ids = [
                            str(ev.get('id'))
                            for ev in poker_evidences
                            if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404', '405', '501', '502'}
        ...
        ask_all('先按物证06/07方向查，不等705：李海天随身U盘是否从失物招领出现；谁愿意给联系方式；谁说拿到那个不认识的运动少女联系方式就会交换杀手秘密。若能给706/707/708请直接交出。')
        ...
        def add_contact_target(npc_id: str) -> None:
            if npc_id and npc_id not in contact_targets:
                contact_targets.append(npc_id)

        for npc_id, reply in yuan_replies.items():
            if re.search(r'联系方式|运动少女|直说|要联系方式|保研|夜跑|生物馆', reply):
                add_contact_target(npc_id)
        for npc_id in (chu_contact_id, forensic_target_id if 'forensic_target_id' in locals() else '', teacher_id if 'teacher_id' in locals() else ''):
            add_contact_target(npc_id)
        for npc_id in false_ids + current_npcs:
            add_contact_target(npc_id)

        if '707' not in set(yuan_ids):
            for contact_id in contact_targets[:5]:
                resp = chat_visible_or_probe(g, contact_id, g.npcs() or npcs, '我不是问U盘，也不是问投票。刚才你提到“联系方式”或运动少女线索；许清和说如果拿到那个不认识的运动少女联系方式，就会交换关于杀手的秘密。请直接给联系方式；如果对应物证07/707，请现在交出。', yuan_ids)
        ...
        if '707' in set(yuan_ids) and '708' not in set(yuan_ids):
            ...
            early_exchange_targets: list[str] = []
            for pattern in (r'用于与([一-龥]{1,4})交换', r'([一-龥]{1,4})曾表示'):
                for exchange_name in re.findall(pattern, ev707_text):
                    for npc_id in global_name_ids(exchange_name, g.npcs() or npcs):
                        if npc_id and npc_id not in early_exchange_targets:
                            early_exchange_targets.append(npc_id)
            for npc_id in story_target_ids(ev707_text, g.npcs() or npcs, max_ids=6):
                if npc_id and npc_id not in early_exchange_targets:
                    early_exchange_targets.append(npc_id)
            for exchange_id in early_exchange_targets[:4]:
                resp = chat_visible_or_probe(g, exchange_id, g.npcs() or npcs, '707联系方式已经拿到。按你提出的交换条件，现在请兑现关于杀手的秘密：你为什么提到运动少女，真正杀手是谁，生物馆、世纪林尸块、1919黑车、保安网站和袁樱瞳手机凌晨照片之间缺哪份实物？如果对应708请直接交出。', yuan_ids)
        ...
        if '707' in set(yuan_ids) and '708' not in set(yuan_ids):
            ev707 = next((ev for ev in yuan_evidences if str(ev.get('id')) == '707'), None)
            ev707_text = str(ev707.get('name', '')) + '\n' + str(ev707.get('content', '')) if isinstance(ev707, dict) else ''
            xu_secret_id = xu_secret_id or id_for_name_any('许清和', g.npcs() or npcs)
            chu_contact_id = chu_contact_id or id_for_name_any('楚戎臻', g.npcs() or npcs)
            ...
            for npc_id in story_target_ids(ev707_text + '\n' + '\n'.join(yuan_replies.values()), g.npcs() or npcs, max_ids=8):
                if npc_id and npc_id not in post707_targets:
                    post707_targets.append(npc_id)
            for npc_id in (xu_secret_id, chu_contact_id, guard_id if 'guard_id' in locals() else '', runner_id if 'runner_id' in locals() else ''):
                if npc_id and npc_id not in post707_targets:
                    post707_targets.append(npc_id)
            for npc_id in locals().get('contact_targets', [])[:4]:
                if npc_id and npc_id not in post707_targets:
                    post707_targets.append(npc_id)
            for source_id in post707_targets[:7]:
                resp = chat_visible_or_probe(g, source_id, g.npcs() or npcs, '707联系方式已经拿到，按物证07内容里的交换条件继续追708：把联系方式交给提出条件的人后，他关于杀手的秘密是什么？谁实际杀了袁樱瞳，生物馆、世纪林尸块、1919黑车、保安网站和凌晨照片之间的最终实物证据在哪里？', yuan_ids)
                yuan_replies[source_id] = yuan_replies.get(source_id, '') + '\n' + response_text(resp)
                yuan_evidences = g.evidences()
                yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
                if '708' in set(yuan_ids):
