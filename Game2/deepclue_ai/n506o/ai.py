...
def solve_rose_direct(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    g.answer(
        murderer=murderer,
        motivation=f'{murderer}家族生意和资金处境紧张，想通过接近并独占{banker}获得地位与财力；因误以为Rose和{banker}关系亲近、会阻碍自己，出于嫉妒和利益动机杀害Rose。',
...
def solve_rose(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    g.answer(
        murderer=cui_name,
        motivation=f'{cui_name}家族生意和资金处境紧张，想通过接近并独占{deng_name}获得地位与财力；因误以为Rose和{deng_name}关系亲近、会阻碍自己，出于嫉妒和利益动机杀害Rose。',
