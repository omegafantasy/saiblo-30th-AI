...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        # Yuan 706-first ceiling probe: test whether the U-disk/official-source lane is a controllable gateway.
        ask_all('袁樱瞳案先不要追联系方式交换。只查物证06/706入口：李海天随身U盘从哪里出现、失物招领是谁登记、电子系保研名单是谁导出、不雅视频照片是谁复制、保卫处或教务系统哪份日志能登记为706？')
        ask_all('继续只问706官方源：李海天尸检报告、蓝色背包海豚挂件、世纪林尸块DNA、生物馆监控、1919黑车、保安奇怪网站、电子系名单和袁樱瞳手机原图元数据之间，哪一项能直接给物证06/706？请说持有人和证据名。')
        ...
        ask_all('先按物证06方向查，不等705也不跳到联系方式：李海天随身U盘是否从失物招领出现，U盘登记人、电子系名单、不雅视频照片、手机清空、保研名额和王科瑾未保研之间怎么连？若能给706请直接交出。')
