#!/usr/bin/env python3
"""Game2 DeepClue AI v27 — Evidence-presentation + exact murderer name fix.

v26 breakthrough: evidence presentation ('313','315','311') triggered stage 4->5 reliably.
v26 result: Case 0 stage 6, Case 1 stage 7, score=2220.

KEY FIX in v27: murderer answer for Case 1 changed from 'NPC_E' to 'E'.
Analysis: v26 trace vs admin trace were IDENTICAL through step 39 (both 38 testimonies,
same marks). Only the answer differed: admin says 'E', we said 'NPC_E'. Stage 8 requires
the in-story character name 'E', not the NPC ID 'NPC_E'.
...
def solve_case0(g: Game) -> None:
    ...
    log(f'[v27] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1(g: Game) -> None:
    ...
    log(f'[v27] case1 stage={g.stage} calls={g.calls}')
    result = g.submit(
        murderer='E',
...
def solve_case(g: Game, case_index: int) -> bool:
    ...
    log(f'[v27] case={case_index} npcs={sorted_npcs}')
    g.stage = 0
    g.calls = 0
...
def main() -> int:
    ...
    while True:
        try:
            ...
            log(f'[v27] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
