...
class Game:
    ...
    def req(self, action: str, **kwargs: Any) -> Any:
        try:
            ...
            log(f'[n556y8] request failed action={action}: {exc}')
            return {}
    ...
    def chat(self, npc: str, question: str, evidences: list[str] | None = None) -> dict[str, Any]:
        ...
        for attempt in range(3):
            ...
            if not (isinstance(resp, dict) and resp.get('error')):
                break
            log(f'[n556y8] chat error retry={attempt} npc={npc} q={compact(question, 36)} err={compact(resp.get("error"), 80)}')
        ...
        if new_stage > self.stage:
            log(f'[n556y8] stage {self.stage}->{new_stage} npc={npc} q={compact(question, 36)}')
    ...
    def retry_trigger(self, trigger_qs: list[tuple[Any, ...]], max_retries: int = 2) -> bool:
        ...
        for attempt in range(max_retries):
            for item in trigger_qs:
                ...
                log(f'[n556y8] retry a{attempt} npc={npc} q={compact(question, 40)}')
...
def solve_rose_direct(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    if not banker:
        banker = '银行家'
    log(f'[n556y8] rose direct murderer_id={murderer_id} murderer={murderer} banker={banker}')
    g.answer(
        murderer=murderer,
...
def solve_rose(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    if set(roles) != {'xiao', 'bai', 'deng', 'fan', 'ye', 'cui'}:
        log(f'[n556y8] rose role mapping incomplete roles={roles}')
    ...
    log(
        f'[n556y8] rose script xiao={xiao_name}/{xiao} bai={bai_name}/{bai} '
    ...
    log(f'[n556y8] rose final stage={g.stage} calls={g.calls}')
    g.answer(
        murderer=cui_name,
...
def solve_z_script(g: Game, npcs: list[str], evidences: list[dict[str, Any]]) -> None:
    ...
    if set(roles) != {'A', 'B', 'C', 'D', 'E'}:
        log(f'[n556y8] z role mapping incomplete roles={roles}')
    ...
    d_name, d = roles['D']
    e_name, e = roles['E']
    log(f'[n556y8] z script A={a_name}/{a} B={b_name}/{b} C={c_name}/{c} D={d_name}/{d} E={e_name}/{e}')
    g.stage = 0
    g.calls = 0
    ...
    log(f'[n556y8] z final stage={g.stage} calls={g.calls}')
    g.answer(
        murderer=e_name,
...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        ask_all('先只说袁樱瞳手机、凌晨1点尸体照片、lo裙栗色假发、黄色行李箱、课程投票23比24、多出异笔迹正字、出国名额竞争这些你能确认的事实。')
        yuan_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}]
        ask_all('结合手机照片和投票异常，如果你知道凶手或关键隐瞒者，请直接给出名字、动机、作案过程和证据链。', yuan_ids)
        ask_all('再核实生物馆、世纪林、张壹传闻、林晚舟跑出、生物馆尸块、1919黑车和保安奇怪网站如何影响凶手判断。', yuan_ids)
        g.answer(murderer='无名氏', motivation='无', method='无')
        return
    else:
        method = '未知'
    log(f'[n556y8] unknown hint={compact(hint, 50)} suspect={suspect}')
...
def solve_case(g: Game, case_idx: int) -> bool:
    ...
    text = all_text(hint, evidences)
    log(f'[n556y8] case={case_idx} npcs={sorted(npcs)} marks={marks} hint={compact(hint, 60)}')
    if 'Rose' in text:
...
def main() -> int:
    ...
    for case_idx in range(6):
        try:
            ...
            log(f'[n556y8] fatal case={case_idx}: {exc}')
            try:
                g.answer('', '未知', '未知')
            except Exception:
                pass
