                        ...
                        if post601_set & {'601', '602', '603', '604'}:
                            ...
                            if '606' in post601_set:
                                ...
                                def follow_verbal_605(npc_id: str, resp: dict[str, Any]) -> None:
                                    ...
                                    action_hit = bool(re.search(r'地下室|档案室|钥匙|唯一保管人|保管人|绝密卷宗|刑警队长权限|内部卷宗|原件|自愿交出|顾云舒', reply_text or ''))
                                    if not (verbal_hit or challenge_hit or action_hit):
                                        return
                                    if action_hit:
                                        question = (
                                            '我现在按你刚才给出的具体位置和权限执行，不再重复问编号：'
                                            '如果605是地下室档案室钥匙，就用钥匙打开档案室并取出被篡改的警方结案卷宗复印件；'
                                            '如果607/608在唯一保管人或顾云舒手里，就向该保管人正式申请原件；'
                                            '如果需要刑警队长权限，就由景观调取内部绝密卷宗。'
                                            '请把当前能公开登记的605、607或608原件交出来，或给出唯一保管人的姓名。'
                                        )
