#!/usr/bin/env python3
"""Game2 DeepClue AI v26 — Evidence-presentation strategy (mirrors admin exactly).

KEY FINDING: Admin presents specific evidence IDs when asking key NPC questions.
This causes NPC LLM responses to include specific details that trigger testimony unlocks.
...
def solve_case0(g: Game) -> None:
    ...
    log(f'[v26] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1(g: Game) -> None:
    ...
    log(f'[v26] case1 stage={g.stage} calls={g.calls}')
    result = g.submit(
        murderer='NPC_E',
...
def solve_case(g: Game, case_index: int) -> bool:
    ...
    log(f'[v26] case={case_index} npcs={sorted_npcs}')
    g.stage = 0
    g.calls = 0
...
def main() -> int:
    ...
    while True:
        try:
            ...
            log(f'[v26] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
