#!/usr/bin/env python3
"""Game2 DeepClue AI n507e.

Diagnostic copy of n506e. It keeps behavior unchanged and only records compact
response/evidence summaries for the new post-update cases.
"""
...
def compact_payload(obj: Any, limit: int = 900) -> str:
    def trim(value: Any) -> Any:
        if isinstance(value, str):
            return compact(value, 180)
        if isinstance(value, list):
            return [trim(x) for x in value[:8]]
        if isinstance(value, dict):
            ...
            if out:
                return out
            return {str(k): trim(v) for k, v in list(value.items())[:8]}
        return value if isinstance(value, (int, float, bool)) or value is None else str(value)

    try:
        return compact(json.dumps(trim(obj), ensure_ascii=False), limit)
    except Exception:
        return compact(obj, limit)
...
class Game:
    ...
    def answer(self, murderer: str, motivation: str, method: str) -> dict[str, Any]:
        ...
        if isinstance(resp, dict):
            log(f'[n507e] answer resp={compact_payload(resp)}')
...
def solve_unknown(g: Game, npcs: list[str], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    for ev in evidences:
        log(
            f'[n507e] evidence id={ev.get("id")} name={compact(ev.get("name"), 60)} '
            f'content={compact(ev.get("content"), 220)}'
        )
    if '扑克公馆' in text:
