#!/usr/bin/env python3
"""Game2 DeepClue AI v42 — post-answer hidden-stage probe.

Purpose:
- Preserve the proven v37 scripted path exactly through both cases
- After the case1 `answer`, replay the two hidden tail chats seen in old
  admin/server logs
- Verify whether a normal account can also reach a post-answer stage that
  triggers the known `KeyError: '8'` path
...
class Game:
    ...
    def post_answer_probe(self, npc: str, question: str, evidences: list[str] | None = None) -> None:
        evid = evidences if evidences is not None else []
        self.sdk.request('chat', npc=npc, question=question, evidences=evid)
...
def solve_case0(g: Game) -> None:
    ...
    log(f'[v42] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1(g: Game) -> None:
    ...
    log(f'[v42] case1 stage={g.stage} calls={g.calls}')
    result = g.submit(
        murderer='E',
    ...
    log(f'[v42] case1 submit_keys={sorted(result.keys()) if isinstance(result, dict) else []}')

    # Old admin/server logs show these two post-answer chats after the hidden
    # stage-8 transition. Replaying them lets us see whether a normal account
    # can hit the same backend path.
    for npc, question in [
        ('NPC_C', '你是色盲？'),
        ('NPC_D', 'F的QQ里面还有什么信息？'),
    ]:
        try:
            g.post_answer_probe(npc, question)
        except EOFError:
            log(f'[v42] post-answer eof [{npc}] {question}')
            break
        except Exception as exc:
            log(f'[v42] post-answer exc [{npc}] {question} {type(exc).__name__}: {exc}')
...
def solve_case(g: Game, case_index: int) -> bool:
    ...
    log(f'[v42] case={case_index} npcs={sorted_npcs}')
    g.stage = 0
    g.calls = 0
...
def main() -> int:
    ...
    while True:
        try:
            ...
            log(f'[v42] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
