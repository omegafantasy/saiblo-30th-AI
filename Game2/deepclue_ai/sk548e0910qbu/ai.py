...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        if '706' in set(yuan_ids):
            ...
            for pattern in (
                r'([一-龥]{2,4})今天在失物招领处找到',
                r'([一-龥]{2,4})[^。；\n]{0,12}找到[^。；\n]{0,8}U盘',
                r'([一-龥]{2,4})[^。；\n]{0,12}李海天随身U盘',
            ):
                for holder_name in re.findall(pattern, ev706_text):
                    for npc_id in global_name_ids(holder_name, g.npcs() or npcs):
                        add_post706_target(npc_id)
            add_post706_target(teacher_id)
            add_post706_target(forensic_target_id)

            for holder_id in post706_targets[:2]:
                resp = chat_visible_or_probe(
                    g,
                    holder_id,
                    g.npcs() or npcs,
                    '706李海天U盘已经出现。不要先泛问708，先问持有人链：这个U盘是谁放到失物招领处、谁复制或删改过、为什么电子系保研名单里袁樱瞳和张壹成功而林晚舟失败；李海天侵犯女生的视频照片是谁拍摄/保存/威胁过，和生物馆、1919黑车、保安网站、杀手秘密怎么连接？若你能据此给708或下一份记忆/雇主证据，请直接交出。',
                    yuan_ids,
                )
                yuan_replies[holder_id] = yuan_replies.get(holder_id, '') + '\n' + response_text(resp)
                yuan_evidences = g.evidences()
                yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
                if '708' in set(yuan_ids):
                    break
