...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        def ordered_npcs() -> list[str]:
            ...
            order = []
            if len(marked) > 1:
                order.append(marked[1])
            if marked:
                order.append(marked[0])
            order.extend(unmarked)
            order.extend(ynpc for ynpc in ynpcs if ynpc not in set(order))
            return order
        def ask_all(question: str, evidences_arg: list[str] | None = None) -> None:
            for ynpc in ordered_npcs():
