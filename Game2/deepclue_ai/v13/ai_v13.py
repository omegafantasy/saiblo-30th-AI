...
def solve_case1_campus(sdk: SDK, kb: KnowledgeBase) -> None:
    ...
    Focuses on NPC_E, NPC_D, NPC_B with short, direct questions.
    Avoids drifting toward Z/U盘/校医院/凌晨归还 topics.
    """
    # --- Phase 1: Generic opening for all 5 NPCs ---
    if kb.stage >= 8:
        log('[game2-v13] case1 skip phase1: stage >= 8')
        return
    ...
    # --- Phase 2: Focus on NPC_E ---
    if kb.stage >= 8:
        log('[game2-v13] case1 skip phase2: stage >= 8')
        return
    phase2: list[dict[str, Any]] = [
        {'npc': 'NPC_E', 'question': 'F是谁？她怎么了？'},
        {'npc': 'NPC_E', 'question': '你和F是什么关系？'},
        {'npc': 'NPC_E', 'question': '你听说过什么传言？'},
        {'npc': 'NPC_E', 'question': 'QQ或者微信上有什么线索？'},
    ]
    run_scripted_phase(sdk, kb, phase2)

    # --- Phase 3: Ask NPC_D and NPC_B ---
    if kb.stage >= 8:
        log('[game2-v13] case1 skip phase3: stage >= 8')
        return
    phase3: list[dict[str, Any]] = [
        {'npc': 'NPC_D', 'question': '你对NPC_E怎么看？'},
        {'npc': 'NPC_D', 'question': 'F失联这件事你怎么看？'},
        {'npc': 'NPC_D', 'question': '你有什么可疑的发现？'},
        {'npc': 'NPC_B', 'question': '你对NPC_E怎么看？'},
        {'npc': 'NPC_B', 'question': 'F失联这件事你怎么看？'},
        {'npc': 'NPC_B', 'question': '你有什么可疑的发现？'},
    ]
    run_scripted_phase(sdk, kb, phase3)

    # --- Phase 4: Ask NPC_A and NPC_C ---
    if kb.stage >= 8:
        log('[game2-v13] case1 skip phase4: stage >= 8')
        return
    phase4: list[dict[str, Any]] = [
        {'npc': 'NPC_A', 'question': '你觉得谁最可疑？'},
        {'npc': 'NPC_A', 'question': '有没有什么你一直没说的事？'},
        {'npc': 'NPC_C', 'question': '你觉得谁最可疑？'},
        {'npc': 'NPC_C', 'question': '有没有什么你一直没说的事？'},
    ]
    run_scripted_phase(sdk, kb, phase4)

    # --- Optional targeted sweep if marks still active and stage < 8 ---
    if kb.stage >= 8:
        log('[game2-v13] case1 skip targeted_sweep: stage >= 8')
        return
...
def solve_case(sdk: SDK, case_index: int) -> bool:
    ...
    if not kb.background and not kb.npcs:
        return False
    log(f'[game2-v13] case={case_index} stage={kb.stage} npcs={len(kb.npcs)}')
    ...
    if is_case0_rose(kb):
        ...
        for _ in range(2):
            if kb.stage >= 8:
                log('[game2-v13] generic: stage >= 8, stop investigation')
                break
            current_marks = marks(sdk)
    ...
    log(f'[game2-v13] answer case={case_index} murderer={answer["murderer"]!r}')
    resp = sdk.request('answer', **answer)
    log(f'[game2-v13] answer_result case={case_index} resp={resp}')
    return True
...
def main() -> int:
    ...
    log(f'[game2-v13] welcome={welcome}')
    case_index = 0
    while True:
        try:
            ...
            log(f'[game2-v13] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
