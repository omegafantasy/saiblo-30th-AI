#!/usr/bin/env python3
"""Game2 DeepClue AI v38 — narrow stage-7 probe on top of v37.

Purpose:
- Keep the proven v37 path unchanged through stage 7
- Add only two evidence-backed follow-up questions at case1 stage 7
- Probe whether the remaining NPC_C / NPC_D marks react to tighter, more
  accusation-like phrasing when concrete evidence is attached
- Log a tiny slice of the raw chat response once, to verify whether the
  runtime returns any directly usable reply fields to the agent

This version deliberately does not touch:
- case ordering
- murderer alias
...
class Game:
    ...
    def retry_trigger(self, trigger_qs: list[tuple[Any, ...]], max_retries: int = 3) -> bool:
        ...
        for attempt in range(max_retries):
            for item in trigger_qs:
                ...
                before = self.stage
                self.chat(npc, q, evidences=evid)
                if self.stage > before:
                    return True
        return self.stage > target_stage

    def maybe_log_probe_response(self, tag: str, resp: dict[str, Any]) -> None:
        if self.logged_probe_resp:
            return
        self.logged_probe_resp = True
        keys = sorted(resp.keys())
        preview = {k: type(v).__name__ for k, v in resp.items()}
        log(f'[v38] probe_resp {tag} keys={keys} types={preview}')
...
def solve_case0(g: Game) -> None:
    ...
    log(f'[v38] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1(g: Game) -> None:
    ...
    # Narrow probe: only two extra, evidence-backed questions on the remaining marked NPCs.
    if g.stage >= 7:
        resp_d = g.chat('NPC_D', '你看到E藏在小树林里，是想埋伏F吗？', evidences=['313'])
        g.maybe_log_probe_response('NPC_D_stage7_probe', resp_d)
        resp_c = g.chat('NPC_C', '有人拿你的水果刀按你的小说手法杀了F，还改了你的文档嫁祸你，对吗？', evidences=['315'])
        g.maybe_log_probe_response('NPC_C_stage7_probe', resp_c)

    log(f'[v38] case1 stage={g.stage} calls={g.calls}')
    result = g.submit(
        murderer='E',
...
def solve_case(g: Game, case_index: int) -> bool:
    ...
    log(f'[v38] case={case_index} npcs={sorted_npcs}')
    g.stage = 0
    g.calls = 0
    g.logged_probe_resp = False
...
def main() -> None:
    ...
    while True:
        try:
            if not solve_case(game, case_index):
                break
            case_index += 1
        except EOFError:
            break
        except Exception as exc:
            log(f'[v38] fatal error: {type(exc).__name__}: {exc}')
            break
