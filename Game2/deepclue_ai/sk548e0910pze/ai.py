                ...
                if reception_id and g.stage >= 3:
                    ...
                    if {'401', '402'}.issubset(set(monitor_ids)):
                        ...
                        if password_id:
                            ...
                            g.chat(info_id, '推断过程是：402显示7:30有人提前入馆、8:20离开，8:50又有梅花5到达；401显示12:00餐厅还有梅花5活动。你说衣帽间还有更多破绽，请直接给出密码、破绽和下一阶段证据。', monitor_ids)
                        poker_evidences = g.evidences()
                        ...
                        if isinstance(ev404, dict):
                            ...
                            g.chat(target_id, '结合404车牌号京F·A7590、7:20驶过大门的车辆、死者房间窗户外就是后院停车位置、401/402监控和衣帽间移尸破绽，直接说明车是谁开的、车停在窗边做了什么、Joker在哪里被杀、尸体如何经窗户或后院移到衣帽间，并交出车辆/后备箱/行车记录的下一份证据。', branch_ids)
                            poker_evidences = g.evidences()
                        ...
                        if isinstance(ev501, dict):
                            ...
                            g.chat(target_id, '结合501不明巨额转账记录、看诊后三天收到50万元匿名转账、于书华身份、女儿下落、勒索心理医生或医生、Joker人口贩卖集团幕后身份、林渝植三年前被拐卖和死后戴面具伪装，请直接说明谁用女儿下落勒索了谁、匿名转账是谁打的、看诊记录隐藏了什么、这笔钱和Joker/真正梅花5有什么关系，并交出聊天记录、病历或转账来源的下一份证据。', branch_ids)
                            g.evidences()
