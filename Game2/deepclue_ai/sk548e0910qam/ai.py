...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        ask_all('先按物证06/07方向查，不等705：李海天随身U盘是否从失物招领出现；楚戎臻是否愿意给联系方式；许清和说拿到那个不认识的运动少女联系方式就会交换杀手秘密。若能给706/707/708请直接交出。')
        ...
        if '707' not in set(yuan_ids):
            for contact_id in contact_targets[:5]:
                ...
                if '707' in set(yuan_ids):
                    break
        if xu_secret_id and '707' in set(yuan_ids) and '708' not in set(yuan_ids):
            resp = chat_visible_or_probe(g, xu_secret_id, g.npcs() or npcs, '707已经拿到楚戎臻的联系方式。按你之前的交换条件，现在请兑现关于杀手的秘密：你上周六晚为什么从生物馆慌张跑出，真正杀手是谁，1919黑车、世纪林尸块、保安网站和袁樱瞳手机凌晨照片之间缺哪份实物？如果对应708请直接交出。', yuan_ids)
            yuan_replies[xu_secret_id] = yuan_replies.get(xu_secret_id, '') + '\n' + response_text(resp)
            yuan_evidences = g.evidences()
            yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
        ...
        if '707' in set(yuan_ids) and '708' not in set(yuan_ids):
            ...
            for source_id in post707_targets[:7]:
                resp = chat_visible_or_probe(g, source_id, g.npcs() or npcs, '707是楚戎臻的联系方式，按许清和提出的交换条件继续追708：把联系方式交给许清和后，他关于杀手的秘密是什么？谁实际杀了袁樱瞳，生物馆、世纪林尸块、1919黑车、保安网站和凌晨照片之间的最终实物证据在哪里？', yuan_ids)
                yuan_replies[source_id] = yuan_replies.get(source_id, '') + '\n' + response_text(resp)
                yuan_evidences = g.evidences()
                yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
