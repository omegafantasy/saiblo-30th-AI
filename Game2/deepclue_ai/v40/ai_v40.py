#!/usr/bin/env python3
"""Game2 DeepClue AI v40 — submit-frame diagnostics on top of v37.

Purpose:
- Keep the proven v37 path and final answer unchanged
- Add only minimal logging around the case1 stage-7 boundary and submit call
- Probe whether submit responses expose any hidden fields that correlate with
  the admin-only stage-8 jump

This version deliberately avoids changing:
- question ordering
- final answer text
- startup handshake / protocol flow
"""
...
def summarize_dict(resp: dict[str, Any]) -> str:
...
class Game:
    ...
    def chat_raw(self, npc: str, question: str, evidences: list[str] | None = None) -> dict[str, Any]:
        evid = evidences if evidences is not None else []
        resp = self.sdk.request('chat', npc=npc, question=question, evidences=evid)
        self.calls += 1
...
def solve_case0(g: Game) -> None:
    ...
    log(f'[v40] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1(g: Game) -> None:
    ...
    resp_stage7 = g.chat_raw('NPC_D', '你为什么装病？')
    log(f'[v40] case1 stage7_trigger {summarize_dict(resp_stage7)}')
    if g.stage < 7:
        g.retry_trigger([
            ('NPC_D', '你为什么装病？'),
            ('NPC_D', '你的脚到底有没有受伤？'),
    ...
    resp_last = g.chat_raw('NPC_B', '你是不是准备杀A？')
    log(f'[v40] case1 last_chat {summarize_dict(resp_last)}')

    log(f'[v40] case1 stage={g.stage} calls={g.calls}')
    result = g.submit(
        murderer='E',
    ...
    log(f'[v40] case1 submit_resp {summarize_dict(result)}')
    log(f'[v40] case1 result={result}')
...
def solve_case(g: Game, case_index: int) -> bool:
    ...
    log(f'[v40] case={case_index} npcs={sorted_npcs}')
    g.stage = 0
    g.calls = 0
...
def main() -> int:
    ...
    while True:
        try:
            ...
            log(f'[v40] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
