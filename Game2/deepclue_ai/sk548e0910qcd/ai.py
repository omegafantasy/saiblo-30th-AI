                ...
                if reception_id and g.stage >= 3:
                    ...
                    if {'401', '402'}.issubset(set(monitor_ids)):
                        ...
                        if not (isinstance(ev404, dict) and isinstance(ev501, dict)):
                            ...
                            def add_dual_target(npc_id: str) -> None:
                                if npc_id and npc_id not in dual_targets:
                                    dual_targets.append(npc_id)
                            ...
                            for npc_id in (
                                ...
                                add_dual_target(npc_id)
                            for dual_id in dual_targets[:4]:
                                ...
                                resp = chat_visible_or_probe(
                                    ...
                                    dual_id,
                                    follow_npcs,
                                    '401餐厅监控、402大门监控、0512衣帽间密码和移尸破绽已经满足。现在不要分成车线或转账线深挖，请按同一份刑警材料包一次登记三页：404车辆页、501匿名50万元转账页、502死者手机聊天页。若三页在不同人手里，请直接说每页唯一保管人；若你持有其中任意一页，先交出证据。',
                                    branch_ids,
                                )
                                reply_dual = response_text(resp)
                                poker_evidences = g.evidences()
                                ev404 = next((ev for ev in poker_evidences if str(ev.get('id')) == '404'), None)
                                ev501 = next((ev for ev in poker_evidences if str(ev.get('id')) == '501'), None)
                                branch_ids = [
                                ...
                                if not (isinstance(ev404, dict) and isinstance(ev501, dict)) and re.search(r'保管人|在([一-龥]{1,4})手里|去找([一-龥]{1,4})|问([一-龥]{1,4})', reply_dual or ''):
                                    for source_name in extract_story_names(reply_dual):
                                        for source_id in global_name_ids(source_name, follow_npcs):
                                            add_dual_target(source_id)
                        ...
                        if isinstance(ev404, dict):
                            ...
                            if not isinstance(ev501, dict):
                                merge_targets: list[str] = []

                                def add_merge_target(npc_id: str) -> None:
                                    if npc_id and npc_id not in merge_targets:
                                        merge_targets.append(npc_id)
                                ...
                                for npc_id in (
                                    ...
                                    add_merge_target(npc_id)
                                for merge_id in merge_targets[:4]:
                                    ...
                                    chat_visible_or_probe(
                                        ...
                                        merge_id,
                                        follow_npcs,
                                        '404车辆页已经打开，不要只沿搬尸路线问405。请把同一卷宗里的501匿名50万元转账、于书华病历、转账源账户、女儿下落勒索和林渝植失踪案并到车辆链里；如果501不在你手里，请直接说唯一保管人。',
                                        branch_ids,
                                    )
                                    poker_evidences = g.evidences()
                                    ev501 = next((ev for ev in poker_evidences if str(ev.get('id')) == '501'), ev501)
