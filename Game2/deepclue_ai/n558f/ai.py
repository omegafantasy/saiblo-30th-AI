        ...
        if info_id:
            ...
            if g.stage >= 2:
                ...
                if 'reception_id' in locals() and reception_id:
                    g.chat(reception_id, '如果聊天记录或到场表还不完整，请先补原始记录；再说谁最可能不是表面身份。', deep_ids)
                    g.evidences()
                deep_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '101', '102', '103', '104', '201', '202', '203', '204', '205'}]
                for npc in [info_id] + [x for x in deep_npcs if x != info_id]:
                    if not npc:
                        continue
                    g.chat(npc, '把邀请函、Joker聊天记录、宾客到达表、电脑浏览记录、塑料盒和缺刀按时间顺序串起来；最后只说梅花5真实身份。', deep_ids)
                    if g.stage >= 4:
                        break
