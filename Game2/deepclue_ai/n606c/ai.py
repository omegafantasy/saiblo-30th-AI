                ...
                if 'poker_after_ids' in locals():
                    ...
                    if {'601', '602', '603', '604', '606'} & set(n604_ids):
                        for npc_id in n604_targets[:14]:
                            n604_ask(npc_id, '从“时机未到”的缺口继续。601-604已经证明失踪少女和花纹村组织，请把现场已掌握但未交出的605/606交出来：三人照片、左臂POKER纹身、组织名册、红桃Q/刘瑄、密码来源、谁把Joker引入衣帽间。')
                        n604_ids = [
                            str(ev.get('id')) for ev in g.evidences()
                            if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404', '405', '501', '502', '503', '504', '601', '602', '603', '604', '605', '606', '607', '608'}
                        ]
                        for npc_id in n604_targets[:14]:
                            n604_ask(npc_id, '不要再说等时机。条件已经满足：501转账、502聊天、503邀请函、504随身物、601-604旧案都在。请直接交出605/606/607/608，或说明唯一还缺的条件和持有人。')
                    elif {'501', '502', '503', '504'} & set(n604_ids):
                        for npc_id in n604_targets[:10]:
                            n604_ask(npc_id, '501/502打开后不要分散问DNA。只查时机条件：还缺哪一项才能公开三人照片、POKER纹身、组织名册、红桃Q/刘瑄、最终凶手和605/606/607/608。')
