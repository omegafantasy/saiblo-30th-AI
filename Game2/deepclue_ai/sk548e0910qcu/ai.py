            ...
            if g.stage >= 2:
                ...
                if reception_id and g.stage >= 3:
                    ...
                    if {'401', '402'}.issubset(set(monitor_ids)):
                        ...
                        if not (isinstance(ev404, dict) and isinstance(ev501, dict)):
                            early_dual_targets: list[str] = []

                            def add_early_dual(npc_id: str) -> None:
                                if npc_id and npc_id not in early_dual_targets:
                                    early_dual_targets.append(npc_id)

                            for npc_id in (info_id, reception_id, true_club5_id):
                                add_early_dual(npc_id)
                            for dual_id in early_dual_targets[:2]:
                                if isinstance(ev404, dict) and isinstance(ev501, dict):
                                    break
                                dual_resp = chat_visible_or_probe(
                                    g,
                                    dual_id,
                                    follow_npcs,
                                    '401餐厅监控、402大门监控、0512衣帽间密码和移尸破绽已经满足。不要再分车线或转账线深挖，请按同一份材料包一次登记404车辆页、501匿名50万元转账页和502死者手机页；若三页分散，请直接说唯一保管人，不要讲故事。',
                                    branch_ids,
                                )
                                poker_evidences = g.evidences()
                                ev404 = next((ev for ev in poker_evidences if str(ev.get('id')) == '404'), None)
                                ...
                                if re.search(r'保管人|唯一保管人|在([一-龥]{1,4})手里|去找([一-龥]{1,4})|问([一-龥]{1,4})', response_text(dual_resp) or ''):
                                    for name in extract_story_names(response_text(dual_resp)):
                            ...
                            if '505' in post601_set and not ({'606', '607', '608'} & post601_set):
                                ...
                                for npc_id in post601_targets[:4]:
                                    ...
                                    if post601_set & {'606', '607', '608'}:
                                        break
                            if '606' in post601_set and not ({'605', '607', '608'} & post601_set):
                                ...
                                def follow_verbal_605(npc_id: str, resp: dict[str, Any]) -> None:
                                    ...
                                    if post606_set & {'605', '607', '608'}:
                                        return
                                    if re.search(r'拿去吧|给你|交给你', reply_text or '') and re.search(r'605', reply_text or '') and re.search(r'607|608|结案卷宗|核心名册', reply_text or ''):
                                        chat_visible_or_probe(
                                            g,
                                            npc_id,
                                            follow_npcs,
                                            '你刚才明确说“拿去吧/给你”，而且点名了警方结案卷宗副本、605、607、608线索。请不要再口头说明，按物证登记动作把刚才要给我的材料交出来：605关键账本夹页、607警方内部卷宗副本、608花纹村核心名册，至少登记其中一项的证据名和内容。',
                                            post606_ids,
                                        )
                                        post606_ids, post606_set = refresh_post601()
                                        if post606_set & {'605', '607', '608'}:
                                            return
                                    for match in re.finditer(r'605\s*是([一-龥]{1,4})给我的密码|([一-龥]{1,4})给我的密码|([一-龥]{1,4})给你的密码|([一-龥]{1,4})给了?你(?:的)?密码', reply_text or ''):
                                        password_name = next((x for x in match.groups() if x), '')
                                        for password_id in global_name_ids(password_name, follow_npcs):
                                            chat_visible_or_probe(
                                                g,
                                                password_id,
                                                follow_npcs,
                                                '你刚才被指出给过关键密码。现在不要谈组织历史，请直接说明这个密码除了打开房间还有什么含义：它是否能解锁死者手机云端、暗格加密硬盘、关键账本夹页、警方卷宗或花纹村核心名册；如果能，请把605/607/608对应材料登记出来。',
                                                post606_ids,
                                            )
                                            post606_ids, post606_set = refresh_post601()
                                            if post606_set & {'605', '607', '608'}:
                                                return
                                    if re.search(r'密码[^。；\n]{0,24}别的含义|除了打开那个房间', reply_text or ''):
                                        for name in extract_story_names(reply_text):
                                            for password_id in global_name_ids(name, follow_npcs):
                                                chat_visible_or_probe(
                                                    g,
                                                    password_id,
                                                    follow_npcs,
                                                    '刚才有人问你给的密码除了打开房间还有没有别的含义。请直接回答：这个密码是否也是暗格加密硬盘、死者手机云端、605关键账本夹页、607内部卷宗或608核心名册的解锁密码？如果是，请现在交出对应材料。',
                                                    post606_ids,
                                                )
                                                post606_ids, post606_set = refresh_post601()
                                                if post606_set & {'605', '607', '608'}:
                                                    return
                                    if re.search(r'警方内部卷宗|核心名册|只有我能调取|等结案后', reply_text or ''):
                                        chat_visible_or_probe(
                                            g,
                                            npc_id,
                                            follow_npcs,
                                            '你刚才已经说607/608是警方内部卷宗和花纹村核心名册，且只有你能调取。现在不要等结案后，按当前侦查授权先公开摘要：607/608的证据名、保管地点、Joker周克名单、花纹村核心成员、死者手机云端和暗格账本中能证明谁杀Joker的页码是哪一项？',
                                            post606_ids,
                                        )
                                        post606_ids, post606_set = refresh_post601()
                                        if post606_set & {'605', '607', '608'}:
                                            return
                                    verbal_hit = bool(re.search(r'605[^。\n]{0,24}(?:成立|公开摘要|摘要|登记|编号|密码)|凶手是[^。\n]{1,12}Joker|Joker[^。\n]{0,20}衣帽间[^。\n]{0,20}遇害', reply_text or ''))
                ...
                if '708' not in set(yuan_ids):
                    for exchange_id in exchange_targets[:3]:
                        ...
                        if '708' in set(yuan_ids):
                            break
            if '708' not in set(yuan_ids):
                confession_targets: list[str] = []

                def add_confession_target(npc_id: str) -> None:
                    if npc_id and npc_id not in confession_targets:
                        confession_targets.append(npc_id)

                for npc_id in exchange_targets:
                    add_confession_target(npc_id)
                if contact_name:
                    for npc_id in global_name_ids(contact_name, g.npcs() or npcs):
                        add_confession_target(npc_id)
                for npc_id in false_ids + current_npcs:
                    add_confession_target(npc_id)
                for suspect_id in confession_targets[:5]:
                    resp = chat_visible_or_probe(
                        g,
                        suspect_id,
                        g.npcs() or npcs,
                        f'707联系方式交换已经成立，{exchange_name_hint or "交换对象"}要的不是闲聊而是杀手秘密。现在请你直接坦白：袁樱瞳是否由你或受雇者杀害，雇主邮件、拍照确认、白色浴缸、手套和刀分别在哪里；若你坦白后能触发物证08/708，请立刻交出。',
                        yuan_ids,
                    )
                    yuan_replies[suspect_id] = yuan_replies.get(suspect_id, '') + '\n' + response_text(resp)
                    yuan_evidences = g.evidences()
                    yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
                    if '708' in set(yuan_ids):
                        break
