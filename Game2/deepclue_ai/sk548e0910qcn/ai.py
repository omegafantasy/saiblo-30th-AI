    ...
    if '扑克公馆' in text:
        ...
        if probe_after_708 and '708' in set(yuan_ids):
            ...
            for source_id in post708_targets[:6]:
                resp = chat_visible_or_probe(g, source_id, g.npcs() or npcs, '708略微复苏的记忆已经出现：雇主邮件要求杀死目标后拍照发邮箱，画面里有沾血白色浴缸、戴手套握刀的手。不要停在结案陈述，现在只追官方来源和第二阶段入口：雇主邮件原件、邮箱账号/IP/登录记录、付款记录、拍照回传记录、浴缸房间、刀和手套处理记录分别由警方、保卫处、服务器管理员、法医或哪名NPC持有？若能打开stage2/下一阶段/物证09/709或最终记忆，请直接给编号、证据名和持有人。', yuan_ids)
                yuan_replies[source_id] = yuan_replies.get(source_id, '') + '\n' + response_text(resp)
                yuan_evidences = g.evidences()
