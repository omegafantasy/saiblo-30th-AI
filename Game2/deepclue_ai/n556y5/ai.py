...
class Game:
    ...
    def req(self, action: str, **kwargs: Any) -> Any:
        try:
            ...
            log(f'[n556y5] request failed action={action}: {exc}')
            return {}
    ...
    def chat(self, npc: str, question: str, evidences: list[str] | None = None) -> dict[str, Any]:
        ...
        for attempt in range(3):
            ...
            if not (isinstance(resp, dict) and resp.get('error')):
                break
            log(f'[n556y5] chat error retry={attempt} npc={npc} q={compact(question, 36)} err={compact(resp.get("error"), 80)}')
        ...
        if new_stage > self.stage:
            log(f'[n556y5] stage {self.stage}->{new_stage} npc={npc} q={compact(question, 36)}')
    ...
    def retry_trigger(self, trigger_qs: list[tuple[Any, ...]], max_retries: int = 2) -> bool:
        ...
        for attempt in range(max_retries):
            for item in trigger_qs:
                ...
                log(f'[n556y5] retry a{attempt} npc={npc} q={compact(question, 40)}')
...
def solve_rose_direct(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    if not banker:
        banker = '银行家'
    log(f'[n556y5] rose direct murderer_id={murderer_id} murderer={murderer} banker={banker}')
    g.answer(
        murderer=murderer,
...
def solve_rose(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    if set(roles) != {'xiao', 'bai', 'deng', 'fan', 'ye', 'cui'}:
        log(f'[n556y5] rose role mapping incomplete roles={roles}')
    ...
    log(
        f'[n556y5] rose script xiao={xiao_name}/{xiao} bai={bai_name}/{bai} '
    ...
    log(f'[n556y5] rose final stage={g.stage} calls={g.calls}')
    g.answer(
        murderer=cui_name,
...
def solve_z_script(g: Game, npcs: list[str], evidences: list[dict[str, Any]]) -> None:
    ...
    if set(roles) != {'A', 'B', 'C', 'D', 'E'}:
        log(f'[n556y5] z role mapping incomplete roles={roles}')
    ...
    d_name, d = roles['D']
    e_name, e = roles['E']
    log(f'[n556y5] z script A={a_name}/{a} B={b_name}/{b} C={c_name}/{c} D={d_name}/{d} E={e_name}/{e}')
    g.stage = 0
    g.calls = 0
    ...
    log(f'[n556y5] z final stage={g.stage} calls={g.calls}')
    g.answer(
        murderer=e_name,
...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        def ask_all_ordered(question: str, evidences_arg: list[str] | None = None) -> None:
            ynpcs = g.npcs() or npcs
            ordered = list(reversed(ynpcs)) + [x for x in ynpcs if x not in set(reversed(ynpcs))]
            for ynpc in ordered:
                resp = g.chat(ynpc, question, evidences_arg)
                yuan_replies[ynpc] = yuan_replies.get(ynpc, '') + '\n' + response_text(resp)
        ask_all_ordered('不要只讲传闻。请说明你本人看到或确认了什么：谁从生物馆出来，谁接触尸块或行李箱，谁清空手机，谁伪造死亡时间，谁从投票中获利？')
        ask_all_ordered('袁樱瞳碎尸案请完整说明：手机、凌晨1点女性尸体照片、lo裙、栗色假发、黄色行李箱、投票异常、出国名额、张朔、张壹、生物馆、世纪林、李海天、1919黑车、保安奇怪网站分别是什么线索？')
        yuan_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}]
        ask_all_ordered('结合现有证据重新推理袁樱瞳死亡：实际死者是谁，凌晨照片是谁，张壹传闻哪里错，生物馆和世纪林尸块如何连接？', yuan_ids)
        ask_all_ordered('如果你知道凶手或关键隐瞒者，请直接给出名字、动机、作案过程和证据链。', yuan_ids)
        g.answer(murderer='无名氏', motivation='无', method='无')
        return
    else:
        method = '未知'
    log(f'[n556y5] unknown hint={compact(hint, 50)} suspect={suspect}')
...
def solve_case(g: Game, case_idx: int) -> bool:
    ...
    text = all_text(hint, evidences)
    log(f'[n556y5] case={case_idx} npcs={sorted(npcs)} marks={marks} hint={compact(hint, 60)}')
    if 'Rose' in text:
...
def main() -> int:
    ...
    for case_idx in range(6):
        try:
            ...
            log(f'[n556y5] fatal case={case_idx}: {exc}')
            try:
                g.answer('', '未知', '未知')
            except Exception:
                pass
