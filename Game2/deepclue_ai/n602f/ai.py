                ...
                if 'poker_after_ids' in locals():
                    ...
                    if {'501', '502', '503', '504'} & set(n602_ids):
                        for npc_id in n602_targets[:14]:
                            n602_ask(npc_id, '于书华、刘丽雯、张壹、林渝植、Joker和张子韩这条线已经串起来。请只补缺口：失踪女儿的报案记录、亲子或身份鉴定、心形胎记照片、花纹村组织名单、POKER纹身照片和谁把Joker引入衣帽间。')
                        n602_ids = [
                        ...
                        for npc_id in n602_targets[:14]:
                            n602_ask(npc_id, '如果还有一份材料没有交出，它应当是失踪少女/刘丽雯女儿/林渝植身份的原件。请说清它由谁保管，内容是什么，如何证明最终凶手。')
