                ...
                if reception_id and g.stage >= 3:
                    ...
                    if {'401', '402'}.issubset(set(monitor_ids)):
                        ...
                        if isinstance(ev501, dict):
                            ...
                            poker_evidences = g.evidences()
                            ev404_after_501 = next((ev for ev in poker_evidences if str(ev.get('id')) == '404'), None)
                            if not isinstance(ev404_after_501, dict):
                                merge_ids = [
                                    str(ev.get('id'))
                                    for ev in poker_evidences
                                    if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404', '501', '502', '503', '504', '601', '602', '603', '604'}
                                ]
                                merge_sources: list[str] = []

                                def add_merge_source(npc_id: str) -> None:
                                    if npc_id and npc_id not in merge_sources:
                                        merge_sources.append(npc_id)
                                ...
                                for npc_id in (
                                    ...
                                    add_merge_source(npc_id)
                                merge_q = (
                                    '501匿名50万元转账已经登记，同一批刑警材料还缺车辆页。'
                                    '不要展开405/607/608，也不要总结凶手，只把402里7:20经过大门并停到后院窗边的车登记成404：'
                                    '车牌号京F·A7590、车主/使用人、7:20大门记录和后院停车记录。若404不在你手里，请只说保管人。'
                                )
                                for merge_id in merge_sources[:4]:
                                    chat_visible_or_probe(g, merge_id, follow_npcs, merge_q, merge_ids)
                                    poker_evidences = g.evidences()
                                    ev404_after_501 = next((ev for ev in poker_evidences if str(ev.get('id')) == '404'), None)
                                    if isinstance(ev404_after_501, dict):
                                        ev404 = ev404_after_501
                                        globals()['POKER_HAS_404'] = True
                                        break
                        poker_evidences = g.evidences()
                        post601_ids = [
