#!/usr/bin/env python3
"""Game2 DeepClue AI v41 — ultra-narrow submit-response probe.

Purpose:
- Preserve the proven v37 scripted path exactly
- Avoid platform stderr truncation by removing all non-essential logs
- Capture only the raw `answer` response for case1
...
def compact_payload(obj: Any) -> str:
    try:
...
class Game:
    ...
    def submit(self, murderer: str, motivation: str, method: str) -> Any:
        return self.sdk.request('answer', murderer=murderer, motivation=motivation, method=method)
...
def solve_case1(g: Game) -> None:
    ...
    log(f'[v41] case1 submit_resp stage_before={g.stage} payload={compact_payload(result)}')
...
def main() -> int:
    ...
    while True:
        try:
            ...
            log(f'[v41] fatal exc={type(exc).__name__}: {exc}')
            try:
