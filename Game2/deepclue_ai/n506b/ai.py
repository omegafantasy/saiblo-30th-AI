...
def npc_named_in_hint(hint: str, npcs: list[str]) -> str:
    m = re.search(r'，([^，。]+)是个好的信息来源', hint)
    if not m:
        return ''
    return id_for_name(m.group(1).strip(), npcs)
...
def solve_unknown(g: Game, npcs: list[str], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        info = npc_named_in_hint(hint, npcs)
        if info:
            g.stage = 0
            g.calls = 0
            ...
            after_marks = g.marks()
            after_npcs = g.npcs()
            reception = next((npc for npc in after_npcs if npc != info and after_marks.get(npc) is True), '')
            if reception:
                g.chat(reception, '公馆内有什么异常发现？厨房刀具、门窗、血迹和面具哪里不对？')
