...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        if info_id:
            suspect = cn_name(info_id)
            first_resp = g.chat(info_id, '请说说案发现场情况，以及你手中的证据。')
            first_reply = str(first_resp.get('reply') or '')
            g.chat(info_id, '你是谁？你和死者是什么关系？')
            if g.stage < 2 and any(key in first_reply for key in ('手机', '聊天记录', '随身物品', '证据')):
                g.chat(info_id, '死者面具、手机聊天记录和随身物品分别说明什么？')
            if g.stage >= 2:
                g.evidences()
                follow_hint = g.hint()
                ...
                if reception_id and '人口贩卖' not in first_reply:
