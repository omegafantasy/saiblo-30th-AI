...
def solve_rose_direct(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    if not banker:
        banker = '银行家'
    if (murderer, banker) in ROSE_RISKY_NAME_PAIRS:
        motivation = f'{murderer}因与{banker}的情感纠葛将Rose视为阻碍，遂预谋投毒杀害Rose。'
    else:
...
def solve_rose(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    if (cui_name, deng_name) in ROSE_RISKY_NAME_PAIRS:
        motivation = f'{cui_name}因与{deng_name}的情感纠葛将Rose视为阻碍，遂预谋投毒杀害Rose。'
    else:
