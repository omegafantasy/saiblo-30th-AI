    ...
    if '扑克公馆' in text:
        ...
        if '707' in set(yuan_ids) and '708' not in set(yuan_ids):
            ...
            if '707' in set(yuan_ids) and '708' not in set(yuan_ids):
                ...
                yuan_evidences = g.evidences()
                yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
        # After 708, keep probing official-source/stage2 handoffs instead of stopping at the memory fragment.
        probe_after_708 = True
        if probe_after_708 and '708' in set(yuan_ids):
            ...
            confess_names: list[str] = []
            for name in re.findall(r'([一-龥]{1,4})坦白后', ev708_text):
                if name not in confess_names:
                    confess_names.append(name)
                for npc_id in global_name_ids(name, g.npcs() or npcs):
                    if npc_id and npc_id not in post708_targets:
                        post708_targets.append(npc_id)
            for name in re.findall(r'([一-龥]{1,4})坦白', ev708_text):
                if name not in confess_names:
                    confess_names.append(name)
                for npc_id in global_name_ids(name, g.npcs() or npcs):
                    if npc_id and npc_id not in post708_targets:
                        post708_targets.append(npc_id)
            confess_label = '、'.join(confess_names) or '物证08写明的坦白者'
            for npc_id in story_target_ids(ev708_text + '\n' + '\n'.join(yuan_replies.values()), g.npcs() or npcs, max_ids=8):
                if npc_id and npc_id not in post708_targets:
                    post708_targets.append(npc_id)
            ...
            for source_id in post708_targets[:6]:
                resp = chat_visible_or_probe(g, source_id, g.npcs() or npcs, f'708略微复苏的记忆已经出现：雇主邮件要求杀死目标后拍照发邮箱，画面里有沾血白色浴缸、戴手套握刀的手。不要停在结案陈述，现在只追官方来源和第二阶段入口：如果你就是{confess_label}或参与坦白的人，请先说明这段记忆对应谁坦白、谁发邮件、谁拍照、谁处理刀和手套；如果不是，请直接指出{confess_label}、雇主邮件原件、邮箱账号/IP/登录记录、付款记录、拍照回传记录、浴缸房间由警方、保卫处、服务器管理员、法医或哪名NPC持有。若能打开stage2/下一阶段/物证09/709或最终记忆，请直接给编号、证据名和持有人。', yuan_ids)
                yuan_replies[source_id] = yuan_replies.get(source_id, '') + '\n' + response_text(resp)
                yuan_evidences = g.evidences()
