...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        competitor = current_npcs[0] if len(current_npcs) > 0 else ''
        teacher = current_npcs[1] if len(current_npcs) > 1 else ''
        witness = current_npcs[2] if len(current_npcs) > 2 else ''
        if competitor:
            remember(competitor, g.chat(competitor, '请只交代你本人能提供的原始材料：袁樱瞳手机、凌晨1点尸体照片、照片删除记录、lo裙栗色假发、黄色行李箱维修或来源、你和她长相相似及出国名额竞争。'))
        if teacher:
            remember(teacher, g.chat(teacher, '请只交出或说明课程展示的原始材料：投票纸、签到或缺勤、47人/48票/24比23、异笔迹票、谁因多出一票直接获利。'))
        if witness:
            remember(witness, g.chat(witness, '请只交出或说明你能核验的原始材料：1919黑车记录、保安奇怪网站日志、生物馆或楼道监控、世纪林尸块记录、李海天尸检报告和蓝色背包海豚挂件。'))
        yuan_ids = yuan_ids_now()
        if competitor:
            remember(competitor, g.chat(competitor, '结合已公开物证，只回答：手机照片、lo裙假发、黄色行李箱和相似外貌是否构成身份置换；谁有机会清空手机或伪造凌晨1点照片？', yuan_ids))
            yuan_ids = yuan_ids_now()
        if teacher:
            remember(teacher, g.chat(teacher, '结合已公开物证，只回答：多出的异笔迹票是否改变了出国名额结果，袁樱瞳说等到周五是否准备揭发投票作弊？', yuan_ids))
            yuan_ids = yuan_ids_now()
        if witness:
            remember(witness, g.chat(witness, '结合已公开物证，只回答：生物馆跑出者、1919黑车、保安网站、世纪林尸块和李海天旧案中，哪一项能证明尸体被二次转移？', yuan_ids))
            yuan_ids = yuan_ids_now()
        if '705' in yuan_ids and witness:
            remember(witness, g.chat(witness, '依据705李海天尸检报告继续给下一份证据：DNA比对、海豚挂件来源、生物馆监控、楼道监控或黑车轨迹，哪一项能直接确认袁樱瞳真实死亡时间？', yuan_ids))
            yuan_ids = yuan_ids_now()
        elif witness:
            remember(witness, g.chat(witness, '如果李海天尸检报告还没公开，请明确报告在谁手里；如果你能拿到，请直接交给我，不要只复述传闻。', yuan_ids))
            yuan_ids = yuan_ids_now()
        for ynpc in current_npcs:
            remember(ynpc, g.chat(ynpc, '现在只按可核验物证给出凶手或关键隐瞒者、动机、作案过程，不要复述无法证实的传闻。', yuan_ids))
