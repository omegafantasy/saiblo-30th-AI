...
ROSE_METHOD_RISK_BANKERS = {'张朔', '林晚舟', '许清和', '顾云舒', '王泽', '叶青衡', '罗方琛', '周林君'}
...
def rose_method_text(murderer: str, banker: str) -> str:
    text = f'{murderer}利用家族药材生意取得夹竹桃毒素，趁18:40左右在准备室将毒投入Rose的专用蜂蜜水杯，Rose饮用后中毒身亡。'
    if banker in ROSE_METHOD_RISK_BANKERS:
        text += f'为确保{banker}不再与Rose接触，{murderer}选择在演出前的短时空档快速下毒并立即离场，降低被当场识破的风险。'
    return text
...
def solve_rose_direct(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    g.answer(
        ...
        method=rose_method_text(murderer, banker),
    )
...
def solve_rose(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    g.answer(
        ...
        method=rose_method_text(cui_name, deng_name),
    )
