...
def solve_case(sdk: SDK, case_index: int) -> bool:
    ...
    if not kb.background and not kb.npcs:
        return False
    log(f'[game2-v2] case={case_index} stage={kb.stage} npcs={len(kb.npcs)}')
    ...
    if method_too_generic(answer.get('method', '')) and len(kb.npcs) > 0:
        method_probe(sdk, kb)
        answer = final_answer(sdk, kb)
    log(f'[game2-v9] answer case={case_index} murderer={answer["murderer"]!r} method={answer["method"]!r}')
    resp = sdk.request('answer', **answer)
    log(f'[game2-v9] answer_result case={case_index} resp={resp}')
    return True
...
def main() -> int:
    ...
    log(f'[game2-v9] welcome={welcome}')
    case_index = 0
    while True:
        try:
            ...
            log(f'[game2-v9] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
