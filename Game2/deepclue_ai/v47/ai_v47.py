#!/usr/bin/env python3
"""Game2 DeepClue AI v47 — g2auto-path pre-answer hidden-question probe.

Purpose:
- Preserve the current g2auto/v26 local path represented by v36, including
  its Case 1 ordering and `NPC_E` answer spelling
- Add only the proven pre-answer hidden question pair before submit
- Verify whether the 2357 trigger transfers cleanly onto the actual g2auto
  path, rather than only working on the v37/v43 branch
...
def solve_case0(g: Game) -> None:
    ...
    log(f'[v47] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1_reorder(g: Game) -> None:
    ...
    for npc, question in [
        ...
        if isinstance(resp, dict):
            log(
                f"[v47] pre-answer probe [{npc}] stage={resp.get('stage')} "
                f"keys={sorted(resp.keys())} error={resp.get('error')!r}"
            )
        else:
            log(f'[v47] pre-answer probe [{npc}] non-dict={type(resp).__name__}')

    log(f'[v47] case1 stage={g.stage} calls={g.calls}')
    result = g.submit(
        murderer='NPC_E',
...
def solve_case(g: Game, case_index: int) -> bool:
    ...
    log(f'[v47] case={case_index} npcs={sorted_npcs}')
    g.stage = 0
    g.calls = 0
