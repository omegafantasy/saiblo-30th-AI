...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        ask_all('这起案子里谁不是凶手但在替凶手掩护？谁清空手机、搬运行李箱、伪造照片、转移尸块或制造张壹传闻？')
        ask_all('谁知道真相却只说传闻？请指出每个人隐瞒的具体事实和保护对象。')
        yuan_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}]
        ask_all('结合现有证据，掩护链条如何连接投票异常、手机照片、生物馆、世纪林和1919黑车？', yuan_ids)
        ask_all('请给出关键隐瞒者、被隐瞒的事实、凶手或真实处理尸体者以及证据链。', yuan_ids)
