...
class Game:
    ...
    def req(self, action: str, **kwargs: Any) -> Any:
        try:
            ...
            log(f'[n556y3] request failed action={action}: {exc}')
            return {}
    ...
    def chat(self, npc: str, question: str, evidences: list[str] | None = None) -> dict[str, Any]:
        ...
        for attempt in range(3):
            ...
            if not (isinstance(resp, dict) and resp.get('error')):
                break
            log(f'[n556y3] chat error retry={attempt} npc={npc} q={compact(question, 36)} err={compact(resp.get("error"), 80)}')
        ...
        if new_stage > self.stage:
            log(f'[n556y3] stage {self.stage}->{new_stage} npc={npc} q={compact(question, 36)}')
    ...
    def retry_trigger(self, trigger_qs: list[tuple[Any, ...]], max_retries: int = 2) -> bool:
        ...
        for attempt in range(max_retries):
            for item in trigger_qs:
                ...
                log(f'[n556y3] retry a{attempt} npc={npc} q={compact(question, 40)}')
...
def solve_rose_direct(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    if not banker:
        banker = '银行家'
    log(f'[n556y3] rose direct murderer_id={murderer_id} murderer={murderer} banker={banker}')
    g.answer(
        murderer=murderer,
...
def solve_rose(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    if set(roles) != {'xiao', 'bai', 'deng', 'fan', 'ye', 'cui'}:
        log(f'[n556y3] rose role mapping incomplete roles={roles}')
    ...
    log(
        f'[n556y3] rose script xiao={xiao_name}/{xiao} bai={bai_name}/{bai} '
    ...
    log(f'[n556y3] rose final stage={g.stage} calls={g.calls}')
    g.answer(
        murderer=cui_name,
...
def solve_z_script(g: Game, npcs: list[str], evidences: list[dict[str, Any]]) -> None:
    ...
    if set(roles) != {'A', 'B', 'C', 'D', 'E'}:
        log(f'[n556y3] z role mapping incomplete roles={roles}')
    ...
    d_name, d = roles['D']
    e_name, e = roles['E']
    log(f'[n556y3] z script A={a_name}/{a} B={b_name}/{b} C={c_name}/{c} D={d_name}/{d} E={e_name}/{e}')
    g.stage = 0
    g.calls = 0
    ...
    log(f'[n556y3] z final stage={g.stage} calls={g.calls}')
    g.answer(
        murderer=e_name,
...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        method = '未知'
    log(f'[n556y3] unknown hint={compact(hint, 50)} suspect={suspect}')
...
def solve_case(g: Game, case_idx: int) -> bool:
    ...
    text = all_text(hint, evidences)
    log(f'[n556y3] case={case_idx} npcs={sorted(npcs)} marks={marks} hint={compact(hint, 60)}')
    if 'Rose' in text:
...
def main() -> int:
    ...
    for case_idx in range(6):
        try:
            ...
            log(f'[n556y3] fatal case={case_idx}: {exc}')
            try:
                g.answer('', '未知', '未知')
            except Exception:
                pass
