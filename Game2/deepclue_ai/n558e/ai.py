        ...
        if info_id:
            ...
            if g.stage >= 2:
                ...
                if 'reception_id' in locals() and reception_id:
                    g.chat(reception_id, '你先把宾客到达顺序和每个人到场时间精确讲一遍。', deep_ids)
                    g.chat(reception_id, '只围绕聊天记录和到场表：谁最可能是林渝植，理由是什么？', deep_ids)
                    g.chat(reception_id, '请给出“聊天记录-到场表-异常发现”三段式摘要，各一句。', deep_ids)
                if info_id:
                    g.chat(info_id, '你对Joker身份、林渝植和梅花5关系是否有不同判断？', deep_ids)
