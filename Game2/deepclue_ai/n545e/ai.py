...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        if info_id:
            ...
            if g.stage >= 2:
                ...
                if g.stage < 3 and ev_ids:
                    ...
                    g.evidences()
        if info_id and g.stage >= 3:
            poker_motivation = '凶手为掩盖扑克公馆中Joker委托安排、宾客到达时间表、死者真实身份和梅花5不在名单中的破绽，利用全员戴面具造成身份混淆并杀人灭口。'
        ...
        method = '未知'
    log(f'[v56] unknown hint={compact(hint, 50)} suspect={suspect}')
    g.answer(murderer=suspect, motivation=poker_motivation if '扑克公馆' in text else '未知', method=method)
