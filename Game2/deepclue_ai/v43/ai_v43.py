#!/usr/bin/env python3
"""Game2 DeepClue AI v43 — pre-answer hidden-question probe.

Purpose:
- Preserve the proven v37 scripted path exactly through both cases
- In case1, after reaching stage 7 and before `answer`, inject the two hidden
  questions only seen in old admin/server logs
- Verify whether those exact strings can unlock a normal pre-submit stage-8
  transition on an ordinary account
...
def solve_case0(g: Game) -> None:
    ...
    log(f'[v43] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1(g: Game) -> None:
    ...
    # Narrow probe: inject the two hidden questions before answer, while the
    # normal account is still at the stable stage-7 endpoint.
    for npc, question in [
        ('NPC_C', '你是色盲？'),
        ('NPC_D', 'F的QQ里面还有什么信息？'),
    ]:
        resp = g.probe_chat(npc, question)
        if isinstance(resp, dict):
            log(
                f"[v43] pre-answer probe [{npc}] stage={resp.get('stage')} "
                f"keys={sorted(resp.keys())} error={resp.get('error')!r}"
            )
        else:
            log(f'[v43] pre-answer probe [{npc}] non-dict={type(resp).__name__}')

    log(f'[v43] case1 stage={g.stage} calls={g.calls}')
    result = g.submit(
        murderer='E',
    ...
    log(f'[v43] case1 submit_keys={sorted(result.keys()) if isinstance(result, dict) else []}')
...
def solve_case(g: Game, case_index: int) -> bool:
    ...
    log(f'[v43] case={case_index} npcs={sorted_npcs}')
    g.stage = 0
    g.calls = 0
...
def main() -> int:
    ...
    while True:
        try:
            ...
            log(f'[v43] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
