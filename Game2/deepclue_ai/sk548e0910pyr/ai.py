    ...
    if '扑克公馆' in text:
        ...
        if info_id:
            ...
            if g.stage >= 2:
                ...
                method_target = reception_id or info_id
                probe_targets = []
                for target in (method_target, info_id):
                    if target and target not in probe_targets:
                        probe_targets.append(target)
                probe_targets.extend([npc for npc in (g.npcs() or npcs) if npc not in probe_targets][:3])
                poker_probe_questions = [
                    '只谈死亡方式：背部三把刀、冰块融化、方形塑料盒、厨房缺刀、无指纹刀具和稀释血水分别说明什么？',
                    '死者、Joker、林渝植、梅花5面具、粘住的面具、双臂灼烧和被稀释血水之间到底是什么身份链？',
                    '11:30电脑浏览“冰块固定刀柄伪装谋杀”是谁留下的？这是自杀伪装他杀、他杀伪装自杀，还是第三人布置？',
                    '谁有机会进入厨房冰柜、死者房间和衣帽间，把三把刀、方形塑料盒、面具和尸体串成完整作案过程？',
                ]
                for question in poker_probe_questions:
                    for target in probe_targets:
                        g.chat(target, question, deep_ids)
                    g.evidences()
        ...
        def yuan_ids_now() -> list[str]:
            return [
                str(ev.get('id'))
                for ev in g.evidences()
                if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}
            ]
        ...
        role_scores: dict[str, dict[str, int]] = {npc: {'competitor': 0, 'teacher': 0, 'witness': 0} for npc in current_npcs}
        for npc in current_npcs:
            reply = yuan_replies.get(npc, '')
            role_scores[npc]['competitor'] = sum(1 for key in ('长得', '手机', '行李箱', '险胜', '出国名额', '等到周五') if key in reply)
            role_scores[npc]['teacher'] = sum(1 for key in ('老师', '教师', '课程', '投票', '全班', '24', '23', '笔迹') if key in reply)
            role_scores[npc]['witness'] = sum(1 for key in ('1919', '黑车', '保安', '网站', '生物馆', '世纪林', '夜跑') if key in reply)
        role_order = {
            role: sorted(role_scores, key=lambda npc: (-role_scores[npc][role], cn_name(npc)))
            for role in ('competitor', 'teacher', 'witness')
        }
        competitor = role_order['competitor'][0] if role_order['competitor'] else ''
        teacher = role_order['teacher'][0] if role_order['teacher'] else ''
        witness = role_order['witness'][0] if role_order['witness'] else ''

        yuan_ids = yuan_ids_now()
        if competitor:
            for question in (
                '你和袁樱瞳长得像、竞争出国名额、周五险胜、捡到手机、黄色行李箱、等到周五这些事按时间顺序讲清楚。',
                '袁樱瞳手机为什么被清空，只剩凌晨1点女性尸体照片？照片里的lo裙、栗色假发、女性尸体是谁？',
                '你的黄色行李箱坏在哪里、什么时候出门修、里面是否装过尸块或衣物？有没有人能证明？',
                '袁樱瞳说等到周五到底要揭发谁？揭发投票作弊、替身身份、手机照片还是生物馆尸块？',
            ):
                remember(competitor, g.chat(competitor, question, yuan_ids))
                yuan_ids = yuan_ids_now()
        if teacher and teacher != competitor:
            for question in (
                '课程展示投票请重新精确说明：全班人数、展示两人、张壹是否翘课、应投票数、实际票数、24比23和异笔迹票。',
                '多出来的票对谁最有利？如果袁樱瞳周五要揭发投票异常，谁会最害怕？',
                '出国交流名额到底几个？袁樱瞳、竞争者、张壹分别在名额里处于什么位置？',
            ):
                remember(teacher, g.chat(teacher, question, yuan_ids))
                yuan_ids = yuan_ids_now()
        if witness and witness not in {competitor, teacher}:
            for question in (
                '你看到的1919黑车、保安奇怪网站、保安离岗、夜跑、生物馆逃出者分别是什么时间什么地点？',
                '你看到从生物馆跑出来的人到底是谁？张壹传闻哪里错？这个人和袁樱瞳尸块有什么关系？',
                '世纪林发现尸块、李海天死亡、生物馆和袁樱瞳手机照片之间有没有同一具尸体或同一条转移路线？',
            ):
                remember(witness, g.chat(witness, question, yuan_ids))
                yuan_ids = yuan_ids_now()

        ask_all('现在只为了进入下一阶段：请把袁樱瞳手机703、投票704、行李箱、黑车、生物馆、世纪林尸块连成一条可验证时间线。', yuan_ids)
        yuan_ids = yuan_ids_now()
        ask_all('如果还有未公开物证或证词，请指出证据编号、来源NPC、如何解锁，以及它证明谁杀了袁樱瞳。', yuan_ids)
