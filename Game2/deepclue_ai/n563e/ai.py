...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        ask_all('请确认真实死者和身份置换：袁樱瞳、凌晨1点照片女性、lo裙栗色假发女性、黄色行李箱和世纪林尸块分别是谁？')
        ask_all('谁利用长相相似、假发、手机照片或行李箱制造袁樱瞳仍存活或死亡时间错误的假象？')
        yuan_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}]
        ask_all('结合现有证据，哪一个身份判断是错的？请给出能核验的物证和证词矛盾。', yuan_ids)
        ask_all('还有哪些未公开证据能确认尸块真实身份、死亡时间和替身链条？', yuan_ids)
