        ...
        if info_id:
            ...
            if g.stage >= 2:
                ...
                if ev_ids:
                    ...
                    poker_evidences = g.evidences()
                evidence_ids_now = {str(ev.get('id')) for ev in poker_evidences}
                if g.stage >= 3 and {'203', '204', '205'}.issubset(evidence_ids_now):
                    poker_method = '死者用方形塑料盒冻住三把厨房刀的刀柄，借冰块固定刀具刺入背部；冰融化后留下无指纹刀具、塑料盒和稀释血水，伪装成他杀。'
        method = poker_method or '凶手利用扑克公馆全员戴面具、身份混淆和场馆密室条件，在衣帽间用刀杀害并伪装死者。'
