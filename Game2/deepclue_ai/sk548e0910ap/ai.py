...
def rose_motivation_text(murderer: str, banker: str) -> str:
    if murderer == '王泽' and banker == '张壹':
        return '王泽因爱慕张壹而嫉恨Rose。案发前Rose以曝光王泽和张壹关系相要挟并索要大量钱财，王泽担心失去张壹并遭持续勒索，遂下毒杀害Rose。'
    text = (
        f'{murderer}爱慕并想独占{banker}，认为Rose纠缠{banker}、会阻碍自己与{banker}在一起，'
        f'因此为扫清情敌而杀害Rose。'
    )
    if murderer in ROSE_SUFFIX_KILLERS and (murderer, banker) not in ROSE_SUFFIX_BLOCK_PAIRS:
