    ...
    if '扑克公馆' in text:
                ...
                if 'poker_after_ids' in locals():
                    ...
                    if {'601', '602', '603', '604', '606'} & set(n604_ids):
                        for npc_id in n604_targets[:14]:
                            n604_ask(npc_id, '现在只查身份原件：刘丽雯女儿失踪报案、亲子鉴定、心形胎记医疗记录、林渝植档案、LYZ项链来源、张子韩复学和Joker人口贩卖名单。请说明哪份原件在现场已经掌握但还没有交出。')
                        n604_ids = [
                            str(ev.get('id')) for ev in g.evidences()
                            if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404', '405', '501', '502', '503', '504', '601', '602', '603', '604', '605', '606', '607', '608'}
                        ]
                        for npc_id in n604_targets[:14]:
                            n604_ask(npc_id, '如果林渝植就是刘丽雯女儿或真正梅花5，请直接给DNA/亲子/医疗/报案/警方卷宗的证据名、持有人和内容，并说明它如何闭环Joker周克、于书华和最终凶手。')
                    elif {'501', '502', '503', '504'} & set(n604_ids):
                        for npc_id in n604_targets[:10]:
                            n604_ask(npc_id, '不要停在车牌、转账或邀请函。沿着身份原件链继续查：Joker真实姓名、死者手机、LYZ随身物、刘丽雯女儿、花纹村组织、POKER纹身照片和现场未交原件分别由谁保管。')
                ...
                if 'poker_after_ids' in locals():
                    ...
                    if ({'601', '602', '603', '604'} & n605_id_set) and not ({'605', '606', '607', '608'} & n605_id_set):
                        for npc_id in n605_targets[:10]:
                            n605_ask(npc_id, '从刑警卷宗角度查缺口：601失踪少女、602花纹村、603/604刘丽雯旧身份、501于书华转账、502约见聊天和503/504梅花5物品已经能连上。请直接调606三人照片、605现场证据、人口贩卖名册、亲子/DNA和最终结案卷宗。')
                            n605_ids = [
                                str(ev.get('id')) for ev in g.evidences()
                                if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404', '405', '501', '502', '503', '504', '601', '602', '603', '604', '605', '606', '607', '608'}
                            ]
                        ...
                        if not ({'605', '606', '607', '608'} & set(n605_ids)):
                            for npc_id in n605_targets[:8]:
                                n605_ask(npc_id, '如果因为时机未到不能交605，请说明时机条件是什么；若条件已由501/502/601-604满足，请直接给证据编号、证据名、保管人和内容。')
                                n605_ids = [
                                    str(ev.get('id')) for ev in g.evidences()
                                    if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404', '405', '501', '502', '503', '504', '601', '602', '603', '604', '605', '606', '607', '608'}
                                ]
                        ...
                        for npc_id in n605_targets[:10]:
                            n605_ask(npc_id, '606出现后按卷宗继续：三人照片上的真实姓名、POKER纹身组织等级、Joker周克、于书华、红桃Q/刘瑄、林渝植和刘丽雯女儿之间还缺哪份官方证据；直接给605/607/608。')
                            n605_ids = [
                                str(ev.get('id')) for ev in g.evidences()
                                if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404', '405', '501', '502', '503', '504', '601', '602', '603', '604', '605', '606', '607', '608'}
                            ]
        ...
        if '705' not in set(n605_pre705_ids):
            ...
            for source_id in n605_pre705_targets[:6]:
                ...
                resp = chat_visible_or_probe(g, source_id, g.npcs() or npcs, '先不要问706。你刚才提到李海天、生物馆、世纪林、1919黑车或保安奇怪网站；现在只要李海天尸检报告705：死亡原因、四肢、蓝色背包海豚挂件、报告来源和谁拿到原件。', n605_pre705_ids)
            ...
            if '705' not in set(n605_pre705_ids):
                for source_id in n605_pre705_targets[:4]:
                    resp = chat_visible_or_probe(g, source_id, g.npcs() or npcs, '如果你不能给尸检报告，请只说谁能调取：法医/警方旧案卷宗、保卫处、生物馆监控、世纪林报警记录、1919车辆登记、手机原图元数据。不要猜凶手。', n605_pre705_ids)
                    yuan_replies[source_id] = yuan_replies.get(source_id, '') + '\n' + response_text(resp)
                    n605_pre705_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
                    if {'705', '706', '707', '708'} & set(n605_pre705_ids):
                        break
