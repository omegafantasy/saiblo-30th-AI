...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        for ynpc in ordered:
            ask(ynpc, '袁樱瞳案按官方custody拆分，不问泛化口供：703手机、704投票、705李海天尸检、706/707/708、周五揭发材料、票箱原件、尸源DNA、黄色行李箱、1919黑车、生物馆监控、保安网页分别由谁保管。')
        yuan_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '703', '704', '705', '706', '707', '708'}]
        for ynpc in ordered:
            ask(ynpc, '请按系统而不是传闻回答：教务/办公室/票箱、保卫处、警方旧案、法医、车辆登记、网站后台、手机云端、行李箱物证、蓝色背包海豚挂件中哪一个能打开705之后的下一层。', yuan_ids)
        ...
        if '705' in yuan_ids:
            for npc_id in targets[:16]:
                ask(npc_id, '705李海天尸检已出现时，不要复述背刺断肢；只追706/707/708：报告原件编号、DNA比对、蓝色背包主人、海豚挂件来源、1919车辆轨迹、生物馆监控、网页后台、手机照片元数据和警方旧案卷宗。', yuan_ids)
        else:
