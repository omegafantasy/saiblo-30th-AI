...
def poker_606_exact_holder_ids(text: str, npcs: list[str], current_npc: str = '') -> list[str]:
    targets: list[str] = []
    ...
    for pattern in (
        r'唯一保管人是([一-龥]{1,4})',
        r'([一-龥]{1,4})[^。；\n]{0,10}(?:是|就是|为)?唯一保管人',
        r'([一-龥]{1,4})[^。；\n]{0,14}(?:手里|手上|保管|掌握|负责|持有|拿着)(?:[^。；\n]{0,8}(?:605|607|608|照片原件|手机云端|最终卷宗|档案室))?',
        r'(?:605|607|608|照片原件|手机云端|最终卷宗|档案室)[^。；\n]{0,18}(?:在|由|归)[^。；\n]{0,6}([一-龥]{1,4})',
        r'([一-龥]{1,4})[^。；\n]{0,14}(?:警局授权|签字授权|授权调阅|正式授权)',
    ):
        for match in re.finditer(pattern, text or ''):
            for npc_id in global_name_ids(match.group(1), npcs):
                add(npc_id)

    if re.search(r'照片原件在我这里|605在我手上|在我这里|在我手上|警局授权|签字授权|地下室档案室|警方最终卷宗|时机未到', text or ''):
        add(current_npc)
    return targets[:4]


def poker_606_exact_holder_question() -> str:
    return (
        '606三人POKER纹身照片已经出现。现在不要泛谈组织链，也不要继续展开广搜；'
        '请先交出605/607/608、606照片原件、死者手机云端名单、地下室档案室或警方最终卷宗中你能公开的一项。'
        '如果你说的是唯一保管人、照片原件在你这里、605在你手上、警局授权或签字授权，请直接说明谁是下一手保管人和授权链条。'
    )
                    ...
                    if {'401', '402'}.issubset(set(monitor_ids)):
                        ...
                        if post601_set & {'601', '602', '603', '604'}:
                            ...
                            if '606' in post601_set:
                                ...
                                def follow_exact_606(source_id: str, resp: dict[str, Any]) -> None:
                                    nonlocal post606_ids, post606_set
                                    reply_text = response_text(resp)
                                    if post606_set & {'605', '607', '608'}:
                                        return
                                    if not re.search(r'唯一保管人是|照片原件在我这里|605在我手上|警局授权|签字授权|地下室档案室|警方最终卷宗|时机未到', reply_text or ''):
                                        return
                                    exact_targets = poker_606_exact_holder_ids(reply_text, follow_npcs, source_id)
                                    if not exact_targets:
                                        exact_targets = [source_id]
                                    for target_id in exact_targets:
                                        if target_id in asked_exact_606:
                                            continue
                                        asked_exact_606.add(target_id)
                                        exact_resp = chat_visible_or_probe(g, target_id, follow_npcs, poker_606_exact_holder_question(), post606_ids)
                                        post606_ids, post606_set = refresh_post601()
                                        follow_verbal_605(target_id, exact_resp)
                                        follow_exact_606(target_id, exact_resp)
                                        if post606_set & {'605', '607', '608'}:
                                            break
                        ...
                        if isinstance(ev405_open, dict) or isinstance(ev502_open, dict):
                            ...
                            if deep_id_set & {'503', '504', '505', '601', '602', '603', '604'}:
                                ...
                                asked_exact_deep: set[str] = set()

                                def follow_exact_deep(source_id: str, resp: dict[str, Any]) -> None:
                                    nonlocal deep_ids, deep_id_set
                                    reply_text = response_text(resp)
                                    if '606' not in deep_id_set:
                                        return
                                    ...
                                    if not exact_targets:
                                        exact_targets = [source_id]
                                    for target_id in exact_targets:
                                        if target_id in asked_exact_deep:
                                            continue
                                        asked_exact_deep.add(target_id)
                                        exact_resp = chat_visible_or_probe(g, target_id, follow_npcs, poker_606_exact_holder_question(), deep_ids)
                                        _, deep_ids, deep_id_set = refresh_deep_ids()
                                        follow_exact_deep(target_id, exact_resp)
                                        if deep_id_set & {'605', '607', '608'}:
                                            break

                                if '606' in deep_id_set and not ({'605', '607', '608'} & deep_id_set):
                                    for npc_id in deep_targets[:6]:
                                        resp = chat_visible_or_probe(g, npc_id, follow_npcs, '606三人POKER纹身照片已经出现。不要继续泛谈组织，按NPC刚给出的条件证明谁杀Joker：真正梅花5/林渝植的7:30-8:20时间线、Joker伪装梅花5、503/504/502威胁控制链、许清和接待记录、衣帽间约见记录、刀具血迹、暗格账本、地下室档案室和移尸路线。若证明成立，请交出605、607/608、组织名册或警方结案卷宗。', deep_ids)
                                        _, deep_ids, deep_id_set = refresh_deep_ids()
                                        follow_exact_deep(npc_id, resp)
                                        if deep_id_set & {'605', '607', '608'}:
                                            break

                                if deep_id_set & {'605', '606', '607', '608'}:
                                    for npc_id in deep_targets[:4]:
                                        resp = chat_visible_or_probe(g, npc_id, follow_npcs, 'Poker后续物证已经出现。继续闭环：谁杀Joker、为什么杀、林渝植/张子韩女儿身份怎么确认、王科瑾资金流和花纹村名单如何串起来，最后还缺哪一份实物或照片？', deep_ids)
                                        follow_exact_deep(npc_id, resp)
        ...
        if '707' in set(yuan_ids) and '708' not in set(yuan_ids):
            ...
            if contact_name:
                ...
                for contact_id in contact_targets[:3]:
                    resp = chat_visible_or_probe(g, contact_id, g.npcs() or npcs, f'707写的是你的联系方式，{exchange_name_hint or "交换对象"}想要那个不认识的运动少女联系方式。请先确认现实转交：我能不能把你的号码给他/她，谁能转交联系方式，应该去学生会、电子系、体育场还是宿舍找到{exchange_name_hint or "交换对象"}？这条线还牵到从生物馆出来、1919黑车和保安网站，你知道哪个地点或转交人？', yuan_ids)
            ...
            if '708' not in set(yuan_ids):
                ask_all(f'707已经出现，交换对象是{exchange_name_hint or "物证07写明的人"}。先只走现实联系路径：请直接告诉我如何找到{exchange_name_hint or "这个交换对象"}，他/她是否是学生会副会长、从生物馆出来的人、体育场夜跑遇到的人或要联系方式的人；谁能带我去宿舍/学生会/电子系/体育场见他/她，谁能代为转交{contact_name or "运动少女"}的联系方式，1919黑车和保安网站分别指向哪个地点或转交人？', yuan_ids)
                yuan_evidences = g.evidences()
                yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
            if '708' not in set(yuan_ids):
                for exchange_id in exchange_targets[:3]:
                    resp = chat_visible_or_probe(g, exchange_id, g.npcs() or npcs, f'我已经先向{contact_name or "运动少女"}本人确认转交，并按现实路线去学生会/电子系/体育场/宿舍找你，也确认谁能转交联系方式。现在交换条件已经满足：请兑现你说的杀手秘密，交出物证08/708，或说明雇主邮件、白色浴缸、手套和刀具的下一份证据在哪里。', yuan_ids)
        ...
        if '707' in set(yuan_ids) and '708' not in set(yuan_ids):
            ...
            if contact_name:
                ...
                for npc_id in global_name_ids(contact_name, g.npcs() or npcs):
                    if npc_id and npc_id not in contact_targets:
                        contact_targets.append(npc_id)
                for contact_id in contact_targets[:3]:
                    resp = chat_visible_or_probe(g, contact_id, g.npcs() or npcs, f'707写的是你的联系方式，{exchange_name_hint or "交换对象"}想拿到它来交换杀手秘密。请只确认现实转交：我能否把你的号码给他/她，谁能转交联系方式，应该去学生会、电子系、体育场还是宿舍找到{exchange_name_hint or "交换对象"}？这条线还牵到从生物馆出来、1919黑车和保安网站，你知道哪个地点或转交人？', yuan_ids)
                    yuan_replies[contact_id] = yuan_replies.get(contact_id, '') + '\n' + response_text(resp)
                    yuan_evidences = g.evidences()
                    yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
                    if '708' in set(yuan_ids):
                        break
            if '708' not in set(yuan_ids):
                ask_all(f'707已经出现。不要总结碎尸案，只执行现实转交流程：物证07写明{exchange_name_hint or "交换对象"}想要{contact_name or "运动少女"}的联系方式；请指出{exchange_name_hint or "交换对象"}在哪里、怎样联系、是否要去学生会/宿舍/体育场/电子系/保卫处，谁能代为转交联系方式，从生物馆出来、1919黑车和保安网站分别指向哪个地点或转交人。', yuan_ids)
                yuan_evidences = g.evidences()
                yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
            if '707' in set(yuan_ids) and '708' not in set(yuan_ids):
            ...
            if '708' not in set(yuan_ids):
                for exchange_id in post707_targets[:5]:
                    resp = chat_visible_or_probe(g, exchange_id, g.npcs() or npcs, f'我已经先向{contact_name or "运动少女"}本人确认授权，把联系方式给他/她，并按学生会/电子系/体育场/宿舍的现实路线找到你或找到转交人。你承诺拿到号码就说杀手秘密；现在请直接兑现：谁是杀手，物证08/708在哪里，雇主邮件、白色浴缸、手套和刀具对应什么记忆？', yuan_ids)
                    yuan_replies[exchange_id] = yuan_replies.get(exchange_id, '') + '\n' + response_text(resp)
                    yuan_evidences = g.evidences()
                    yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}]
                    if '708' in set(yuan_ids):
