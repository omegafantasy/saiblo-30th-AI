...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        ask_all('袁樱瞳案按身份法医链重建：袁樱瞳、凌晨1点照片女性、lo裙栗色假发、黄色行李箱尸块、世纪林尸块、李海天分别如何比对。')
        ask_all('只查能证明身份互换或死亡时间的物证：DNA、照片EXIF、手机云备份、蓝色背包海豚挂件、尸检原件、行李箱血迹指纹纤维、1919车辆轨迹。')
        ...
        if '705' in yuan_ids:
            for npc_id in target_ids[:12]:
                ask(npc_id, '705尸检已出现。继续比较李海天和袁樱瞳：死状差异、四肢切断、背刺失血、蓝色背包海豚挂件、尸块DNA、照片女尸身份和下一份706/707/708法医物证。', yuan_ids)
        else:
            for npc_id in target_ids[:10]:
                ask(npc_id, '若没有705，请指出谁能证明尸源身份：法医、警方旧案、手机照片来源、行李箱主人、蓝色背包主人、1919车主、保安网站后台。', yuan_ids)
