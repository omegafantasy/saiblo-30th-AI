...
def solve_rose(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    if g.stage < 6:
        g.retry_trigger([
            (xiao, f'{fan_name}最近对你的态度有什么变化？'),
            (xiao, f'{fan_name}是不是突然对你好又突然冷淡？'),
            (xiao, f'{fan_name}态度奇怪在哪里？'),
        ], max_retries=1)
...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        if info_id:
            suspect = cn_name(info_id)
            g.chat(info_id, '请说说案发现场情况，以及你手中的证据。')
            g.chat(info_id, '你是谁？你和死者是什么关系？')
            if g.stage < 2:
                g.chat(info_id, '你手中的手机、聊天记录和随身物品能给我看吗？')
            if g.stage >= 2:
                follow_hint = g.hint()
                ...
                if reception_id:
                    reception_replies: list[str] = []
                    reception_resp = g.chat(reception_id, '请说说聊天记录、宾客到达时间表，以及公馆内的异常发现。')
                    reception_replies.append(str(reception_resp.get('reply') or ''))
                    if g.stage < 3:
                        reception_resp = g.chat(reception_id, 'Joker给你的聊天记录和宾客到达时间表在哪里？请把记录给我。')
                        reception_replies.append(str(reception_resp.get('reply') or ''))
                    if g.stage < 3:
                        reception_resp = g.chat(reception_id, '扑克牌标号、地点和到达时间对应表里有没有梅花5？')
                        reception_replies.append(str(reception_resp.get('reply') or ''))
                    reception_reply = '\n'.join(reception_replies)
                    poker_evidences = g.evidences()
                    doctor_id = ''
                    for npc in follow_npcs:
                        if npc != reception_id and cn_name(npc) in reception_reply and '医生' in reception_reply:
                            doctor_id = npc
                            break
                    if doctor_id:
                        ev_ids = [str(ev.get('id')) for ev in poker_evidences if str(ev.get('id')) in {'102', '203', '204', '205'}]
                        g.chat(doctor_id, '结合尸体伤口、手臂灼烧、血水稀释、电脑自杀伪装记录、塑料盒和三把刀，你有什么专业判断？', ev_ids)
                        g.evidences()
