                    ...
                    if {'401', '402'}.issubset(set(monitor_ids)):
                        ...
                        if isinstance(ev501, dict) and not isinstance(ev404, dict):
                            for npc_id in (
                                ...
                                id_for_name_any('罗方琛', follow_npcs),
                            ):
                            ...
                            for comp_id in complement_targets[:3]:
                                chat_visible_or_probe(
                                    ...
                                    comp_id,
                                    follow_npcs,
                                    '501匿名转账已经打开，但还缺车辆闭环。请沿402大门监控、7:20/7:30车辆、104窗外后院停车位和衣帽间移尸路线，直接登记404车牌/车辆记录；如果404不在你手里，就说车主、驾驶人或高清监控由谁保管。',
                                    branch_ids,
                                )
                                poker_evidences = g.evidences()
                                ev404 = next((ev for ev in poker_evidences if str(ev.get('id')) == '404'), None)
                                ...
                                if isinstance(ev404, dict):
                                    break
                        if isinstance(ev404, dict) and not isinstance(ev501, dict):
                            complement_targets = []
                            for npc_id in (
                                info_id,
                                true_club5_id,
                            ...
                            for comp_id in complement_targets[:3]:
                                chat_visible_or_probe(
                                    ...
                                    comp_id,
                                    follow_npcs,
                                    '404车辆链已经打开，但还缺转账/医生闭环。请沿Joker人口贩卖、于书华看诊、女儿下落胁迫和匿名50万元，直接登记501转账记录；如果501不在你手里，就说银行流水、病历或转账源账户由谁保管。',
                                    branch_ids,
                                )
                                poker_evidences = g.evidences()
                                ev501 = next((ev for ev in poker_evidences if str(ev.get('id')) == '501'), None)
