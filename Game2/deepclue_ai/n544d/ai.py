    ...
    if '扑克公馆' in text:
        ...
        if info_id:
            ...
            if g.stage >= 2:
                ...
                ev_ids = [str(ev.get('id')) for ev in poker_evidences if str(ev.get('id')) in {'101', '102', '103', '104'}]
                if g.stage == 2 and '人口贩卖集团' in response_text(first) and ev_ids:
                    g.chat(info_id, '聊天记录和到达表是谁保管的？', ev_ids[:2])
                    g.evidences()
