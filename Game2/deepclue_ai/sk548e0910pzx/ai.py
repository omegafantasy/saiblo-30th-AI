    ...
    if '扑克公馆' in text:
                ...
                if reception_id and g.stage >= 3:
                    ...
                    if {'401', '402'}.issubset(set(monitor_ids)):
                        ...
                        if isinstance(ev405_open, dict) or isinstance(ev502_open, dict):
                            ...
                            g.chat(info_id, '405或502已经出现。现在继续追下一阶段而不是复述：完整凶手链、真实杀害地点、尸体转运、Joker手机/账号、人口贩卖名单、林渝植失踪卷宗、DNA/指纹和银行流水之后，还有没有406/503或最终物证？直接给编号、证据名和持有人。', open_ids)
        ...
        if isinstance(ev705, dict):
            ...
            if '706' not in set(yuan_ids):
                ...
                hidden_count = 0
                for source_name in ('张子韩', '陆亦初', '楚戎臻', '王泽', '张壹'):
                    source_id = id_for_name_any(source_name, g.npcs() or npcs)
