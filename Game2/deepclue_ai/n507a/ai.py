...
def solve_rose_direct(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    g.answer(
        murderer=murderer,
        motivation=f'{murderer}家族生意和资金处境紧张，急需通过接近并独占{banker}获得地位与财力；又误以为Rose和{banker}关系亲近、会成为情敌和阻碍，因此为扫清情敌、独占{banker}而杀害Rose。',
...
def solve_rose(g: Game, npcs: list[str], marks: dict[str, bool], evidences: list[dict[str, Any]]) -> None:
    ...
    g.answer(
        murderer=cui_name,
        motivation=f'{cui_name}家族生意和资金处境紧张，急需通过接近并独占{deng_name}获得地位与财力；又误以为Rose和{deng_name}关系亲近、会成为情敌和阻碍，因此为扫清情敌、独占{deng_name}而杀害Rose。',
    ...
    if '扑克公馆' in text:
        ...
        for npc in npcs:
            if current_marks.get(npc) is False:
                ...
                break
        motivation = (
            f'{suspect}因与袁樱瞳竞争出国交流名额，且投票多出一票、袁樱瞳准备等到周五揭穿真相，'
            f'担心作弊、手机照片和相关秘密暴露而杀害袁樱瞳。'
        )
        method = (
            f'{suspect}利用自己与袁樱瞳长相相似及黄色行李箱制造替身和时间线混淆，取得并清空袁樱瞳手机，'
            '围绕凌晨1点女性尸体照片、栗色假发和行李转移视线，随后分尸抛尸。'
        )
