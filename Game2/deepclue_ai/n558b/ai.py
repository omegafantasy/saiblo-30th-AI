...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        marked = [ynpc for ynpc in ynpcs if g.marks().get(ynpc) is True]
        unmarked = [ynpc for ynpc in ynpcs if g.marks().get(ynpc) is False]
        teacher = marked[1] if len(marked) > 1 else (marked[0] if marked else (ynpcs[0] if ynpcs else ''))
        competitor = marked[0] if marked else (ynpcs[0] if ynpcs else '')
        witness = unmarked[0] if unmarked else (ynpcs[-1] if ynpcs else '')
        if teacher:
            g.chat(teacher, '课程展示投票的实到人数、应投票数、实际票数、投票箱多出的一票、笔迹不同和一票之差获利者分别是什么？')
            g.evidences()
        yuan_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}]
        if competitor:
            g.chat(competitor, '袁樱瞳手机、凌晨1点女性尸体照片、lo裙栗色假发、黄色行李箱、长相相似和出国名额竞争分别与你有什么关系？', yuan_ids)
            g.evidences()
        yuan_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}]
        if witness:
            g.chat(witness, '生物馆、世纪林尸块、1919黑车、保安奇怪网站和张壹传闻中，你亲眼确认了哪一段？谁从生物馆跑出来？', yuan_ids)
        yuan_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}]
        for ynpc in [teacher, competitor, witness]:
            if ynpc:
                g.chat(ynpc, '结合703手机线索和704投票线索，谁直接获利，袁樱瞳为什么危险，哪条证据能指向关键隐瞒者？', yuan_ids)
