...
def extract_bio_names(text: str) -> list[str]:
    ...
    for pattern in patterns:
        for match in re.finditer(pattern, text):
            ...
            if name in CN_TO_PINYIN and name not in {'张壹'} and name not in names:
                names.append(name)
    return names


def fallback_false_name(npcs: list[str], marks: dict[str, bool]) -> str:
    for npc in npcs:
        if marks.get(npc) is False:
            return cn_name(npc)
    return cn_name(npcs[-1]) if npcs else '无名氏'


def solve_yuan_bio_answer(g: Game, npcs: list[str], marks: dict[str, bool]) -> None:
    g.stage = 0
    replies: list[str] = []
    ...
    ask_all('你对死者袁樱瞳和这起碎尸案了解什么？现场情况、尸体、手机、行李箱、照片、假发、投票和时间线都说一下。')
    ask_all('你和袁樱瞳是什么关系？她最近和谁有矛盾，谁最后见过她，谁可能有动机？')

    ids = [eid for eid in evidence_ids(g.evidences()) if eid in {'703', '704'}]
    ask_all('有人说张壹半夜从生物馆慌张跑出来；你夜跑时实际看到的是谁？生物馆、世纪林、尸块和袁樱瞳死亡有什么关系？', ids)

    bio_names = extract_bio_names('\n'.join(replies))
    suspect = bio_names[-1] if bio_names else fallback_false_name(npcs, marks)
    g.answer(
        suspect,
...
def solve_case(g: Game) -> bool:
    ...
    if kind == 'yuan':
        solve_yuan_bio_answer(g, npcs, marks)
    else:
        zero_answer(g)
    return True
