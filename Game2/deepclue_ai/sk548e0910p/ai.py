...
def rose_motivation_text(murderer: str, banker: str) -> str:
    if (murderer, banker) in ROSE_MOTIVATION_AB_PAIRS:
        return (
            f'{murderer}爱慕并想独占{banker}，认定Rose纠缠{banker}并会长期阻碍自己与{banker}在一起，'
            f'因此预谋投毒并杀害Rose以扫清情敌。'
        )
    return f'{murderer}爱慕并想独占{banker}，认为Rose纠缠{banker}、会阻碍自己与{banker}在一起，因此为扫清情敌而杀害Rose。'
