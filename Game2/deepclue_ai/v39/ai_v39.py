#!/usr/bin/env python3
"""Game2 DeepClue AI v39 — minimal stage-7 probe on top of v37.

Purpose:
- Keep the proven v37 runtime / ordering / answer path intact
- Add only two evidence-backed follow-up questions at case1 stage 7
- Isolate whether those extra prompts can unlock the last hidden score gap

This version deliberately preserves the v37 startup handshake and avoids the
v38 response-introspection changes that likely caused the early runError.
"""
...
def solve_case0(g: Game) -> None:
    ...
    log(f'[v39] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1(g: Game) -> None:
    ...
    if g.stage >= 7:
        g.chat('NPC_D', '你看到E藏在小树林里，是想埋伏F吗？', evidences=['313'])
        g.chat('NPC_C', '有人拿你的水果刀按你的小说手法杀了F，还改了你的文档嫁祸你，对吗？', evidences=['315'])

    log(f'[v39] case1 stage={g.stage} calls={g.calls}')
    result = g.submit(
        murderer='E',
...
def solve_case(g: Game, case_index: int) -> bool:
    ...
    log(f'[v39] case={case_index} npcs={sorted_npcs}')
    g.stage = 0
    g.calls = 0
...
def main() -> int:
    ...
    while True:
        try:
            ...
            log(f'[v39] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
