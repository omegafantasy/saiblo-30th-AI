...
def solve_z_script(g: Game, npcs: list[str], evidences: list[dict[str, Any]]) -> None:
    ...
    # These legacy post-stage probes can trip the server's stage-8 evidence map.
    # The Z/F chain is already complete here, so skip them instead of risking a
    # harmless but noisy KeyError after the useful evidence is unlocked.
...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
                            ...
                            if '606' in post601_set:
                                ...
                                def follow_verbal_605(npc_id: str, resp: dict[str, Any]) -> None:
                                    ...
                                    if post606_set & {'605', '607', '608'}:
                                        return
                                    verbal_hit = bool(re.search(r'605[^。\n]{0,24}(?:成立|公开摘要|摘要|登记|编号|内容|真相|在我手|交给你|关键物证|破绽记录)|凶手是[^。\n]{1,12}Joker|Joker[^。\n]{0,20}衣帽间[^。\n]{0,20}遇害|编号\s*501\s*到\s*505', reply_text or ''))
                                    custody_hit = bool(re.search(r'照片原件在我这里|保管链完整|605[^。\n]{0,20}在我手|警方[^。\n]{0,16}(?:授权|卷宗|结案)|最终卷宗|内部卷宗|唯一保管人|公开条件|现在还不是公开的时候', reply_text or ''))
                                    challenge_hit = bool(re.search(r'(?:证明|证据)[^。\n]{0,24}(?:谁动手|谁杀|杀了\s*Joker)|如果[^。\n]{0,18}证明[^。\n]{0,18}(?:动手|杀)|真相不能[^。\n]{0,12}掩埋|会告诉[^。\n]{0,8}更多|证据在我手里|时机未到', reply_text or '')) or custody_hit
                                    if not (verbal_hit or challenge_hit):
                                        return
                                    if custody_hit:
                                        question = (
                                            '你刚才已经说明照片原件、605、保管链或警方卷宗的公开条件。现在不要公开607/608内部卷宗，'
                                            '只把可公开的605摘要正式登记：谁约Joker进衣帽间，谁在窗户/后院逃走，'
                                            '死者手机云端约见记录、照片原件、凶器血迹指纹或现场关键物证对应哪一份。'
                                            '如果只能先给公开摘要，也请按物证05/605交出。'
                                        )
                                    elif challenge_hit and not verbal_hit:
                                        question = (
                                    ...
                                    def add_holder_name(name: str) -> None:
                                        name = clean_cn_fragment(name)
                                        if name and name in CN_TO_PINYIN and name not in holder_names:
                                            holder_names.append(name)

                                    for pattern in (
                                        r'唯一保管人是([一-龥]{1,4})',
                                        r'([一-龥]{1,4})掌握[^。\n]{0,24}(?:地下室档案室|终极密钥|最终卷宗|内部卷宗|名册原件)',
                                        r'([一-龥]{1,4})[^。\n]{0,16}(?:保管|持有|掌握)[^。\n]{0,24}(?:607|608|卷宗|名册原件|档案室)',
                                    ):
                                        for match in re.finditer(pattern, reply_text or ''):
                                            add_holder_name(match.group(1))
                                    if re.search(r'警局授权|警方授权|刑警身份|内部卷宗|最终卷宗|调阅权限', reply_text or '') and info_id and info_id != npc_id:
                                        auth_resp = chat_visible_or_probe(
                                            g,
                                            info_id,
                                            follow_npcs,
                                            '你有警方或景观渠道。607/608内部卷宗可以暂不公开，但请先给调阅授权或公开摘要：605警方结案破绽、衣帽间关键物证、死者手机云端约见记录、窗户逃走目击记录中哪一份能正式登记？',
                                            post606_ids,
                                        )
                                        post606_ids, post606_set = refresh_post601()
                                        if post606_set & {'605', '607', '608'}:
                                        ...
                                        for pattern in (
                                            ...
                                            for match in re.finditer(pattern, response_text(auth_resp) or ''):
                                                add_holder_name(match.group(1))
                                    holder_seen: set[str] = set()
                                    for holder_name in holder_names[:2]:
                                        for holder_id in global_name_ids(holder_name, follow_npcs, max_ids=4):
                                            if not holder_id or holder_id == npc_id or holder_id in holder_seen:
                                                continue
                                            holder_seen.add(holder_id)
                                            holder_resp = chat_visible_or_probe(
                                                g,
                                                holder_id,
                                                follow_npcs,
                                                f'你被点名为{holder_name}，掌握地下室档案室、607/608最终卷宗或名册原件。内部卷宗可以不公开，先把可公开的605登记出来：警方结案破绽、衣帽间关键物证、Joker云端约见记录、照片原件、窗户逃走目击记录是哪一份？',
                                                post606_ids,
                                            )
                                            post606_ids, post606_set = refresh_post601()
                                            if post606_set & {'605', '607', '608'}:
                        ...
                        if isinstance(ev405_open, dict) or isinstance(ev502_open, dict):
                            ...
                            if isinstance(ev502_open, dict) and not any(str(ev.get('id')) in {'503', '504'} for ev in poker_evidences):
                                ...
                                bridge_q = (
                                    ...
                                    '请立刻把死者手机/邀请函/随身物链登记出来：503死者特殊邀请函、504刻着LYZ的随身物、'
                                    '林渝植失踪档案、Joker手机云端或能继续打开601-604的旧案材料；先给证据编号和内容。'
                                )
                                for bridge_id in bridge_targets[:4]:
                                    chat_visible_or_probe(g, bridge_id, follow_npcs, bridge_q, open_ids)
                                    poker_evidences = g.evidences()
                                    if any(str(ev.get('id')) in {'503', '504', '505', '601', '602', '603', '604', '606'} for ev in poker_evidences):
                                        break
                            deep_ids = [
                                str(ev.get('id'))
                                for ev in poker_evidences
        ...
        if '707' in set(yuan_ids) and '708' not in set(yuan_ids):
            qbg_ev707 = next((ev for ev in yuan_evidences if str(ev.get('id')) == '707'), None)
            qbg_ev707_text = str(qbg_ev707.get('name', '')) + '\n' + str(qbg_ev707.get('content', '')) if isinstance(qbg_ev707, dict) else ''
            qbg_contact_name = ''
            qbg_exchange_name = ''
            m = re.search(r'物证07：([一-龥]{1,4})的联系方式', qbg_ev707_text)
            if m:
                qbg_contact_name = m.group(1)
            for pattern in (
                r'可用于与([一-龥]{1,4})交换情报',
                r'与([一-龥]{1,4})交换情报',
                r'([一-龥]{1,4})曾表示[^。\n]{0,30}联系方式',
                ...
                m = re.search(pattern, qbg_ev707_text)
                if m:
                    qbg_exchange_name = m.group(1)
                    break
            qbg_contact_targets: list[str] = []
            for npc_id in global_name_ids(qbg_contact_name, g.npcs() or npcs, max_ids=4) if qbg_contact_name else []:
                if npc_id and npc_id not in qbg_contact_targets:
                    qbg_contact_targets.append(npc_id)
            for npc_id in story_target_ids(qbg_ev707_text + '\n' + '\n'.join(yuan_replies.values()), g.npcs() or npcs, max_ids=8):
                if qbg_contact_name and cn_name(npc_id) == qbg_contact_name and npc_id not in qbg_contact_targets:
                    qbg_contact_targets.append(npc_id)
            for contact_id in qbg_contact_targets[:2]:
                resp = chat_visible_or_probe(
                    g,
                    contact_id,
                    g.npcs() or npcs,
                    f'我先只问你本人意见：能不能把你的联系方式转交给{qbg_exchange_name or "那个找你要联系方式的人"}？'
                    f'他通常在哪里能找到，是学生会办公室、电子系楼、体育场还是生物馆附近？'
                    '他上周六晚上为什么从生物馆附近慌张出现，手里有没有手机、U盘、背包或血迹？',
                    yuan_ids,
                )
        ...
        if '707' in set(yuan_ids) and '708' not in set(yuan_ids):
            ...
            def add_concrete_target(npc_id: str) -> None:
                if npc_id and npc_id not in concrete_targets:
                    concrete_targets.append(npc_id)

            def concrete_fact_score(reply: str) -> int:
                text_reply = str(reply or '')
                keys = (
                    '生物馆', '慌张', '跑出来', '22:30', '晚上十点半',
                    '1919', '黑车', '奇怪网站', '奇怪的网站', '周日', '离岗',
                    '保安', '世纪林',
                )
                return sum(1 for key in keys if key in text_reply)

            for npc_id, reply in sorted(yuan_replies.items(), key=lambda item: concrete_fact_score(item[1]), reverse=True):
                if concrete_fact_score(reply) >= 3:
                    add_concrete_target(npc_id)
            for npc_id in story_target_ids('\n'.join(yuan_replies.values()), g.npcs() or npcs, max_ids=6):
                add_concrete_target(npc_id)
            for npc_id in false_ids:
                add_concrete_target(npc_id)
            for npc_id in current_npcs:
                add_concrete_target(npc_id)
            concrete_q = (
                '先别谈交换、授权或证据编号，只核对你亲眼见过的三件事实：'
                '上周六晚上十点半谁从生物馆慌张跑出来，他脸色和手里东西有什么异常；'
                '袁樱瞳坐过的车牌尾号1919黑车是谁安排、从哪里来往；'
                '保安看的奇怪网站、有人靠近就熄屏、周日离岗和世纪林尸块之间有什么联系。'
                '如果这些事实指向同一个人或同一条路线，请直接说人名、地点和经过。'
            )
            for target_id in concrete_targets[:3]:
                resp = chat_visible_or_probe(g, target_id, g.npcs() or npcs, concrete_q, yuan_ids)
