...
def solve_unknown(g: Game, npcs: list[str], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        info = id_for_name(m.group(1).strip(), npcs) if m else ''
        if info:
            g.stage = 0
            g.calls = 0
            g.chat(info, '你为什么是好的信息来源？你发现了哪些别人不知道的线索？')
            g.chat(info, '你是谁？你和死者是什么关系？')
            next_hint = g.hint()
            next_npcs = g.npcs()
        ...
        method = f'{suspect}利用扑克公馆全员戴面具、身份混淆和密室结构，提前准备厨房丢失的三把刀，在衣帽间刺杀戴梅花5面具的死者，并处理面具、门窗、血水和双臂灼烧痕迹来伪造现场。'
        motivation = f'{suspect}为隐藏扑克公馆聚会中的真实身份、梅花5面具秘密以及自己掌握的现场线索，担心身份混淆和证词破绽暴露而杀人灭口。'
