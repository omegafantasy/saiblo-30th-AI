#!/usr/bin/env python3
"""Game2 DeepClue AI v46 — reverse pre-answer hidden-question probe.

Purpose:
- Preserve the proven v43 scripted path exactly through both cases
- In case1, after reaching stage 7 and before `answer`, inject the same two
  hidden questions as v43 but reverse their order to `NPC_D -> NPC_C`
- Verify whether the full 2357 / stage-8 trigger depends on set-membership
  alone or on the original question order
...
def solve_case0(g: Game) -> None:
    ...
    log(f'[v46] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1(g: Game) -> None:
    ...
    # Same hidden pair as v43, but in reversed order to isolate ordering.
    for npc, question in [
        ('NPC_D', 'F的QQ里面还有什么信息？'),
        ('NPC_C', '你是色盲？'),
    ]:
        ...
        if isinstance(resp, dict):
            log(
                f"[v46] pre-answer probe [{npc}] stage={resp.get('stage')} "
                f"keys={sorted(resp.keys())} error={resp.get('error')!r}"
            )
        else:
            log(f'[v46] pre-answer probe [{npc}] non-dict={type(resp).__name__}')

    log(f'[v46] case1 stage={g.stage} calls={g.calls}')
    result = g.submit(
        murderer='E',
    ...
    log(f'[v46] case1 submit_keys={sorted(result.keys()) if isinstance(result, dict) else []}')
...
def solve_case(g: Game, case_index: int) -> bool:
    ...
    log(f'[v46] case={case_index} npcs={sorted_npcs}')
    g.stage = 0
    g.calls = 0
...
def main() -> int:
    ...
    while True:
        try:
            ...
            log(f'[v46] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
