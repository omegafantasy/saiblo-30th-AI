...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
                        ...
                        if post601_set & {'601', '602', '603', '604'}:
                            ...
                            if '606' in post601_set:
                                ...
                                for npc_id, question in post606_questions:
                                    ...
                                    chat_visible_or_probe(g, npc_id, follow_npcs, question, post606_ids)
                                    post606_ids, post606_set = refresh_post601()
                                ...
                                if not (post606_set & {'605', '607', '608'}):
                                    for npc_id in post606_targets[:8]:
                                        if npc_id in asked_post606:
                                            continue
                                        asked_post606.add(npc_id)
                                        chat_visible_or_probe(g, npc_id, follow_npcs, '606已出现，现在只推进下一门槛：请用可核验证据证明谁杀了Joker。目标证据是605现场未交物证、607/608最终卷宗、POKER组织成员名册、Joker人口贩卖名单、林渝植亲子鉴定、刀具血迹或手机云端记录；你持有哪一项就直接交出编号和内容。', post606_ids)
                                        post606_ids, post606_set = refresh_post601()
                                        if post606_set & {'605', '607', '608'}:
                                            break
                        late_ids = [
                        ...
                        if isinstance(ev405_open, dict) or isinstance(ev502_open, dict):
                            ...
                            if deep_id_set & {'503', '504', '505', '601', '602', '603', '604'}:
                                ...
                                if '606' in deep_id_set and not ({'605', '607', '608'} & deep_id_set):
                                    for npc_id in deep_targets[:6]:
                                        chat_visible_or_probe(g, npc_id, follow_npcs, '606三人POKER纹身照片已经出现。不要继续泛谈组织，按NPC刚给出的条件证明谁杀Joker：真正梅花5/林渝植的7:30-8:20时间线、Joker伪装梅花5、503/504/502威胁控制链、刀具血迹和移尸路线。若证明成立，请交出605、607/608、组织名册或警方结案卷宗。', deep_ids)
                                        _, deep_ids, deep_id_set = refresh_deep_ids()
        ...
        yuan_evidence_keys = {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706', '707', '708'}
        yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in yuan_evidence_keys]

        def refresh_yuan_state(extra_keys: set[str] | None = None) -> set[str]:
            nonlocal yuan_evidences, yuan_ids
            wanted = set(yuan_evidence_keys)
            if extra_keys:
                wanted.update(extra_keys)
            yuan_evidences = g.evidences()
            yuan_ids = [str(ev.get('id')) for ev in yuan_evidences if str(ev.get('id')) in wanted]
            return set(yuan_ids)

        def post707_formal_exchange() -> None:
            yuan_set = refresh_yuan_state()
            if '707' not in yuan_set or '708' in yuan_set:
                return
            ...
            if not contact_match:
                contact_match = re.search(r'([一-龥]{1,4})主动提供的个人联系方式', ev707_text)
            contact_name = contact_match.group(1) if contact_match else ''

            exchange_names: list[str] = []

            def add_exchange_name(name: str) -> None:
                name = clean_cn_fragment(name)
                if name and name not in exchange_names and name != contact_name:
            ...
            for pattern in (
                ...
                for exchange_name in re.findall(pattern, ev707_text + '\n' + combined_replies):
                    add_exchange_name(exchange_name)
            exchange_name_hint = exchange_names[0] if exchange_names else ''

            def add_unique(items: list[str], npc_id: str) -> None:
                if npc_id and npc_id not in items:
                    items.append(npc_id)

            def contact_targets() -> list[str]:
                visible = g.npcs() or current_npcs or npcs
                targets: list[str] = []
                if contact_name:
                    for npc_id in global_name_ids(contact_name, visible):
                        add_unique(targets, npc_id)
                return targets

            def exchange_targets() -> list[str]:
                visible = g.npcs() or current_npcs or npcs
                targets: list[str] = []
                for name in exchange_names:
                    for npc_id in global_name_ids(name, visible):
                        add_unique(targets, npc_id)
                for npc_id in story_target_ids(ev707_text + '\n' + '\n'.join(yuan_replies.values()), visible, max_ids=10):
                    add_unique(targets, npc_id)
                return targets

            def remember(npc_id: str, resp: dict[str, Any]) -> None:
                if npc_id:
                    yuan_replies[npc_id] = yuan_replies.get(npc_id, '') + '\n' + response_text(resp)

            for contact_id in contact_targets()[:4]:
                resp = chat_visible_or_probe(
                    g,
                    contact_id,
                    g.npcs() or current_npcs or npcs,
                    f'707写的是你的联系方式，{exchange_name_hint or "那个要联系方式的人"}想拿到它来交换杀手秘密。你不用参与转交或见证，只确认一件事：我能不能把你的号码交给{exchange_name_hint or "他/她"}，让{exchange_name_hint or "他/她"}打给你并兑现秘密？如果可以，请直接说“把我的号码给他/她”。',
                    yuan_ids,
                )
                remember(contact_id, resp)
                if '708' in refresh_yuan_state():
                    return

            guide_targets: list[str] = []
            for npc_id in contact_targets():
                add_unique(guide_targets, npc_id)
            for npc_id in exchange_targets():
                add_unique(guide_targets, npc_id)
            for npc_id in story_target_ids(ev707_text + '\n' + '\n'.join(yuan_replies.values()), g.npcs() or current_npcs or npcs, max_ids=10):
                add_unique(guide_targets, npc_id)
            for npc_id in (g.npcs() or current_npcs or npcs):
                add_unique(guide_targets, npc_id)
            for guide_id in guide_targets[:8]:
                resp = chat_visible_or_probe(
                    g,
                    guide_id,
                    g.npcs() or current_npcs or npcs,
                    f'现在不再谈流程。请立刻带我去{exchange_name_hint or "交换对象"}所在地点，优先去学生会办公室、电子系大楼、体育场、宿舍或宣传部；如果他/她不在，请当场打电话或发消息把{exchange_name_hint or "交换对象"}叫来当前调查，并确认{contact_name or "运动少女"}联系方式已经交到本人手上。',
                    yuan_ids,
                )
                remember(guide_id, resp)
                if '708' in refresh_yuan_state():
                    return
                if exchange_name_hint:
                    refreshed_visible = g.npcs() or current_npcs or npcs
                    if any(npc_id in set(refreshed_visible) for npc_id in exchange_targets()):
                        break

            transfer_targets: list[str] = []
            for npc_id in exchange_targets():
                add_unique(transfer_targets, npc_id)
            for name in ('王泽', '江沐青', '许清和', '张子韩', '赵一橙', '陆亦初', '沈知遥', '叶青衡', '周林君', '楚戎臻'):
                for npc_id in global_name_ids(name, g.npcs() or current_npcs or npcs):
                    add_unique(transfer_targets, npc_id)
            for npc_id in story_target_ids(ev707_text + '\n' + '\n'.join(yuan_replies.values()), g.npcs() or current_npcs or npcs, max_ids=12):
                add_unique(transfer_targets, npc_id)
            for npc_id in (g.npcs() or current_npcs or npcs):
                add_unique(transfer_targets, npc_id)

            for target_id in transfer_targets[:10]:
                resp = chat_visible_or_probe(
                    g,
                    target_id,
                    g.npcs() or current_npcs or npcs,
                    f'707现在只差正式转交流程。{contact_name or "运动少女"}已主动提供或授权联系方式，{exchange_name_hint or "交换对象"}承诺拿到联系方式就说杀手秘密。请你以学生会办公室、宣传部、体育部、电子系或保卫处正式渠道完成转交，确认{exchange_name_hint or "交换对象"}已经收到，并带我去找他兑现物证08/708；若你不是负责人，只说负责人中文名和地点。',
                    yuan_ids,
                )
                remember(target_id, resp)
                if '708' in refresh_yuan_state():
                    return

            for exchange_id in exchange_targets()[:8]:
                resp = chat_visible_or_probe(
                    g,
                    exchange_id,
                    g.npcs() or current_npcs or npcs,
                    f'{contact_name or "运动少女"}本人已经授权，联系方式也已通过正式渠道转交给你，交换条件完成。现在请兑现关于杀手的秘密：王泽坦白了什么，雇主邮件、白色浴缸、戴手套握刀、死亡照片对应的物证08/708在哪里？',
                    yuan_ids,
                )
                remember(exchange_id, resp)
                if '708' in refresh_yuan_state():
                    return

            for wang_id in global_name_ids('王泽', g.npcs() or current_npcs or npcs):
                resp = chat_visible_or_probe(
                    g,
                    wang_id,
                    g.npcs() or current_npcs or npcs,
                    '707联系方式交换链已经完成。你现在只回答坦白内容：谁雇佣杀手，邮件账号/IP和付款记录在哪里，沾血白色浴缸是哪间房，手套和刀具是谁处理的；如果这就是物证08/708，请直接交出。',
                    yuan_ids,
                )
                remember(wang_id, resp)
                if '708' in refresh_yuan_state():
                    return

            ask_all(
                f'707已经出现，不要再讨论隐私能不能给。请按正式调查协助完成三件事：确认{contact_name or "运动少女"}联系方式已转交给{exchange_name_hint or "交换对象"}，指出学生会/电子系/体育部/保卫处谁负责带路或见证，让交换对象兑现杀手秘密和王泽坦白内容；若仍缺条件，只说缺哪份官方材料和708由谁保管。',
                yuan_ids,
            )
            refresh_yuan_state()
