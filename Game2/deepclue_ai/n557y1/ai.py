...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        targeted = [
            ynpc for ynpc, reply in yuan_replies.items()
            if any(key in reply for key in ('生物馆', '世纪林', '1919', '保安', '李海天', '张壹', '行李箱', '投票'))
        ]
        if not targeted:
            targeted = list(g.npcs() or npcs)
        for ynpc in targeted[:2]:
            g.chat(ynpc, '只核对你亲自掌握的事实：谁从生物馆出来，谁处理手机或黄色行李箱，谁因投票或出国名额获利，哪条证据能指向凶手？', yuan_ids)
