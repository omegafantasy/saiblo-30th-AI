...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        if info_id:
            ...
            if g.stage >= 2:
                ...
                if 'poker_after_ids' in locals():
                    ...
                    if {'601', '602', '603', '604'} & set(n601_ids) or '606' in n601_ids:
                        for npc_id in n601_targets[:18]:
                            n601_ask(npc_id, '601-604或606若已出现，直接进入最终指控核验：Joker周克被谁约进衣帽间，谁亲手杀死Joker，606三人照片中于书华和红桃Q/联络人分别承担什么角色；请给可公开的605摘要、供述或手机云端约见记录。')
                        n601_ids = [
                            str(ev.get('id')) for ev in g.evidences()
                            if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404', '501', '502', '503', '504', '601', '602', '603', '604', '605', '606', '607', '608'}
                        ]
                        for npc_id in n601_targets[:18]:
                            n601_ask(npc_id, '若606三人照片已出现，现在我只要求兑现最终指控后的公开材料：谁诱导Joker周克进入衣帽间、凶手是否为照片中的POKER成员、凶手供述/约见聊天/手机云端/现场原件哪一份能作为605公开登记。')
                        n617_ids = [
                            str(ev.get('id')) for ev in g.evidences()
                            if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404', '501', '502', '503', '504', '505', '601', '602', '603', '604', '605', '606', '607', '608'}
                        ]
                        if '606' in set(n617_ids) and not ({'605', '607', '608'} & set(n617_ids)):
                            for npc_id in n601_targets[:10]:
                                n601_ask(npc_id, '最终指控已经明确：请不要再说时机未到，只确认这份605公开摘要是否成立。摘要应包含凶手姓名、Joker周克约见记录、杀害地点、凶器/血迹/指纹或照片原件编号；若你不能交出，请说唯一保管人。')
                                n617_ids = [
                                    str(ev.get('id')) for ev in g.evidences()
                                    if str(ev.get('id')) in {'101', '102', '103', '104', '201', '202', '203', '204', '205', '401', '402', '404', '501', '502', '503', '504', '505', '601', '602', '603', '604', '605', '606', '607', '608'}
                                ]
                ...
                if g.stage < 3 and ev_ids:
                    ...
                    g.evidences()
        n620_ids = [str(ev.get('id')) for ev in g.evidences()]
        ...
        for ynpc in ordered:
            ask(ynpc, '先查开场身份链，不猜凶手：失忆侦探是谁，眼熟保安为什么认识我，口袋模糊网页截图是什么网站，保安周日缺岗、奇怪网站后台、旧案系统、李海天和袁樱瞳之间由谁保管原始材料。')
        yuan_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '703', '704', '705', '706', '707', '708'}]
        for ynpc in ordered:
            ask(ynpc, '只追网页/保安/身份系统来源：网站后台访问日志、保卫处值班表、警方旧案卷宗、法医DNA、手机云端、1919车辆、生物馆监控、蓝色背包海豚挂件和706/707/708。', yuan_ids)
        ...
        if '705' in yuan_ids:
            for npc_id in targets[:16]:
                ask(npc_id, '705李海天尸检若已出现，继续追网页截图和保安线：报告原件来源、保安认识侦探的原因、奇怪网站后台、李海天/袁樱瞳/照片女尸DNA、蓝色背包主人和下一证据706/707/708。', yuan_ids)
        else:
