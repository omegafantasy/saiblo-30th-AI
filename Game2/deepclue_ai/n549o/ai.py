...
def solve_yuan_probe_corrected(g: Game, npcs: list[str], marks: dict[str, bool]) -> None:
    ...
    replies_by_npc: dict[str, list[str]] = {npc: [] for npc in npcs}

    def ask_all(question: str, ids: list[str] | None = None, targets: list[str] | None = None) -> None:
        nonlocal winner, phone_holder
        for npc in (npcs if targets is None else targets):
            resp = g.chat(npc, question, ids)
            reply = response_text(resp)
            all_replies.append(reply)
            replies_by_npc.setdefault(npc, []).append(reply)
    ...
    for npc in npcs:
        text = '\n'.join(replies_by_npc.get(npc, []))
        is_teacher = any(key in text for key in ('老师', '教授', '副教授', '课程展示', '全班', '投票'))
        has_phone_holder = bool(re.search(r'(楼梯|早上|今早|捡到)[^。！？]{0,30}手机', text))
        is_winner_like = has_phone_holder or any(key in text for key in ('黄色行李箱', '出国名额'))
        is_route_like = any(key in text for key in ('黑色豪车', '1919', '世纪林', '学生会', '普通同学', '夜跑', '生物馆'))
        if is_route_like and not is_teacher and not is_winner_like:
            witness_targets.append(npc)
    targeted = [
        '有人说张壹半夜从生物馆慌张跑出来；你夜跑时实际看到的是谁？生物馆、世纪林、尸块和袁樱瞳死亡有什么关系？',
    ]
    for question in targeted:
        ask_all(question, ids, witness_targets)
