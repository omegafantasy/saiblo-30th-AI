...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        for ynpc in ordered:
            ask_visible(ynpc, '先查开场身份链：失忆侦探是谁、保安为什么眼熟、口袋网页截图来自哪个网站、保安上周日缺岗、奇怪网站后台、世纪林旧案和李海天尸检分别由谁保管。')
        ...
        for ynpc in ordered:
            ask_visible(ynpc, '不要重复投票传闻。只给能打开706/707/708的系统来源：保卫处值班表、网站后台、警方旧案卷宗、法医DNA、手机云端、1919车辆登记、蓝色背包海豚挂件。', yuan_ids)
