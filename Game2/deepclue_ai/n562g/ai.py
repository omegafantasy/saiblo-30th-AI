...
def solve_unknown(g: Game, npcs: list[str], marks: dict[str, bool], hint: str, evidences: list[dict[str, Any]]) -> None:
    ...
    if '扑克公馆' in text:
        ...
        ask_all('请按时间线说明袁樱瞳最后出现、手机被捡到或清空、凌晨1点照片、周五揭穿、尸块发现和1919黑车每一步发生时间。')
        ask_all('谁能操作袁樱瞳手机，谁知道手机照片，谁有时间搬运行李箱或尸块，谁的时间线最矛盾？')
        yuan_ids = [str(ev.get('id')) for ev in g.evidences() if str(ev.get('id')) in {'001', '002', '003', '004', '701', '702', '703', '704', '705', '706'}]
        ask_all('结合现有证据，哪一个时间点能证明死亡时间被伪造？请给出手机、照片、假发、行李箱和尸块的先后顺序。', yuan_ids)
        ask_all('我还缺哪项关键证据才能确认凶手？请指出未公开证词、矛盾证词和可核验物证。', yuan_ids)
