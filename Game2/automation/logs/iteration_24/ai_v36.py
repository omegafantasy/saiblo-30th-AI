#!/usr/bin/env python3
"""Game2 DeepClue AI v36 — Alternate Case 1 path (no multi-evidence).

Hypothesis: different question ORDERING for Case 1 might trigger a different
stage progression path that reaches stage 8.

Key changes from v26:
- Case 0: IDENTICAL to v26 (proven optimal)
- Case 1: Same questions but DIFFERENT ORDERING
  - Present evidence 313 to NPC_E FIRST (before NPC_A)
  - Ask about F's death before establishing relationships
  - Present evidence 311 to NPC_E earlier
  - NO multi-evidence (single evidence per call to avoid backend hang)

v34 hung because of multi-evidence presentations. This version is safe.
"""
...
def solve_case0(g: Game) -> None:
    ...
    if g.stage < 2:
        g.retry_trigger([('BaiJingTing', 'Rose给你的印象怎么样？'), ('BaiJingTing', '你怎么看Rose这个人？')], max_retries=2)
    ...
    if g.stage < 3:
        g.retry_trigger([('YeWenXiao', '你今晚都去了哪些地方？'), ('FanMinMin', '你今晚的行踪说一下？')], max_retries=2)
    ...
    if g.stage < 4:
        g.retry_trigger([('YeWenXiao', 'Rose今天有什么异常吗？'), ('FanMinMin', '你和Rose到底怎么了？'), ('CuiAnYan', '你和邓达岭今天来的时候有没有看到什么？')], max_retries=2)
    ...
    if g.stage < 5:
        g.retry_trigger([('XiaoDingAng', '范敏敏最近态度有变化吗？'), ('DengDaLing', '崔安彦最近有什么异常行为？')], max_retries=2)
    ...
    log(f'[v36] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1_reorder(g: Game) -> None:
    """Case 1: REORDERED investigation — present evidence to E first, different flow.

    The idea: admin's code might process NPCs in a different order.
    By changing when we present evidence and to whom first,
    we might trigger a different stage progression path.
    """
    # Stage 1 — Same content but start with broader coverage
    ...
    if g.stage < 2:
        g.retry_trigger([('NPC_E', 'Z平时是什么样的人？'), ('NPC_E', '你和Z关系好吗？')], max_retries=2)
    ...
    if g.stage < 3:
        g.retry_trigger([('NPC_D', '你和E是什么关系？'), ('NPC_D', '你以前是不是和E在一起过？')], max_retries=2)
    ...
    if g.stage < 4:
        g.retry_trigger([('NPC_B', '你对E有什么感觉？'), ('NPC_B', '你是不是暗恋E？')], max_retries=2)

    # Stage 4 — KEY DIFFERENCE: present evidence to E FIRST, then C
    g.chat('NPC_D', '关于高中那件事，你知道是谁造谣的吗？')

    # Present autopsy to E FIRST (instead of A first in v26)
    ...
    # Stage 5 — present knife evidence, continue investigation
    ...
    g.chat('NPC_A', '红U盘去哪了？')
    if g.stage < 6:
        g.retry_trigger([('NPC_A', '红U盘去哪了？'), ('NPC_A', 'F的U盘你知道在哪吗？')], max_retries=2)
    ...
    g.chat('NPC_D', '你为什么装病？')
    if g.stage < 7:
        g.retry_trigger([('NPC_D', '你为什么装病？'), ('NPC_D', '你的脚到底有没有受伤？'), ('NPC_D', '你是不是在骗我们？')], max_retries=2)
    ...
    log(f'[v36] case1 stage={g.stage} calls={g.calls}')
    result = g.submit(
        murderer='NPC_E',
...
def solve_case(g: Game, case_index: int) -> bool:
    ...
    log(f'[v36] case={case_index} npcs={sorted_npcs}')
    g.stage = 0
    g.calls = 0
    if sorted_npcs == CASE0_NPCS:
        solve_case0(g)
    elif sorted_npcs == CASE1_NPCS:
        solve_case1_reorder(g)
    else:
