...
def solve_case1_campus(sdk: SDK, kb: KnowledgeBase) -> None:
    """Semi-scripted investigation for Case 1 (campus mystery).

    Focuses on NPC_E as the murderer (confirmed: E killed F using C's fruit knife
    following C's novel method, disfigured F's face, and buried the body in a grove).
    Motivation: F spread rumors about E having an affair on the campus confession wall,
    and reported to Z's parents causing Z to flee school. Old and new grudges combined.
    """
    # --- Phase 1: Generic opening for all 5 NPCs ---
    if kb.stage >= 8:
        log('[game2-v14] case1 skip phase1: stage >= 8')
        return
    phase1: list[dict[str, Any]] = []
    for npc in CASE1_ORDER:
        phase1.append({'npc': npc, 'question': '请介绍一下你自己'})
        phase1.append({'npc': npc, 'question': '发生了什么事？'})
    run_scripted_phase(sdk, kb, phase1)

    # --- Phase 2: Broad investigation - ask ALL NPCs about Z ---
    if kb.stage >= 8:
        log('[game2-v14] case1 skip phase2: stage >= 8')
        return
    phase2: list[dict[str, Any]] = []
    for npc in CASE1_ORDER:
        phase2.append({'npc': npc, 'question': '你知道Z失踪了吗？'})
        phase2.append({'npc': npc, 'question': '你了解平时的Z吗？'})
    run_scripted_phase(sdk, kb, phase2)

    # --- Phase 3: NPC_C focused - critical for method clues (fruit knife + novel technique) ---
    if kb.stage >= 8:
        log('[game2-v14] case1 skip phase3: stage >= 8')
        return
    phase3: list[dict[str, Any]] = [
        {'npc': 'NPC_C', 'question': '你平时写什么类型的小说？'},
        {'npc': 'NPC_C', 'question': '你的水果刀在哪里？'},
        {'npc': 'NPC_C', 'question': '你的刀有没有丢过？'},
        {'npc': 'NPC_C', 'question': 'F的面部伤是怎么回事？'},
    ]
    run_scripted_phase(sdk, kb, phase3)

    # --- Phase 4: NPC_E and NPC_D ---
    if kb.stage >= 8:
        log('[game2-v14] case1 skip phase4: stage >= 8')
        return
    phase4: list[dict[str, Any]] = [
        {'npc': 'NPC_E', 'question': '你和F是什么关系？'},
        {'npc': 'NPC_E', 'question': 'F是不是在表白墙造过谣？'},
        {'npc': 'NPC_E', 'question': '你知道F对Z做了什么吗？'},
        {'npc': 'NPC_D', 'question': '你对NPC_E怎么看？'},
        {'npc': 'NPC_D', 'question': 'F失联这件事你怎么看？'},
        {'npc': 'NPC_D', 'question': '小树林那边你去过吗？'},
    ]
    run_scripted_phase(sdk, kb, phase4)

    # --- Phase 5: NPC_A and NPC_B ---
    if kb.stage >= 8:
        log('[game2-v14] case1 skip phase5: stage >= 8')
        return
    phase5: list[dict[str, Any]] = [
        {'npc': 'NPC_A', 'question': '你觉得谁最可疑？'},
        {'npc': 'NPC_A', 'question': '有没有什么你一直没说的事？'},
        {'npc': 'NPC_B', 'question': '你觉得谁最可疑？'},
        {'npc': 'NPC_B', 'question': 'F和NPC_E之间有什么恩怨？'},
    ]
    run_scripted_phase(sdk, kb, phase5)

    # --- Optional targeted sweep if marks still active and stage < 8 ---
    if kb.stage >= 8:
        log('[game2-v14] case1 skip targeted_sweep: stage >= 8')
        return
...
def case1_final_answer(sdk: SDK, kb: KnowledgeBase) -> dict[str, str]:
    """LLM-based final answer for Case 1 with strong NPC_E guidance."""
    allowed = allowed_murderers(kb)
    prompt = {
        ...
        'requirements': [
            ...
            '根据大量历史数据分析，NPC_E 是最可疑的嫌疑人。除非有极其明确的证据指向其他人，否则优先选择 NPC_E 作为凶手。',
            '动机提示：NPC_E的动机可能涉及表白墙造谣、出轨传言、Z被迫逃离学校、新仇旧恨——F曾在表白墙散布E出轨的谣言，还向Z父母举报导致Z出走。',
            '手法提示：手法可能涉及水果刀(来自NPC_C)、小说中描述的方式、面部破坏、小树林埋尸——E跟踪F到小树林，趁F返回取埋尸工具时将其击晕，用C的水果刀按小说手法毁坏F面部，将尸体埋入F自己挖好的坑中。',
            '不要选择名字中含有Z的人作为凶手，不要因为U盘或校医院的线索就改变凶手判断。',
            '不要输出额外说明',
        ],
    }
    resp = sdk.call_llm(
...
def final_answer(sdk: SDK, kb: KnowledgeBase) -> dict[str, str]:
    ...
    if is_case1_campus(kb):
        return case1_final_answer(sdk, kb)
    allowed = allowed_murderers(kb)
    prompt = {
        'background': kb.background,
        'hint': kb.hint,
...
def solve_case(sdk: SDK, case_index: int) -> bool:
    ...
    if not kb.background and not kb.npcs:
        return False
    log(f'[game2-v14] case={case_index} stage={kb.stage} npcs={len(kb.npcs)}')

    if is_case0_rose(kb):
        solve_case0_rose(sdk, kb)
    elif is_case1_campus(kb):
        solve_case1_campus(sdk, kb)
    else:
        generic_sweep(sdk, kb)
        for _ in range(2):
            if kb.stage >= 8:
                log('[game2-v14] generic: stage >= 8, stop investigation')
                break
            current_marks = marks(sdk)
    ...
    log(f'[game2-v14] answer case={case_index} murderer={answer["murderer"]!r}')
    resp = sdk.request('answer', **answer)
    log(f'[game2-v14] answer_result case={case_index} resp={resp}')
    return True
...
def main() -> int:
    ...
    log(f'[game2-v14] welcome={welcome}')
    case_index = 0
    while True:
        try:
            ...
            log(f'[game2-v14] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
