#!/usr/bin/env python3
"""Game2 DeepClue AI v30 — Remove get_npcs() overhead (micro-optimization).

v26-v29 all score 2220. Score gap to admin (2357) is structural:
- Stage 7 vs 8 (player identity privilege): ~50 pts
- Efficiency coefficient difference: ~87 pts

Both v26/v27 have IDENTICAL 9120 chars vs admin, yet different efficiency.
Hypothesis: efficiency formula includes non-chat API token overhead.
v30 removes 3 get_npcs() calls to minimize non-chat API calls:
- solve_case(): no longer calls get_npcs() (uses case_index directly)
- solve_case0(): no longer calls get_npcs() for XiaoDingGang (always asks)
"""
...
def solve_case0(g: Game) -> None:
    ...
    log(f'[v30] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1(g: Game) -> None:
    ...
    log(f'[v30] case1 stage={g.stage} calls={g.calls}')
    result = g.submit(
        murderer='E',
    ...
    log(f'[v30] case1 result={result}')


def solve_case(g: Game, case_index: int) -> bool:
    # Game always sends Case 0 (Rose/6 NPCs) first, Case 1 (campus/5 NPCs) second
    # No get_npcs() needed — use case_index directly
    log(f'[v30] case={case_index}')
    g.stage = 0
    g.calls = 0
    ...
    if case_index == 0:
        ...
        log(f'[v30] unexpected case_index={case_index}')
        return False
    return True
...
def main() -> int:
    ...
    while True:
        try:
            ...
            log(f'[v30] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
