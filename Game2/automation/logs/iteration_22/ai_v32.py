#!/usr/bin/env python3
"""Game2 DeepClue AI v32 — Filler removal + testimony hunting for 502/601/606.

Strategy:
1. Remove 9 identified filler steps that don't unlock testimonies
2. Add targeted questions to try unlocking missing testimonies 502/601/606
3. Try presenting unused evidence combinations (312, 314, initial 001-008)
4. Try multi-evidence presentations

Goal: Same stages (6/7) but better efficiency + potentially more clue proportion.
"""
    ...
    def chat(self, npc: str, question: str, evidences: list[str] | None = None) -> bool:
        ...
        if isinstance(resp, dict):
            ...
            if achs:
                ...
                self.achievements_found.extend(achs)
            # Check stage
            s = resp.get('stage')
            if s is not None:
                try:
...
def solve_case0(g: Game) -> None:
    """Case 0 (Rose): Optimized — remove 7 filler steps, keep all testimony-unlocking ones."""
    # Stage 1: all 4 unlock testimonies
    ...
    # Stage 2: all unlock testimonies
    ...
    # Stage 3: evidence presentation + setup
    ...
    # REMOVED FILLER: FanMinMin 你和Rose长得像？ (no testimony)
    g.chat('XiaoDingAng', '你和范敏敏什么关系？')
    # REMOVED FILLER: CuiAnYan 家里生意不好？ (no testimony)
    ...
    # REMOVED 4 FILLER STEPS after stage 6 that don't unlock testimonies:
    # FanMinMin 你对萧定昂好？
    # YeWenXiao 邓达岭喜欢你？
    # FanMinMin 对萧定昂冷淡？
    # FanMinMin 是不是你代替Rose上台？

    # Stage 6 — keep only testimony-unlocking steps
    g.chat('DengDaLing', '18:30和叶文潇在舞台右侧见面？')
    g.chat('BaiJingTing', '18:30和崔安彦在一起？')
    g.chat('XiaoDingAng', '19:05你在干什么？')

    # XiaoDingGang (appears at stage 6)
    ...
    # REMOVED FILLER: DengDaLing Rose是不是威胁你？ (no testimony, only new evidence)

    log(f'[v32] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1(g: Game) -> None:
    """Case 1 (campus): Same as v26 but remove 2 filler steps + add testimony hunting."""
    # Stage 1
    ...
    # Stage 5 — evidence presentation
    g.chat('NPC_C', '为什么你的水果刀会在现场？', evidences=['315'])
    g.chat('NPC_B', '你最后一次见F是什么时候？', evidences=['313'])
    g.chat('NPC_E', '关于F的死你知道什么？', evidences=['313'])
    # REMOVED FILLER: NPC_E 你帮Z躲起来了对吧？ evid=['311'] (no testimony unlock)

    # TESTIMONY HUNTING: Try to unlock 502 with new question+evidence combos
    g.chat('NPC_E', '你知道Z现在在哪里吗？', evidences=['311'])  # diary + Z's location
    g.chat('NPC_D', '你昨晚去小树林了吗？', evidences=['313', '314'])  # autopsy + cola bottle
    ...
    # REMOVED FILLER: NPC_D 你是同性恋吗？ (no testimony)

    # TESTIMONY HUNTING: Try to unlock 601/606 with evidence combos
    g.chat('NPC_D', '你对E还有感情吗？', evidences=['311'])  # alternate question for D
    g.chat('NPC_C', '这张地图是谁画的？', evidences=['312'])  # map evidence
    g.chat('NPC_B', '你知道这个可乐瓶子是谁的吗？', evidences=['314'])  # cola bottle
    ...
    log(f'[v32] case1 stage={g.stage} calls={g.calls} achievements={g.achievements_found}')
    result = g.submit(
        murderer='NPC_E',
...
def solve_case(g: Game, case_index: int) -> bool:
    ...
    log(f'[v32] case={case_index} npcs={sorted_npcs}')
    g.stage = 0
    g.calls = 0
...
def main() -> int:
    ...
    while True:
        try:
            ...
            log(f'[v32] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
