#!/usr/bin/env python3
"""Game2 DeepClue AI v28 — Achievement probe: unused evidences 312+314.

v27 result: 2220 (stage 7 Case 1, 0 achievements). Traces identical to admin.
Stage 8 is structural (tied to player identity), cannot be fixed in code.

Score gap analysis: 2357 - 2220 = 137 points.
Key discovery: There are 3 MISSING testimonies (502, 601, 606) that neither
admin nor v27 unlock. These likely require evidences 312 (mysterious map)
and 314 (empty cola bottle) which admin never presents.

Hypothesis: 502/601/606 are achievement-type testimonies.
If unlocked, 3 achievements × 40 × efficiency ≈ 137 pts → matches admin score!

Strategy:
- Keep identical main sequence from v27 (proven optimal)
- Add 4 bonus questions after stage 5, using evidences 312 and 314
- Target: unlock testimonies 502, 601, 606

Evidence map (Case 1):
- 312: 神秘的地图 (mysterious map) — never used by admin
- 314: 空可乐瓶子 (empty cola bottle) — never used by admin
- NPC_E helped Z hide → map might show Z's location (→ 502)
- Empty cola bottle at crime scene → who left it? (→ 601 or 606)
"""
...
def solve_case0(g: Game) -> None:
    ...
    log(f'[v28] case0 stage={g.stage} calls={g.calls}')
    g.submit(
        murderer='崔安彦',
...
def solve_case1(g: Game) -> None:
    """Case 1 (campus): Mirror admin's evidence-presentation + achievement probe."""
    # Stage 1
    ...
    # === ACHIEVEMENT PROBE: present unused evidences 312 (map) and 314 (cola bottle) ===
    # These are never presented by admin but might unlock testimonies 502, 601, 606
    # Hypothesis: 3 achievement testimonies × 40 × efficiency ≈ 137 pts (closes admin gap)
    g.chat('NPC_E', 'Z藏在这张地图标记的位置吗？', evidences=['312'])
    g.chat('NPC_D', '这张地图是你藏在小树林用的吗？', evidences=['312'])
    g.chat('NPC_B', '这个空可乐瓶是谁留在案发现场的？', evidences=['314'])
    g.chat('NPC_D', '这个空可乐瓶子是你留下的吗？', evidences=['314'])
    ...
    log(f'[v28] case1 stage={g.stage} calls={g.calls}')
    result = g.submit(
        murderer='E',
...
def solve_case(g: Game, case_index: int) -> bool:
    ...
    log(f'[v28] case={case_index} npcs={sorted_npcs}')
    g.stage = 0
    g.calls = 0
...
def main() -> int:
    ...
    while True:
        try:
            ...
            log(f'[v28] fatal case={case_index} exc={type(exc).__name__}: {exc}')
            try:
