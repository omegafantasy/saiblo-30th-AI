...
private:
    ...
    /**
     * @brief Check if any building-related operation has been added to a player's operations.
     * @return Whether there is any building-related operation in "operations[player_id]".
     */
    bool has_building_related_operation(int player_id) const
    {
        return std::any_of(operations[player_id].begin(), operations[player_id].end(), [](const Operation &op)
                           { return op.is_building_related(); });
    }
    ...
    /**
     * @brief Calculate the global pheromone decay.
     * @see GameInfo::global_pheromone_attenuation for details.
     */
     ...
     * @see GameInfo::update_pheromone_for_ants for details.
     */
    ...
    /**
     * @brief Clear dead and succeeded ants.
     * @see GameInfo::clear_dead_and_succeeded_ants for details.
     */
     ...
     * @see GameInfo::count_down_super_weapons_left_time for details.
     */
     ...
     * @see GameInfo::count_down_super_weapons_cd for details.
     */
     ...
     * @see GameInfo::apply_active_super_weapons for details.
     */
...
public:
    ...
    /**
     * @brief Construct a new Simulator object from a Controller instance. Current game state
     * (held by GameInfo object) and your player ID will be copied. 
     * @param c The Controller instance as data source.
     */
    Simulator(const Controller& c) : info(c.get_info()), self_player_id(c.self_player_id){}
     ...
     */
    bool add_operation_of_player(int player_id, const Operation& op)
    {
        if (!(op.is_building_related() && has_building_related_operation(player_id)) && info.is_operation_valid(player_id, op))
        {
            operations[player_id].emplace_back(op);
            return true;
        }
        return false;
    }
    ...
    void apply_operations_of_player(int player_id)
    {
        ...
        count_down_super_weapons_left_time(player_id);
        ...
        apply_active_super_weapons(player_id);
    }
     ...
     GameState next_round()
    {
        ...
        if (state != GameState::Running)
            return state;
        // 4) Update pheromone
        global_pheromone_attenuation();
        update_pheromone_for_ants();
        // 5) Clear dead and succeeded ants
        clear_dead_and_succeeded_ants();
