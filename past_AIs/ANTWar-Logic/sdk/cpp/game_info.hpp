 ...
 * various types of information such as Entity, Economy, Pheromone, Item and Operation. 
 */
struct GameInfo
{
    ...
    GameInfo(unsigned long long seed)
        : round(0), coins{COIN_INIT, COIN_INIT}, bases{Base(0), Base(1)},
     ...
     * @return Whether the operation is valid (can be applied). 
     */
        ...
        switch (op.type)
        {
            case BuildTower:
                return is_valid_pos(op.arg0, op.arg1)
                       ...
                       && build_tower_cost(tower_num_of_player(player_id)) <= coins[player_id]
            ...
            case UpgradeTower:
            {
                ...
                return t && t.value().player == player_id
                       ...
                       && upgrade_tower_cost(op.arg1) <= coins[player_id]
            ...
            case UseEmergencyEvasion:
                return  is_valid_pos(op.arg0, op.arg1)
                        && use_super_weapon_cost(op.type % 10) <= coins[player_id]
                        && SuperWeapon::cd[player_id][op.type % 10] <= 0;
            case UpgradeGenerationSpeed:
                return bases[player_id].gen_speed_level < 3
                       && upgrade_base_cost(bases[player_id].gen_speed_level) <= coins[player_id];
            case UpgradeGeneratedAnt:
                return bases[player_id].ant_level < 3
                       && upgrade_base_cost(bases[player_id].ant_level) <= coins[player_id];
            default:
                return false;
        }
    }
    ...
    void apply_operation(int player_id, const Operation& op)
    {
        ...
        {
            case BuildTower:
                update_coin(player_id, -build_tower_cost(tower_num_of_player(player_id)));
                build_tower(next_tower_id++, player_id, op.arg0, op.arg1);
                break;
            case UpgradeTower:
                update_coin(player_id, -upgrade_tower_cost(op.arg1));
            ...
            case DowngradeTower:
            {
                ...
                auto t = tower_of_id(op.arg0);
                if (t) // Tower exists -> Downgraded
                    update_coin(player_id, downgrade_tower_income(t.value().type * 10 + 1));
                else // Tower does not exist -> Destroyed
                    update_coin(player_id, destroy_tower_income(tower_num_of_player(player_id) + 1));
                break;
            }
            case UseLightningStorm:
            case UseEmpBlaster:
            case UseDeflector:
            case UseEmergencyEvasion:
                update_coin(player_id, -use_super_weapon_cost(op.type % 10));
                use_super_weapon(static_cast<SuperWeaponType>(op.type % 10), player_id, op.arg0, op.arg1);
                break;
            case UpgradeGenerationSpeed:
                update_coin(player_id, -upgrade_base_cost(bases[player_id].gen_speed_level));
                upgrade_generation_speed(player_id);
                break;
            case UpgradeGeneratedAnt:
                update_coin(player_id, -upgrade_base_cost(bases[player_id].ant_level));
    ...
    void use_super_weapon(SuperWeaponType type, int player, int x, int y)
    {
        ...
        // Reset cd
        SuperWeapon::cd[player][type] = SuperWeaponInfo[type][2];
    }
    ...
    void show() const
    {
     ...
     /**
     * @brief Append current information to file "replay.out".
     */
