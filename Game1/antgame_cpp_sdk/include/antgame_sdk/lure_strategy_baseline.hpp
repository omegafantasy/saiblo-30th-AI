...
inline double downgrade_refund_penalty(double refund) {
    return refund * baseline_lure_config().downgrade_refund_penalty_scale;
}
...
inline double downgrade_operation_penalty(const PublicState &state, int player, const Operation &operation) {
    ...
    if (tower != nullptr && tower->tower_type == TowerType::Sniper) {
        penalty += baseline_lure_config().sniper_downgrade_penalty;
    }
    return penalty;
}
...
inline double behavior_threat_scale(AntBehavior behavior) {
    if (behavior == AntBehavior::Random) {
        return baseline_lure_config().randomized_threat_scale;
    }
    if (behavior == AntBehavior::Bewitched) {
        return baseline_lure_config().bewitched_threat_scale;
    }
    return 1.0;
}
...
inline bool is_lightning_center_candidate(int x, int y) {
    return is_valid_pos(x, y) &&
           hex_distance(kEdge - 1, kEdge - 1, x, y) <= baseline_lure_config().lightning_center_radius;
}
...
inline double c1_build_heuristic(int code) {
    return code == C1 ? baseline_lure_config().c1_build_bonus : 0.0;
}
...
inline const Tower *forced_lure_sell_target(const PublicState &state, int player) {
    ...
    for (const auto &tower : state.towers) {
        ...
        if (nearest > baseline_lure_config().forced_lure_sell_distance) {
            continue;
        }
...
inline std::vector<SinglePlan> generate_base_candidates(const PublicState &state, int player) {
    ...
    const bool can_build_more = base_tower_count < baseline_lure_config().max_non_lure_towers;
    const bool can_expand_existing = base_tower_count <= baseline_lure_config().max_non_lure_towers;
    const bool c1_sniper_ready = c1_has_sniper(state, player);
...
inline double enemy_tower_lightning_damage_score(const PublicState &state, int player, int x, int y) {
    ...
    const int enemy = 1 - player;
    const int strikes = lightning_tower_strikes_within_horizon(baseline_lure_config().lightning_horizon);
    if (strikes <= 0) {
        return 0.0;
    }
    ...
    for (const auto &tower : state.towers) {
        ...
        score += (before - after) * baseline_lure_config().tower_value_weight * baseline_lure_config().lightning_tower_value_ratio;
    }
    return score;
}
...
inline RootPlanSet generate_root_plans(
    ...
    auto add_plan = [&](const std::string &name,
        ...
        if (has_lightning && !combined.empty()) {
            lightning_static_bonus += enemy_super_effect_active(state, player)
                                          ? baseline_lure_config().lightning_enemy_super_bonus
                                          : baseline_lure_config().lightning_no_enemy_super_penalty;
        }
        if (has_lightning) {
    ...
    if (has_base_hold || has_lure_hold) {
        add_plan(
            ...
            no_lightning.name,
            {},
            baseline_lure_config().hold_bonus,
            0.0,
            0.0,
            false,
            baseline_lure_config().long_eval_horizon,
            FollowupAction{});
    }
    ...
    for (const auto &center_plan : lightning_center) {
        add_plan(
            ...
            true,
            baseline_lure_config().lightning_horizon,
            FollowupAction{});
    }
...
inline double worker_threat_score(const PublicState &state, int player) {
    ...
    for (const auto &ant : state.ants) {
        ...
        threat += baseline_lure_config().worker_threat_unit * std::max(ant.hp, 0) / std::max(1, ant.max_hp()) / distance;
    }
    return threat;
}
...
inline double combat_anchor_threat_at(const PublicState &state, int player, int x, int y) {
    ...
    for (const auto &tower : state.towers) {
        ...
        double tower_threat = value * baseline_lure_config().combat_anchor_threat_coin_ratio / distance;
        if (distance <= baseline_lure_config().combat_anchor_ring_distance) {
            tower_threat += value * baseline_lure_config().combat_anchor_ring1_bonus_ratio;
        }
...
inline double combat_threat_at(const PublicState &state, int player, const Ant &ant, int x, int y) {
    ...
    double threat = baseline_lure_config().combat_base_threat_unit / distance;
...
inline double forced_rollout_ant_priority(const rs::DefenseSimulator &simulator, int player, const rs::SearchAnt &ant) {
    ...
    const auto [base_x, base_y] = kPlayerBases[player];
    const int distance = std::max(1, hex_distance(ant.x, ant.y, base_x, base_y));
    double threat = baseline_lure_config().worker_threat_unit * std::max(ant.hp, 0) / std::max(1, ant.max_hp()) / distance;
    if (ant.behavior == AntBehavior::Random) {
        threat *= baseline_lure_config().randomized_threat_scale;
    } else if (ant.behavior == AntBehavior::Bewitched) {
        threat *= baseline_lure_config().bewitched_threat_scale;
    }
    return threat;
}
...
inline RolloutForcedPlan build_first_round_rollout_plan(
    ...
    const int limit = std::min(baseline_lure_config().rollout_forced_ant_limit, rs::kMaxImportantAnts);
...
inline double lightning_counterfactual_bonus(
    ...
    for (const auto &ant : without_lightning.ants) {
        ...
        bonus += std::max(0.0, before_threat - after_threat) * baseline_lure_config().lightning_combat_threat_ratio;
        if (without_shield > 0 && with_shield < without_shield) {
            bonus += baseline_lure_config().lightning_shield_break_bonus;
        }
        const int damage = std::max(0, ant.hp - with_hp);
        bonus += static_cast<double>(damage) * baseline_lure_config().lightning_damage_bonus_per_hp;
        if (with_ant == nullptr) {
            bonus += baseline_lure_config().lightning_kill_bonus;
        }
    }
    return bonus;
}
...
inline double c1_state_bonus(TowerType tower_type, bool transition_phase) {
    ...
    case TowerType::Ice:
    case TowerType::Bewitch:
        return transition_phase ? baseline_lure_config().c1_heavy_side_trans_bonus : baseline_lure_config().c1_heavy_bonus;
    case TowerType::Quick:
    case TowerType::QuickPlus:
    case TowerType::Double:
        return transition_phase ? baseline_lure_config().c1_quick_trans_bonus : 0.0;
    case TowerType::Sniper:
        return transition_phase ? baseline_lure_config().c1_sniper_trans_bonus : 0.0;
    default:
        return 0.0;
    }
}
...
inline double c1_root_bonus(const rs::DefenseSimulator &post_root, int player, double root_coins) {
    ...
    const bool transition_phase =
        root_coins > static_cast<double>(baseline_lure_config().c1_quick_transition_coin_threshold);
...
inline EvalBreakdown evaluate_terminal(const rs::DefenseSimulator &simulator, int player) {
    ...
    out.base_hp_score = out.base_hp_raw * baseline_lure_config().base_hp_weight;
    out.tower_value_raw = tower_full_salvage_value(simulator);
    out.tower_value_score = out.tower_value_raw * baseline_lure_config().tower_value_weight;
    out.money_raw = simulator.coins;
    out.money_score = out.money_raw * baseline_lure_config().money_weight;
...
inline RolloutEvaluation rollout_plan_score(
    ...
    const int mid_horizon = std::max(0, std::min(plan.horizon, baseline_lure_config().mid_eval_horizon));
    int step = 1;
    ...
    out.terminal = combine_eval_breakdowns(mid_eval, terminal_eval, baseline_lure_config().mid_eval_weight);
    const double mid_weight = std::max(0.0, std::min(1.0, baseline_lure_config().mid_eval_weight));
...
inline std::vector<EvaluatedPlan> evaluate_root_plans(
    ...
    for (std::size_t index = 0; index < root_plans.plans.size(); ++index) {
        ...
        const int normal_rollouts = rollout_count > 0 ? rollout_count : std::max(1, baseline_lure_config().rollout_count);
        if (plan.has_lightning) {
            UcbArm arm;
    ...
    if (!lightning_arms.empty() && baseline_lure_config().lightning_ucb_total_rollouts > 0) {
        const int budget = std::max(1, baseline_lure_config().lightning_ucb_total_rollouts);
        ...
        while (total_samples < budget) {
            ...
            for (std::size_t arm_index = 0; arm_index < lightning_arms.size(); ++arm_index) {
                ...
                const double explore =
                    baseline_lure_config().lightning_ucb_exploration *
...
inline std::vector<Operation> decide_lure_strategy(
    ...
    const std::vector<EvaluatedPlan> evaluated =
        evaluate_root_plans(state, defense_root, context.player, serial, baseline_lure_config().rollout_count, root_plans);
