...
inline RootPlanSet generate_root_plans(
    ...
    auto add_plan = [&](const std::string &name,
        ...
        if (has_lightning && !combined.empty()) {
            lightning_static_bonus += enemy_super_effect_active(state, player)
                                          ? v3_lure_config().lightning_enemy_super_bonus
                                          : v3_lure_config().lightning_no_enemy_super_penalty;
        }
        if (has_lightning) {
    ...
    if (has_base_hold || has_lure_hold) {
        add_plan(
            ...
            no_lightning.name,
            {},
            v3_lure_config().hold_bonus,
            0.0,
            0.0,
            false,
            v3_lure_config().long_eval_horizon,
            FollowupAction{});
    }
    ...
    for (const auto &center_plan : lightning_center) {
        add_plan(
            ...
            true,
            v3_lure_config().lightning_horizon,
            FollowupAction{});
    }
...
inline double worker_threat_score(const PublicState &state, int player) {
    ...
    for (const auto &ant : state.ants) {
        ...
        threat += v3_lure_config().worker_threat_unit * std::max(ant.hp, 0) / std::max(1, ant.max_hp()) / distance;
    }
    return threat;
}
...
inline double combat_anchor_threat_at(const PublicState &state, int player, int x, int y) {
    ...
    for (const auto &tower : state.towers) {
        ...
        double tower_threat = value * v3_lure_config().combat_anchor_threat_coin_ratio / distance;
        if (distance <= v3_lure_config().combat_anchor_ring_distance) {
            tower_threat += value * v3_lure_config().combat_anchor_ring1_bonus_ratio;
        }
...
inline double combat_threat_at(const PublicState &state, int player, const Ant &ant, int x, int y) {
    ...
    double threat = v3_lure_config().combat_base_threat_unit / distance;
...
inline double forced_rollout_ant_priority(const rs::DefenseSimulator &simulator, int player, const rs::SearchAnt &ant) {
    ...
    const auto [base_x, base_y] = kPlayerBases[player];
    const int distance = std::max(1, hex_distance(ant.x, ant.y, base_x, base_y));
    double threat = v3_lure_config().worker_threat_unit * std::max(ant.hp, 0) / std::max(1, ant.max_hp()) / distance;
    if (ant.behavior == AntBehavior::Random) {
        threat *= v3_lure_config().randomized_threat_scale;
    } else if (ant.behavior == AntBehavior::Bewitched) {
        threat *= v3_lure_config().bewitched_threat_scale;
    }
    return threat;
}
...
inline RolloutForcedPlan build_first_round_rollout_plan(
    ...
    const int limit = std::min(v3_lure_config().rollout_forced_ant_limit, rs::kMaxImportantAnts);
...
inline double c1_root_bonus(const rs::DefenseSimulator &post_root, int player, double root_coins) {
    ...
    const bool transition_phase =
        root_coins > static_cast<double>(v3_lure_config().c1_quick_transition_coin_threshold);
...
inline EvalBreakdown evaluate_terminal(const rs::DefenseSimulator &simulator, int player) {
    ...
    out.tower_value_score = out.tower_value_raw * v3_lure_config().tower_value_weight;
    out.money_raw = simulator.coins;
    out.money_score = out.money_raw * v3_lure_config().money_weight;
...
inline std::vector<EvaluatedPlan> evaluate_root_plans(
    ...
    for (std::size_t index = 0; index < root_plans.plans.size(); ++index) {
        ...
        const int normal_rollouts = rollout_count > 0 ? rollout_count : std::max(1, v3_lure_config().rollout_count);
        if (plan.has_lightning) {
            UcbArm arm;
    ...
    if (!lightning_arms.empty() && v3_lure_config().lightning_ucb_total_rollouts > 0) {
        const int budget = std::max(1, v3_lure_config().lightning_ucb_total_rollouts);
...
inline OffensiveEvasionChoice choose_offensive_evasion(
    ...
    if (!v3_lure_config().offensive_evasion_enabled) {
        out.reason = "disabled";
        return out;
    }

    const int enemy = 1 - player;
    out.enemy_lightning_cooldown =
        state.weapon_cooldowns[enemy][static_cast<int>(SuperWeaponType::LightningStorm)];
    if (enemy_lightning_active(state, player)) {
        out.reason = "enemy_lightning_active";
        return out;
    }
    if (out.enemy_lightning_cooldown < v3_lure_config().offensive_evasion_min_enemy_lightning_cd) {
        out.reason = "enemy_lightning_cd_too_low";
        return out;
    }
    ...
    if (out.post_action_coins <= v3_lure_config().offensive_evasion_min_post_action_coins) {
        out.reason = "post_action_coins_too_low";
        return out;
    }
    ...
    if (out.worker_count < v3_lure_config().offensive_evasion_min_worker_count) {
