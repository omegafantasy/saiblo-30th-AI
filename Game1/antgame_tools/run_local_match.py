...
def stage_ai(target: str, parent: Path, label: str) -> Path:
    ...
    subprocess.run([str(PACKAGE_AI), target, str(output_dir)], cwd=GAME1_ROOT, check=True)
    return output_dir
...
def main() -> int:
    ...
    ai_choices = ["cpp_heavy_baseline"]
    parser.add_argument("--ai0", default="cpp_heavy_baseline", choices=ai_choices)
    parser.add_argument("--ai1", default="cpp_heavy_baseline", choices=ai_choices)
