...
class AntGameAI {
  public:
    std::vector<Op> decide(const Snapshot &snapshot) {
        ...
        if (auto shield = maybe_panic_shield(snapshot, severe_threat)) {
            return {*shield};
        }
  ...
  private:
    ...
    static std::string slot_group(const SlotInfo &slot) {
        const std::string &code = slot.code;
        if (code.empty()) {
            return "";
        }
        if (code[0] == 'C') {
        ...
        if (code.rfind("LL", 0) == 0) {
            return "left_far";
        }
        if (code.rfind("RR", 0) == 0) {
            return "right_far";
        }
        if (code.rfind("ML", 0) == 0) {
            return "mid_left";
        }
        if (code.rfind("MR", 0) == 0) {
            return "mid_right";
        }
        if (code.rfind("FL", 0) == 0) {
            return "front_left";
        }
    ...
    static int alive_ant_count(const Snapshot &snapshot, int player) {
        int count = 0;
    ...
    static double alive_ant_mass(const Snapshot &snapshot, int player) {
        double mass = 0.0;
        for (const auto &ant : snapshot.ants) {
            if (ant.player == player && is_alive_status(ant.status)) {
                mass += ant_weight(ant);
    ...
    void update_attack_mode(const Snapshot &snapshot) {
        ...
        const double live_edge = alive_ant_mass(snapshot, me) - alive_ant_mass(snapshot, enemy);
        const double kill_edge = (snapshot.die_count[enemy] - snapshot.die_count[me]) * 1.6 + live_edge;
        if (snapshot.round >= 450 && pressure + kill_edge * 0.7 >= threat * 0.85) {
            attack_commit_ = true;
            return;
        }
        if (kill_edge <= -3.0) {
        ...
        if (pressure - threat + kill_edge * 0.5 >= 8.0) {
            attack_commit_ = true;
        } else if (threat - pressure - kill_edge * 0.4 >= 14.0) {
            attack_commit_ = false;
        }
    }
    ...
    std::optional<Op> maybe_panic_shield(const Snapshot &snapshot, bool severe_threat) const {
        const int me = snapshot.player;
        if (!severe_threat) {
            return std::nullopt;
        }
        double best_evasion = -1.0;
        double best_deflector = -1.0;
        std::pair<int, int> evasion_center = {-1, -1};
        std::pair<int, int> deflector_center = {-1, -1};
        for (const auto &ant : snapshot.ants) {
            if (ant.player != me || !is_alive_status(ant.status)) {
                continue;
            }
            const int base_dist = hex_distance(ant.x, ant.y, snapshot.bases[me].x, snapshot.bases[me].y);
            double support = 0.0;
            for (const auto &ally : snapshot.ants) {
                if (ally.player != me || !is_alive_status(ally.status)) {
                    continue;
                }
                const int dist = hex_distance(ant.x, ant.y, ally.x, ally.y);
                if (dist <= 3) {
                    support += 12.0 + ally.level * 6.0;
                }
            }
            double danger = 0.0;
            for (const auto &enemy_ant : snapshot.ants) {
                if (enemy_ant.player == me || !is_alive_status(enemy_ant.status)) {
                    continue;
                }
                const int dist = hex_distance(ant.x, ant.y, enemy_ant.x, enemy_ant.y);
                if (dist <= 5) {
                    danger += std::max(0, 6 - dist) * (12.0 + enemy_ant.level * 4.0);
                }
            }
            const double deflector_score = support + danger * 0.6 - base_dist * 4.0;
            const double evasion_score = support * 0.8 + danger - base_dist * 3.0;
            if (deflector_score > best_deflector) {
                best_deflector = deflector_score;
                deflector_center = {ant.x, ant.y};
            }
            if (evasion_score > best_evasion) {
                best_evasion = evasion_score;
                evasion_center = {ant.x, ant.y};
            }
        }
        if (snapshot.weapon_cooldowns[me][4] == 0 && snapshot.coins[me] >= weapon_cost(kUseEmergencyEvasion) &&
            best_evasion >= 120.0 && evasion_center.first >= 0) {
            return Op{kUseEmergencyEvasion, evasion_center.first, evasion_center.second};
        }
        if (snapshot.weapon_cooldowns[me][3] == 0 && snapshot.coins[me] >= weapon_cost(kUseDeflector) &&
            best_deflector >= 130.0 && deflector_center.first >= 0) {
            return Op{kUseDeflector, deflector_center.first, deflector_center.second};
        }
        return std::nullopt;
    }
    ...
    void append_build_candidates(const Snapshot &snapshot, std::vector<std::vector<Op>> &candidates) const {
        ...
        for (const auto &slot : snapshot.slots) {
            if (slot.tower_id >= 0) {
                ++group_counts[slot_group(slot)];
            }
        }
        std::vector<std::pair<double, SlotInfo>> ranked;
        ranked.reserve(snapshot.slots.size());
        for (const auto &slot : snapshot.slots) {
            if (!slot.build_legal) {
                continue;
            }
            double score = slot.priority * 1.6;
            score += (slot.branch == "heavy" ? 22.0 : slot.branch == "quick" ? 16.0 : 14.0);
            score -= group_counts[slot_group(slot)] * 18.0;
            score -= hex_distance(slot.x, slot.y, snapshot.bases[snapshot.player].x, snapshot.bases[snapshot.player].y) * 1.2;
            ranked.emplace_back(score, slot);
        }
        ...
        for (size_t i = 0; i < ranked.size() && i < 8; ++i) {
            candidates.push_back({Op{kBuildTower, ranked[i].second.x, ranked[i].second.y}});
        }
    }
    ...
    void append_upgrade_candidates(const Snapshot &snapshot, std::vector<std::vector<Op>> &candidates) const {
        ...
        for (const auto &slot : snapshot.slots) {
            ...
            for (int target : preferred_targets(snapshot, slot)) {
                double score = tower_static_value(target) + slot.priority * 0.6;
                score += (slot.branch == "heavy" ? 10.0 : slot.branch == "quick" ? 6.0 : 8.0);
                ranked.emplace_back(score, Op{kUpgradeTower, slot.tower_id, target});
            }
        }
        std::sort(ranked.begin(), ranked.end(), [](const auto &lhs, const auto &rhs) { return lhs.first > rhs.first; });
    ...
    static void apply_operation_abstract(Snapshot &snapshot, const Op &op) {
        ...
        if (op.type == kUseEmpBlaster || op.type == kUseDeflector || op.type == kUseEmergencyEvasion) {
            snapshot.coins[me] -= weapon_cost(op.type);
            return;
        }
    }
    ...
    ScoreBreakdown evaluate_candidate(const Snapshot &snapshot, const std::vector<Op> &ops) const {
        ...
        const double live_edge = alive_ant_mass(sim, me) - alive_ant_mass(sim, enemy);
        score -= threat * 7.0;
        score += pressure * (attack_commit_ ? 5.5 : 3.2);
        score += live_edge * (attack_commit_ ? 3.0 : 1.4);
        ...
        for (const auto &it : group_counts) {
            if (it.second > 1) {
                score -= (it.second - 1) * 14.0;
            }
        }
...
void print_ops(const std::vector<Op> &ops) {
    ...
    for (const auto &op : ops) {
        if (op.type == kBuildTower || op.type == kUseLightningStorm || op.type == kUseEmpBlaster ||
            op.type == kUseDeflector || op.type == kUseEmergencyEvasion) {
