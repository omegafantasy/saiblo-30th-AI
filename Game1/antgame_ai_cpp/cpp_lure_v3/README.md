...
这是 Game1 当前 v3 C++ AI。2026-04-27 之后，`cpp_heavy_baseline` 已被当前 v2 完全覆盖，因此 v2 与 baseline 当前是同一策略冻结点。`cpp_lure_v2` 源码目录和打包目标已经删除。后续进攻性探索应新开 v3，不应直接改 baseline。
...
默认兼容入口 `lure_strategy.hpp` / `lure_strategy_params.hpp` 也已同步为 v2/baseline 口径；旧 `lure_strategy_baseline*.hpp` 兼容层已删除。
...
## 2026-04-27 v3 vs baseline 结果

座位平衡 32 局完整官方对局。结果目录沿用当时的历史命名：
