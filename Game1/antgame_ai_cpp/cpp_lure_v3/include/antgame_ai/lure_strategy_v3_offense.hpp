...
inline OffensivePostActionGate make_offensive_post_action_gate(
    ...
    if (gate.enemy_lightning_cooldown < v3_lure_config().offensive_evasion_min_enemy_lightning_cd) {
    ...
    if (gate.post_action_coins <= v3_lure_config().offensive_evasion_min_post_action_coins) {
...
inline OffensiveEmpChoice choose_offensive_emp(
    ...
    if (!v3_lure_config().offensive_emp_enabled) {
        out.reason = "disabled";
        return out;
    }
    ...
    const int enemy = 1 - player;
    const int trigger_distance = v3_lure_config().offensive_emp_combat_to_top_tower_distance;
