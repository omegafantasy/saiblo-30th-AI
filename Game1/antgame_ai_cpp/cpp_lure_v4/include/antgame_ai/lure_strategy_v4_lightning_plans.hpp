...
#include "antgame_ai/lure_strategy_v4_lure_plans.hpp"
...
inline double enemy_tower_lightning_damage_score(const PublicState &state, int player, int x, int y) {
    ...
    const int strikes = lightning_tower_strikes_within_horizon(v4_lure_config().lightning_horizon);
    if (strikes <= 0) {
        return 0.0;
    }
    ...
    for (const auto &tower : state.towers) {
        ...
        score += (before - after) * v4_lure_config().tower_value_weight * v4_lure_config().lightning_tower_value_ratio;
    }
    return score;
}
