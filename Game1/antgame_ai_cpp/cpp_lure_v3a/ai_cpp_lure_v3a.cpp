...
constexpr int kAggressiveAntUpgradeCoinThreshold = 200;
...
bool should_upgrade_generated_ant_now(const AiRuntime &runtime) {
    const auto &state = runtime.public_state;
    return state.coins[runtime.player] >= kAggressiveAntUpgradeCoinThreshold &&
           state.bases[runtime.player].ant_level < 2;
}

void perform_self_turn(AiRuntime &runtime, ProtocolIO &io) {
    std::vector<Operation> proposed;
    if (should_upgrade_generated_ant_now(runtime)) {
        proposed.emplace_back(OperationType::UpgradeGeneratedAnt);
    }

    const auto v3_ops = compute_turn(runtime);
    proposed.insert(proposed.end(), v3_ops.begin(), v3_ops.end());
