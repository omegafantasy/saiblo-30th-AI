...
assemble_layout() {
  ...
  mkdir -p "$output_dir"
  copy_file_mapping "$output_dir" "${ANT_GAME_ROOT}/AI/main.py:main.py"
  copy_file_mapping "$output_dir" "${ANT_GAME_ROOT}/AI/common.py:common.py"
  copy_file_mapping "$output_dir" "${ANT_GAME_ROOT}/AI/protocol.py:protocol.py"
  copy_tree_mapping "$output_dir" "${ANT_GAME_ROOT}/SDK:SDK"
  copy_tree_mapping "$output_dir" "${ANT_GAME_ROOT}/tools:tools"
...
case "$TARGET" in
  ...
  cpp_lure_v2)
    ...
    ;;
  cpp_lure_v3)
    ARCHIVE_NAME="ai_cpp_lure_v3.zip"
    CPP_AI_ROOT="${SCRIPT_DIR}/cpp_lure_v3"
    make -C "${CPP_AI_ROOT}" -j2 >/dev/null
    FILE_MAPPINGS=(
      "${SCRIPT_DIR}/ai_cpp_protocol.py:ai.py"
