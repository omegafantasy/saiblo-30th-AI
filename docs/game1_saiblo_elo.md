...
5. 对 `评测成功` 且缺少回合数/最终血量的 Game1 对局临时下载 replay，解析后只保存元信息；详情轮询保持串行，replay 下载/解析默认最多 `3` 并发，主线程串行写 SQLite。
6. 对 `准备中`、`评测中`、`队列中` 等未完成状态维护等待列表，按低频 backoff 轮询。
7. 对 `评测失败` 等终态失败保存状态和错误，不纳入 Elo。
8. 补局请求创建后也会先写入等待列表，后续仍走同一套详情/回放解析流程。
