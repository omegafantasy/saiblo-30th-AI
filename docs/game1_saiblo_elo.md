...
- 自动补局默认排除用户名 `theend` 的 AI：它仍会被爬取、记录并参与已有对局的 Elo 统计，但不会被选作补局目标或补局对手，避免主动暴露我方版本强度。排除补局不等于从榜单隐藏；若该用户名下的版本暂无跨版本有效样本，也会以默认 `1500` 分显示在榜中。

历史回填：

- 若某个用户名的历史成功局已经进库，但 replay 元信息还没补完，可用 `python3 saiblo_game1_elo.py backfill --username <用户名>` 触发补回放元信息。`thebeginning` 现在按普通 AI 处理，不再被默认排除。
