...
更新时间：`2026-05-10 05:55 UTC+8 / 2026-05-09 21:55 UTC`
...
- 新增 Yuan 大答案假设：`n598a-d` 分别把最终答案指向失忆侦探本人、保安/奇怪网站来源、生物馆跑出者、教师/行政名额链；`n598e` 在完整骨架中测试保安/网站来源是否是跨案隐藏链核心。
- 新增窄触发后续链：`n599a-c` 用旧 `n559/n562` Yuan 触发方式先复现 `705`，再分 source custody、身份法医/DNA 追 `706+`；`n599d` 将该链放回完整骨架；`n599e-f` 在 Poker `401/402` 后分别追车辆 `404->405`、转账 `501->502`、警方卷宗和 Joker 数字账号链。
- 新增 Poker 深层后续链：`n600a-e` 根据新观察到的 `502/503/504/601/602/603/604`，在 `401/402` 后复现“景观 -> 501-504 -> 于书华/医生 -> 601-604”，并测试梅花5/医生两个最终答案方向和 `605+` 续证。
...
- `Game2/tools/make_n598_candidates.py`
- `Game2/tools/make_n599_candidates.py`
- `Game2/tools/make_n600_candidates.py`
...
- watcher 队列共 `112` 个标签，均有对应 `Game2/deepclue_ai/<label>/ai.py`。
- `112` 个标签全部通过 Python `compile()` 检查。
- watcher 标签全部被 `summarize_late_probe_results.py` 覆盖。
- `scripts/game2_late_probe_retry.sh` 通过 `bash -n`。
- 当前后台 watcher 已用 `nohup + setsid` 重启为五队列并发版本，PID `426830`；必须仍通过 `thebeginning` profile 守卫后才上传。最新一次已通过 profile 守卫并开始上传/开房间评测。

## 当前运行状态

Saiblo `/api/profile/` 已恢复一次并返回 `thebeginning`。`n576/n578/n586/n588/n577e` 第一批已拿到少量房间结果但未突破；`n600a` 单样本 Poker 隔离拿到 `547`，证据扩到 `404/501/502/503/504/601/602/603/604/606`。`n599/n600` 已加入后续 watcher 配置并可继续单独开房间评测。若后续 API 再次 timeout，脚本仍保留 profile 守卫和 `300s` 退避，不绕过账号守卫。

额外安全处理：发现另线程遗留 `sk*` watcher 会继续创建 Saiblo 可见的非中性 `sk548...` 房间，已停止运行中的 `sk*` 上传/房间评测进程；未修改其文件或结果。
...
- `n598a-e`：补 Yuan 最终答案层的大方向，而不是继续追问现有证词。若 `705` 已经能出但分数不稳定，可能问题不是 stage holder，而是最终凶手/动机/手法没有命中隐藏答案层；因此分别测试“侦探本人/保安网页源/生物馆跑出者/教师行政链/完整骨架保安源”五类互斥答案。
- `n599a-f`：补“旧窄触发 + 后续链”大方向。Yuan 侧不先全局扫，而是复用旧版本能偶发 `705` 的简短问题，再把 `705` 的报告来源、蓝色背包/海豚挂件、DNA、1919车、保安网页、生物馆监控和警方旧案作为 `706+` 入口；Poker 侧在 `401/402` 后独立追车辆、转账、官方卷宗和 Joker 数字账号，不把它们合并成一个泛化问题。
- `n600a-e`：确认 Poker `502` 之后还有 `503/504/601/602/603/604/606`，因此新目标从“是否存在 502”上移到“如何稳定打开 series6 与答案层”。关键信息是 `502` 梅花5与 Joker 的 8:50 衣帽间聊天、`503` 特殊邀请函、`504` LYZ 项链、`601/602` 失踪/人口贩卖报道、`603/604` 张子韩/刘丽雯背景、`606` Joker/于书华/接待者三人 POKER 纹身照片。当前单独 Poker 分数只到 `547`，但它已经显著拓展了 Poker 理论上限和剧情上限；下一步应测 `n600b/c` 的梅花5/医生答案层以及 `605` 缺口。
