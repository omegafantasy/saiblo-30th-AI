...
这份文档描述当前 `Game1/antgame_ai_cpp/cpp_heavy_baseline` 实际使用的搜索与估值逻辑。2026-04-27 起，baseline 已被当前 v2 完全覆盖；清理后 `cpp_lure_v2` 源码目录和打包目标已删除。`cpp_lure_v3` 在此冻结点之外独立迭代。
...
- v3：`Game1/antgame_cpp_sdk/include/antgame_sdk/lure_strategy_v3_params.hpp`

旧 `lure_strategy_baseline_params.hpp` 已删除，不再维护第二套参数。当前实现里，策略 header 不应再包含未暴露到参数文件的隐藏评分常数。

## 9. v3 进攻性回避试验

v3 当前不改变 v2 的普通搜索逻辑，只在最优 root action 选出后做一次后处理：
...
debug summary 会输出 `v3_evasion_used`、`v3_evasion_reason`、`v3_evasion_worker_count`、`v3_evasion_x/y`、`final_first`、`final_pretty` 等字段，用于确认是否真实生效。
...
- `Game1/antgame_cpp_sdk/include/antgame_sdk/lure_strategy_v3.hpp`
- `Game1/antgame_cpp_sdk/include/antgame_sdk/lure_strategy_v3_params.hpp`
...
- 主动 `EMP/Deflector` 还没纳入搜索
- 主动 `Emergency Evasion` 当前只作为 v3 后处理，不在 rollout 内评估长期收益
...
同日 v3 vs baseline 进攻性回避测试。结果目录沿用当时的历史命名：
