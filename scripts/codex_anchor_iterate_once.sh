...
PROMPT_FILE="$ROOT_DIR/docs/codex_anchor_iteration_prompt.md"
OBJECTIVE_FILE="$ROOT_DIR/docs/codex_anchor_objective_fixed.md"
RUNBOOK_FILE="$ROOT_DIR/docs/codex_automation_runbook.md"
LOG_DIR="$ROOT_DIR/autolab/runtime"
LOG_FILE="$LOG_DIR/codex_anchor_iterate.log"
LAST_FILE="$LOG_DIR/codex_anchor_last_message.txt"
SESSION_FILE="$LOG_DIR/codex_anchor_session_id.txt"
EVENTS_FILE="$LOG_DIR/codex_anchor_events.jsonl"
PAUSE_FILE="${CODEX_ANCHOR_ITER_PAUSE_FILE:-$LOG_DIR/codex_anchor_iterate.paused}"
...
TIMEOUT_SEC="${CODEX_ANCHOR_ITER_TIMEOUT_SEC:-1500}"
...
TMP_JSON="$(mktemp "$LOG_DIR/codex_anchor_exec_XXXXXX.jsonl")"
TIMEOUT_LABEL="none"
...
echo "$(date -u +'%Y-%m-%dT%H:%M:%SZ') | START codex anchor iteration mode=${MODE} timeout=${TIMEOUT_LABEL} sid=${SID:-none}" >> "$LOG_FILE"
...
if [[ $rc -eq 124 ]]; then
  echo "$(date -u +'%Y-%m-%dT%H:%M:%SZ') | END codex anchor iteration rc=$rc (timeout)" >> "$LOG_FILE"
else
  echo "$(date -u +'%Y-%m-%dT%H:%M:%SZ') | END codex anchor iteration rc=$rc" >> "$LOG_FILE"
fi
exit $rc
