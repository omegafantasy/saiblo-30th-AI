...
double evaluate_state(const State& st, int player, int enemy_moves) {
    ...
    if (st.my_main_x >= 0) {
        ...
        score -= threat[st.my_main_x][st.my_main_y] * 1.2;
        score += st.board[st.my_main_x][st.my_main_y].army * 0.7;
    }

    if (st.enemy_main_x >= 0) {
        if (st.board[st.enemy_main_x][st.enemy_main_y].owner == player) {
            score += 5000.0;
        } else {
            int best_dist = 99;
            for (int x = 0; x < kRow; ++x) {
                for (int y = 0; y < kCol; ++y) {
                    ...
                    if (c.owner == player && c.army > 0) {
                        best_dist = std::min(best_dist, manhattan(x, y, st.enemy_main_x, st.enemy_main_y));
                    }
                }
            }
...
double simulate_sequence_score(const State& init, int player, int enemy_moves, const std::vector<Candidate>& seq) {
    State st = init;
    for (const auto& c : seq) {
        apply_move(st, player, c);
    }
    return evaluate_state(st, player, enemy_moves);
}
...
int main() {
    ...
    while (std::getline(std::cin, line)) {
        ...
        //    hybrid planner: beam sequence + greedy sequence arbitration.
        ...
        if (!beam_seq.empty()) {
            const double greedy_score = simulate_sequence_score(st, seat, enemy_moves, greedy_seq);
            const double beam_score = simulate_sequence_score(st, seat, enemy_moves, beam_seq);
            // Use beam only when it predicts a clear advantage.
            if (beam_score >= greedy_score + 8.0) {
                seq = beam_seq;
            }
        }
