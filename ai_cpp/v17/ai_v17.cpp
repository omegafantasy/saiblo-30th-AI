...
int main() {
    ...
    while (std::getline(std::cin, line)) {
        ...
        const bool exchange_guard_window =
            ...
            anti.beam_like_prob >= 0.32 &&
            anti.beam_like_prob <= 0.78;
        const double safety_delta_floor = std::clamp(
            -1.00 + 0.80 * anti.initiative_gate - 0.60 * anti.alert_effective + (beam_family_latched ? -0.36 : 0.22),
            -2.20,
            0.40
        );
        ...
        auto consider_seq = [&](const std::vector<Candidate>& cand_seq, double bias, bool apply_exchange_guard) {
            if (cand_seq.empty()) return;
            double adjusted_bias = bias;
            if (apply_exchange_guard && exchange_guard_window) {
                const double safety_delta = estimate_first_step_safety_delta(st, seat, enemy_moves, cand_seq);
                if (safety_delta < safety_delta_floor) {
                    const double deficit = safety_delta_floor - safety_delta;
                    adjusted_bias -= deficit * (2.40 + 1.20 * anti.alert_effective);
                    if (deficit >= 1.35) return;
        ...
        consider_seq(counter_seq, counter_bias, true);
        consider_seq(beam_seq, beam_bias, true);
