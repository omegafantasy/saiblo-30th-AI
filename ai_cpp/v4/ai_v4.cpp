...
double evaluate_state(const State& st, int player, int enemy_moves) {
    ...
    if (my_main.first >= 0) {
        ...
        score -= threat[my_main.first][my_main.second] * 1.2;
        score += st.board[my_main.first][my_main.second].army * 0.7;
    }
...
double evaluate_after_enemy_response(const State& st_after_my, int player, int enemy_moves, int my_moves) {
    ...
    const auto enemy_cands = collect_top_moves(st_after_my, enemy, enemy_threat, enemy_reserve, 3);
...
std::vector<Candidate> plan_moves_counterfactual_opening(const State& init, int player, int enemy_moves, int move_budget) {
    if (move_budget <= 0) return {};
    ...
    Grid threat = compute_threat(st, 1 - player, enemy_moves);
    int main_safe_reserve = compute_main_safe_reserve(st, player, threat);
    auto opening_cands = collect_top_moves(st, player, threat, main_safe_reserve, 8);

    Candidate best_opening;
    double best_opening_score = -1e100;
    for (const auto& c : opening_cands) {
        if (!c.ok) continue;
        State next = st;
        apply_move(next, player, c);
        double score = score_state_robust(next, player, enemy_moves, my_moves);
        score += 0.10 * c.score;
        if (score > best_opening_score) {
            best_opening_score = score;
            best_opening = c;
        }
    }

    if (best_opening.ok) {
        seq.push_back(best_opening);
        apply_move(st, player, best_opening);
    }

    for (int step = 1; step < move_budget; ++step) {
        threat = compute_threat(st, 1 - player, enemy_moves);
        main_safe_reserve = compute_main_safe_reserve(st, player, threat);
        Candidate cand = select_best_move(st, player, threat, main_safe_reserve);
        if (!cand.ok || cand.score < 1.0) break;
        seq.push_back(cand);
        apply_move(st, player, cand);
...
int main() {
    ...
    while (std::getline(std::cin, line)) {
        ...
        //    layered planner: counterfactual opening + beam + greedy arbitration.
        ...
        auto counter_seq = plan_moves_counterfactual_opening(st, seat, enemy_moves, move_budget);
