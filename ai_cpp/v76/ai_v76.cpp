...
Candidate select_best_move_overlay(
    ...
    for (auto cand : pool) {
        ...
        // Generals threat-origin mapping: prune low-priority high-drop branches before deeper reply search.
        // ANTWar reserved mapping: in calm endgame, tighten branch budget to avoid volatile switches.
        const double precheck_raw_drop_cap = tuning.followup_raw_drop_cap + 3.0 - 2.0 * tuning.endgame_stable_signal;
