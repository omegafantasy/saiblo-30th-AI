...
OverlayTuning choose_overlay_tuning(
    ...
    // ANTWar global-state mapping: late rounds keep more reserve, but deficit state keeps a small release floor.
    // This keeps one continuous scalar (no new branch state) while reducing late over-conservatism vs comeback lines.
    const double late_release_floor = 0.20 + 0.45 * behind_signal;
