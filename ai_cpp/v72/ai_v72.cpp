...
struct OverlayTuning {
    ...
    double endgame_lock_signal = 0.0;
};
...
OverlayTuning choose_overlay_tuning(
    ...
    const double endgame_lock = ahead_signal * endgame_signal;
    ...
    // ANTWar global-state mapping: when leading in late game, reserve more and avoid high-variance switches.
    tuning.endgame_lock_signal = endgame_lock;
    tuning.switch_margin += 0.45 * tuning.endgame_lock_signal;
    tuning.base_anchor_penalty += 0.08 * tuning.endgame_lock_signal;
    tuning.followup_raw_drop_cap -= 2.0 * tuning.endgame_lock_signal;
    tuning.base_reply_veto_slack -= 1.5 * tuning.endgame_lock_signal;
    tuning.base_reply_drop_scale -= 0.04 * tuning.endgame_lock_signal;
...
Candidate select_best_move_overlay(
    ...
    for (auto cand : pool) {
        ...
        if (!cand_is_base && tuning.base_reply_veto_enabled) {
            ...
            const double lock_penalty = tuning.endgame_lock_signal * std::max(0.0, raw_drop - 2.0);
            // Generals threat-origin mapping: collapse hard veto branches into one continuous risk gate.
