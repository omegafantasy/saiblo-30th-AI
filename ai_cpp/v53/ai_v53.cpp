...
Candidate select_best_move_base(const State& st, int player, const Grid& threat, int main_safe_reserve) {
    Candidate best;
        ...
        for (int sy = 0; sy < kCol; ++sy) {
            ...
            for (int d = 0; d < kDirs; ++d) {
                ...
                if (max_send >= need) {
                    ...
                    if (st.enemy_main_x >= 0) score += std::max(0, 20 - manhattan(nx, ny, st.enemy_main_x, st.enemy_main_y));
                    if (dst.type == 1 && st.tech[player][2] == 0) score -= 4.0;
                    score += (threat[sx][sy] - threat[nx][ny]) * 0.4;
                    score -= send * 0.65;
                    push_candidate(pool, {true, sx, sy, d, send, score}, limit);
...
Candidate select_best_move_overlay(
    ...
    Candidate base = select_best_move_base(st, player, enemy_threat, main_safe_reserve);
    if (!base.ok) return base;

    auto pool = collect_move_candidates_base(st, player, enemy_threat, main_safe_reserve, 10);
    ...
    for (auto cand : pool) {
        ...
        const Candidate enemy_best = select_best_move_base(after, enemy, my_threat_after, enemy_reserve);
        const Candidate my_follow = select_best_move_base(after, player, enemy_threat_after, my_reserve_after);

        double overlay_score = cand.score;
        overlay_score += 0.32 * std::max(0.0, my_follow.score);
        overlay_score -= 0.56 * std::max(0.0, enemy_best.score);
        if (st.my_main_x >= 0) {
            overlay_score += (enemy_threat[st.my_main_x][st.my_main_y] - enemy_threat_after[st.my_main_x][st.my_main_y]) * 0.42;
        }
        if (cand.score >= 550.0) overlay_score += 500.0;  // keep instant main-kill bias dominant

        if (overlay_score > best_overlay_score + 0.8) {
    ...
    while (std::getline(std::cin, line)) {
        ...
        for (int step = 0; step < move_budget; ++step) {
            ...
            Candidate cand = select_best_move_overlay(
                st, seat, enemy, move_budget, enemy_moves, threat, main_safe_reserve
            );
