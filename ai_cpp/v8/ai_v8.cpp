...
ModeSignals analyze_strategy_mode(const State& st, int player, int enemy_moves) {
    ...
    if (sig.pressure_ratio >= 0.90 || (sig.pressure_ratio >= 0.75 && sig.army_ratio < 0.95)) {
...
double mode_switch_confidence(const ModeSignals& sig, StrategyMode target) {
    if (target == StrategyMode::kDefense) {
        return std::max(0.0, sig.pressure_ratio - 0.78) + std::max(0.0, 1.0 - sig.army_ratio) * 0.45;
    }
...
int main() {
    ...
    while (std::getline(std::cin, line)) {
        ...
        auto consider_seq = [&](const std::vector<Candidate>& cand_seq, double bias) {
        ...
        consider_seq(counter_seq, counter_bias);
        consider_seq(beam_seq, beam_bias);
