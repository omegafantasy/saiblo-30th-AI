...
int main() {
    ...
    while (std::getline(std::cin, line)) {
        ...
        const double shell_safety_floor = base_main_safety - shell_safety_drop_cap;
        ...
        auto consider_seq = [&](const std::vector<Candidate>& cand_seq, double bias, bool apply_exchange_guard) {
            ...
            if (apply_exchange_guard && exchange_guard_window) {
                ...
                const double next_main_safety = base_main_safety + safety_delta;
                if (next_main_safety < shell_safety_floor) return;
