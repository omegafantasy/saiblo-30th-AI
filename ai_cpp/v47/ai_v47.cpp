...
int main() {
    ...
    while (std::getline(std::cin, line)) {
        ...
        static int arb_v10_counter_streak = 0;
        ...
        const bool hardline_recovery_base_window =
            !v2_narrow_burst_window &&
            mode_sig.mode != StrategyMode::kDefense &&
            !beam_family_latched &&
            anti.beam_like_prob >= 0.54 &&
            anti.beam_like_prob <= 0.76 &&
            raw_mode_sig.pressure_ratio >= 0.72 &&
            raw_mode_sig.pressure_ratio <= 0.90 &&
            anti.alert_effective >= 0.36 &&
            anti.alert_effective <= 0.82 &&
            anti.initiative_gate >= 0.58 &&
            anti.initiative_gate <= 0.84 &&
            raw_mode_sig.chain_pressure >= 14.0 &&
            raw_mode_sig.chain_pressure <= 16.8 &&
            hardening_intensity >= 0.34 &&
            !high_pressure_counterline_strong_guard;
        const bool hardline_pulse_recovery_window =
            hardline_recovery_base_window &&
            chain_jump_norm >= 0.24 &&
            density_jump_norm >= 0.20;
        const bool hardline_stable_recovery_window =
            hardline_recovery_base_window &&
            !hardline_pulse_recovery_window &&
            chain_jump_norm <= 0.28 &&
            density_jump_norm <= 0.26 &&
            raw_mode_sig.attack_signal >= 13.8;
        const bool v1_guardrail_window =
            anti.beam_like_prob >= 0.42 &&
            anti.beam_like_prob <= 0.56 &&
            raw_mode_sig.pressure_ratio >= 0.62 &&
            raw_mode_sig.pressure_ratio <= 0.78 &&
            anti.initiative_gate >= 0.66 &&
            raw_mode_sig.chain_pressure <= 14.8 &&
            raw_mode_sig.attack_signal >= 12.0 &&
            raw_mode_sig.attack_signal <= 18.0 &&
            chain_jump_norm <= 0.18 &&
            density_jump_norm <= 0.16 &&
            hardening_intensity <= 0.44;
        const bool v1_guardrail_release_window =
            v1_guardrail_window &&
            anti.beam_like_prob >= 0.50 &&
            raw_mode_sig.pressure_ratio >= 0.68 &&
            raw_mode_sig.chain_pressure >= 13.8 &&
            raw_mode_sig.attack_signal >= 14.0 &&
            (raw_mode_sig.advance_density >= 0.54 || arb_low_jump_attack_streak >= 3);
        const bool v6_cap_risk_window =
            v1_guardrail_release_window &&
            raw_mode_sig.pressure_ratio >= 0.70 &&
            raw_mode_sig.pressure_ratio <= 0.88 &&
            raw_mode_sig.chain_pressure >= 13.8 &&
            raw_mode_sig.chain_pressure <= 16.0 &&
            anti.alert_effective >= 0.40 &&
            hardening_intensity >= 0.22;
        const double v6_cap_risk = v6_cap_risk_window ?
            std::clamp(
                0.45 * std::clamp((raw_mode_sig.pressure_ratio - 0.70) / 0.18, 0.0, 1.0) +
                0.35 * std::clamp((raw_mode_sig.chain_pressure - 13.8) / 2.2, 0.0, 1.0) +
                0.20 * std::clamp((anti.alert_effective - 0.40) / 0.32, 0.0, 1.0),
                0.0,
                1.0
            ) : 0.0;
        const double v6_bonus_limit_scale = std::clamp(1.0 - 0.32 * v6_cap_risk, 0.64, 1.0);
        const double pulse_bonus_limit_scale = std::clamp(1.0 - 0.16 * v6_cap_risk, 0.78, 1.0);
        const double hardline_transition_scale =
            v1_guardrail_release_window ? 1.01 :
            (v1_guardrail_window ? 0.97 :
            (hardline_stable_recovery_window ? 1.04 : (hardline_pulse_recovery_window ? 0.94 : 1.0)));
        const double hardline_midpressure_scale =
            v1_guardrail_release_window ? 1.04 :
            (v1_guardrail_window ? 0.98 :
            (hardline_stable_recovery_window ? 1.06 : (hardline_pulse_recovery_window ? 1.10 : 1.0)));
        const double hardline_v6_scale =
            v1_guardrail_release_window ? 1.03 :
            (v1_guardrail_window ? 0.99 :
            (hardline_stable_recovery_window ? 1.06 : (hardline_pulse_recovery_window ? 1.02 : 1.0)));
        const double hardline_rebound_scale =
            v1_guardrail_release_window ? 1.02 :
            (v1_guardrail_window ? 0.97 :
            (hardline_stable_recovery_window ? 1.05 : (hardline_pulse_recovery_window ? 0.96 : 1.0)));
        ...
        if (v1_guardrail_release_window) {
            safety_delta_floor = std::clamp(safety_delta_floor - 0.05, -2.30, 0.65);
            guard_penalty_slope = std::max(0.82, guard_penalty_slope - 0.06);
            guard_reject_deficit = std::min(2.34, guard_reject_deficit + 0.08);
        }
        if (hardline_stable_recovery_window && (!v1_guardrail_window || v1_guardrail_release_window)) {
            safety_delta_floor = std::clamp(safety_delta_floor - 0.03, -2.30, 0.65);
            guard_penalty_slope = std::max(0.84, guard_penalty_slope - 0.04);
            guard_reject_deficit = std::min(2.32, guard_reject_deficit + 0.05);
        }
        ...
        const bool v19_shellfloor_guard_window =
            mode_sig.mode != StrategyMode::kDefense &&
            !beam_family_latched &&
            anti.beam_like_prob >= 0.44 &&
            anti.beam_like_prob <= 0.80 &&
            raw_mode_sig.pressure_ratio >= 0.66 &&
            raw_mode_sig.pressure_ratio <= 0.92 &&
            anti.alert_effective >= 0.24 &&
            anti.alert_effective <= 0.84 &&
            anti.initiative_gate >= 0.46 &&
            raw_mode_sig.chain_pressure >= 13.0 &&
            raw_mode_sig.chain_pressure <= 16.8 &&
            chain_jump_norm <= 0.46 &&
            density_jump_norm <= 0.42 &&
            hardening_intensity <= 0.66;
        const bool v1_v5_transition_guard_window =
            mode_sig.mode != StrategyMode::kDefense &&
            !beam_family_latched &&
            anti.beam_like_prob >= 0.30 &&
            anti.beam_like_prob <= 0.56 &&
            ...
            anti.initiative_gate >= 0.56 &&
            raw_mode_sig.attack_signal >= 13.8 &&
            raw_mode_sig.chain_pressure <= 15.2 &&
            chain_jump_norm <= 0.40 &&
            density_jump_norm <= 0.36 &&
            hardening_intensity <= 0.58;
        const double v19_shellfloor_guard_base = v19_shellfloor_guard_window ?
            std::clamp(
                0.40 * std::clamp((raw_mode_sig.pressure_ratio - 0.66) / 0.26, 0.0, 1.0) +
                0.30 * std::clamp((raw_mode_sig.chain_pressure - 13.0) / 3.8, 0.0, 1.0) +
                0.30 * std::clamp((anti.beam_like_prob - 0.44) / 0.36, 0.0, 1.0),
                0.0,
                1.0
            ) : 0.0;
        double v19_shellfloor_soft_guard = v19_shellfloor_guard_base;
        double v19_shellfloor_hard_guard = v19_shellfloor_guard_base;
        if (v19_shellfloor_guard_window && v1_v5_transition_guard_window) {
            const double transition_headroom = std::clamp((anti.initiative_gate - 0.56) / 0.24, 0.0, 1.0);
            const double attack_headroom = std::clamp((raw_mode_sig.attack_signal - 13.8) / 6.0, 0.0, 1.0);
            const double soft_relax = std::clamp(
                1.0 - (0.10 + 0.14 * (0.55 * transition_headroom + 0.45 * attack_headroom)),
                0.78,
                1.0
            );
            const double hard_relax = std::clamp(
                1.0 - (0.28 + 0.24 * (0.55 * transition_headroom + 0.45 * attack_headroom)),
                0.48,
                1.0
            );
            v19_shellfloor_soft_guard *= soft_relax;
            v19_shellfloor_hard_guard *= hard_relax;
        }
        if (v19_shellfloor_guard_window) {
            const double guarded_drop_cap = 1.76 - 0.30 * v19_shellfloor_soft_guard;
            shell_safety_drop_cap = std::min(shell_safety_drop_cap, guarded_drop_cap);
        }
        ...
        if (v19_shellfloor_guard_window) {
            const double guarded_hard_margin = 0.80 - 0.14 * v19_shellfloor_hard_guard;
            shell_hard_margin = std::min(shell_hard_margin, guarded_hard_margin);
        }
        ...
        const bool v6_safety_priority_window =
            mode_sig.mode != StrategyMode::kDefense &&
            !beam_family_latched &&
            anti.beam_like_prob >= 0.34 &&
            anti.beam_like_prob <= 0.62 &&
            ...
            raw_mode_sig.pressure_ratio <= 0.88 &&
            raw_mode_sig.chain_pressure >= 13.0 &&
            raw_mode_sig.chain_pressure <= 15.8 &&
            raw_mode_sig.attack_signal >= 13.4 &&
            chain_jump_norm <= 0.42 &&
            density_jump_norm <= 0.38 &&
            hardening_intensity <= 0.58;
        const bool v27_priority_safeguard_window =
            mode_sig.mode != StrategyMode::kDefense &&
            !beam_family_latched &&
            anti.beam_like_prob >= 0.28 &&
            anti.beam_like_prob <= 0.60 &&
            raw_mode_sig.pressure_ratio >= 0.62 &&
            raw_mode_sig.pressure_ratio <= 0.86 &&
            anti.initiative_gate >= 0.50 &&
            anti.initiative_gate <= 0.78 &&
            raw_mode_sig.attack_signal >= 13.0 &&
            raw_mode_sig.chain_pressure >= 13.2 &&
            raw_mode_sig.chain_pressure <= 15.6 &&
            chain_jump_norm <= 0.46 &&
            density_jump_norm <= 0.40 &&
            hardening_intensity <= 0.56;
        const bool v9_priority_restore_window =
            v6_safety_priority_window &&
            anti.beam_like_prob <= 0.52 &&
            raw_mode_sig.attack_signal >= 14.2 &&
            anti.initiative_gate >= 0.58 &&
            anti.alert_effective <= 0.56 &&
            raw_mode_sig.advance_density >= 0.50 &&
            chain_jump_norm <= 0.34 &&
            density_jump_norm <= 0.30;
        const bool v10_counter_restore_signal =
            v9_priority_restore_window &&
            anti.beam_like_prob >= 0.40 &&
            raw_mode_sig.pressure_ratio >= 0.72 &&
            anti.initiative_gate >= 0.66 &&
            anti.alert_effective >= 0.34 &&
            raw_mode_sig.attack_signal >= 14.8 &&
            raw_mode_sig.chain_pressure >= 14.0 &&
            raw_mode_sig.advance_density >= 0.58 &&
            hardening_intensity >= 0.20;
        if (v10_counter_restore_signal) {
            arb_v10_counter_streak = std::min(6, arb_v10_counter_streak + 1);
        } else if (v9_priority_restore_window) {
            arb_v10_counter_streak = std::max(0, arb_v10_counter_streak - 1);
        } else {
            arb_v10_counter_streak = std::max(0, arb_v10_counter_streak - 2);
        }
        const bool v10_counter_restore_window =
            v10_counter_restore_signal &&
            arb_v10_counter_streak >= 2;
        const bool v27_v10_mutex_whitelist =
            v27_priority_safeguard_window &&
            anti.initiative_gate <= 0.74 &&
            raw_mode_sig.chain_pressure <= 15.4;
        const bool v10_counter_restore_effective =
            v10_counter_restore_window &&
            !v27_v10_mutex_whitelist;
        const bool v9_priority_restore_effective =
            v9_priority_restore_window && !v10_counter_restore_effective;
        const bool v6_priority_effective =
            v6_safety_priority_window && !v9_priority_restore_effective;
        const double v6_floor_block = v6_priority_effective ? 0.24 : (v9_priority_restore_effective ? 0.78 : 1.0);
        const bool counter_midpressure_floor_window =
            mode_sig.mode != StrategyMode::kDefense &&
            !beam_family_latched &&
            !high_pressure_counterline_strong_guard &&
            anti.beam_like_prob >= 0.24 &&
            ...
            raw_mode_sig.pressure_ratio >= 0.60 &&
            raw_mode_sig.pressure_ratio <= 0.84 &&
            anti.initiative_gate >= 0.52 &&
            raw_mode_sig.attack_signal >= 13.0;
        if (counter_midpressure_floor_window &&
            !rebound_suppressed) {
            const double attack_floor = std::clamp((raw_mode_sig.attack_signal - 13.0) / 7.0, 0.0, 1.0);
            const double initiative_floor = std::clamp((anti.initiative_gate - 0.52) / 0.28, 0.0, 1.0);
            const double beam_floor = std::clamp((0.58 - anti.beam_like_prob) / 0.34, 0.0, 1.0);
            double midpressure_floor =
                (0.012 + 0.020 * (0.50 * attack_floor + 0.30 * initiative_floor + 0.20 * beam_floor));
            midpressure_floor *= counterline_relax_scale;
            midpressure_floor *= v2_narrow_midpressure_scale;
            midpressure_floor *= hardline_midpressure_scale;
            midpressure_floor *= v6_floor_block;
            counter_midpressure_bonus_cap = std::max(counter_midpressure_bonus_cap, midpressure_floor);
        }
        const bool counter_transition_floor_window =
            counter_midpressure_floor_window &&
            raw_mode_sig.chain_pressure <= 14.8 &&
            chain_jump_norm <= 0.54;
        if (counter_transition_floor_window &&
            !rebound_suppressed) {
            const double initiative_floor = std::clamp((anti.initiative_gate - 0.48) / 0.30, 0.0, 1.0);
            const double hardening_relax = std::clamp((0.74 - hardening_intensity) / 0.74, 0.0, 1.0);
            double transition_floor =
                (0.010 + 0.018 * (0.60 * initiative_floor + 0.40 * hardening_relax));
            transition_floor *= v2_narrow_transition_scale;
            transition_floor *= hardline_transition_scale;
            transition_floor *= v6_floor_block;
            counter_transition_bonus_cap = std::max(counter_transition_bonus_cap, transition_floor);
        }
        if (v27_priority_safeguard_window &&
            !rebound_suppressed) {
            const double v27_pressure = std::clamp((raw_mode_sig.pressure_ratio - 0.62) / 0.24, 0.0, 1.0);
            const double v27_attack = std::clamp((raw_mode_sig.attack_signal - 13.0) / 7.0, 0.0, 1.0);
            const double v27_chain = std::clamp((15.6 - raw_mode_sig.chain_pressure) / 2.4, 0.0, 1.0);
            double v27_midpressure_floor =
                0.014 + 0.020 * (0.45 * v27_pressure + 0.35 * v27_attack + 0.20 * v27_chain);
            v27_midpressure_floor *= counterline_relax_scale;
            v27_midpressure_floor *= v2_narrow_midpressure_scale;
            v27_midpressure_floor *= hardline_midpressure_scale;
            if (v10_counter_restore_window) {
                v27_midpressure_floor *= 1.06;
            }
            counter_midpressure_bonus_cap = std::max(counter_midpressure_bonus_cap, v27_midpressure_floor);

            const double v27_initiative = std::clamp((anti.initiative_gate - 0.50) / 0.28, 0.0, 1.0);
            const double v27_hardening_relax = std::clamp((0.72 - hardening_intensity) / 0.72, 0.0, 1.0);
            double v27_transition_floor =
                0.011 + 0.017 * (0.55 * v27_initiative + 0.45 * v27_hardening_relax);
            v27_transition_floor *= v2_narrow_transition_scale;
            v27_transition_floor *= hardline_transition_scale;
            if (v10_counter_restore_window) {
                v27_transition_floor *= 1.04;
            }
            counter_transition_bonus_cap = std::max(counter_transition_bonus_cap, v27_transition_floor);
        }
        ...
        if (counter_v6_2ply_window &&
            ...
            counter_v6_2ply_bonus_cap *= v6_bonus_limit_scale;
            if (v6_priority_effective) {
                counter_v6_2ply_bonus_cap *= 1.08;
            } else if (v9_priority_restore_effective) {
                counter_v6_2ply_bonus_cap *= 1.02;
            }
        }
        ...
        const bool counter_hardline_pulse_window =
            hardline_pulse_recovery_window &&
            (!v1_guardrail_window || v1_guardrail_release_window) &&
            !rebound_suppressed &&
            anti.initiative_gate >= 0.60 &&
            raw_mode_sig.attack_signal >= 13.0 &&
            base_main_safety >= shell_soft_floor + 0.36;
        double counter_hardline_pulse_bonus_cap = 0.0;
        if (counter_hardline_pulse_window) {
            const double initiative_pulse = std::clamp((anti.initiative_gate - 0.60) / 0.22, 0.0, 1.0);
            const double attack_pulse = std::clamp((raw_mode_sig.attack_signal - 13.0) / 8.0, 0.0, 1.0);
            const double jump_pulse = std::clamp((0.65 * chain_jump_norm + 0.35 * density_jump_norm - 0.22) / 0.44, 0.0, 1.0);
            const double safety_pulse = std::clamp((base_main_safety - shell_soft_floor - 0.36) / 1.8, 0.0, 1.0);
            const double hardening_room = std::clamp((0.78 - hardening_intensity) / 0.50, 0.0, 1.0);
            counter_hardline_pulse_bonus_cap =
                (0.015 + 0.085 * (0.40 * initiative_pulse + 0.30 * attack_pulse + 0.30 * jump_pulse)) *
                (0.55 + 0.45 * safety_pulse) * hardening_room;
            if (v1_guardrail_release_window) {
                counter_hardline_pulse_bonus_cap *= 0.72;
            }
            if (v6_cap_risk_window) {
                counter_hardline_pulse_bonus_cap *= pulse_bonus_limit_scale;
            }
            if (v6_priority_effective) {
                counter_hardline_pulse_bonus_cap *= 0.84;
            } else if (v9_priority_restore_effective) {
                counter_hardline_pulse_bonus_cap *= 0.96;
            }
            if (high_pressure_counterline_weak_guard) {
                counter_hardline_pulse_bonus_cap *= 0.88;
            }
        }
        arb_prev_chain_pressure = raw_mode_sig.chain_pressure;
        arb_prev_advance_density = raw_mode_sig.advance_density;
        ...
        consider_seq(
            ...
            true,
            (counter_transition_bonus_cap + counter_midpressure_bonus_cap + counter_v6_2ply_bonus_cap + counter_hardline_pulse_bonus_cap) *
