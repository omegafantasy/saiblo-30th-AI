...
OverlayTuning choose_overlay_tuning(
    ...
    const double behind_signal = std::max(0.0, -lead_signal);
    ...
    // Generals threat-origin mapping: fold source pressure into one continuous reserve signal.
    double reserve_signal = source_pressure * (0.30 + 0.70 * ahead_signal);
    // ANTWar global-state mapping: keep a deficit release floor, but taper it by endgame/risk to avoid late over-release.
    const double late_release_floor_raw =
        (0.16 + 0.40 * behind_signal) * (1.0 - 0.45 * endgame_signal) * (0.80 + 0.20 * (1.0 - risk_alpha));
    const double late_release_floor = std::clamp(late_release_floor_raw, 0.12, 0.50);
    const double release_window = std::max(0.20 + 0.80 * (1.0 - endgame_signal), late_release_floor);
    const double release_signal =
        0.85 * behind_signal * (0.35 + 0.65 * (1.0 - risk_alpha)) * release_window;
    reserve_signal *= (1.0 - std::clamp(release_signal, 0.0, 0.90));
